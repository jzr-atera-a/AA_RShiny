            ...
            try:
                ...
                if data.get('command') == 'EXECUTE_M25':
                    ...
                    try:
                        ...
                        print(f"{'='*100}")
                        print(f"{'Time':>8} {'DistOrg':>10} {'DistEnd':>10} {'Speed':>10}  Command")
                        print(f"{'-'*100}")
                        
                        # Stream commands to Quest 3 and display
                        for i, cmd in enumerate(commands):
                            time_at_cmd = i * 0.2
                            
                            # Calculate speed from distance between points (cm/s)
                            if i < len(commands) - 1:
                                dist_to_next = math.sqrt(
                                    (points[(i+1) % len(points)]['x'] - points[i % len(points)]['x'])**2 +
                                    (points[(i+1) % len(points)]['z'] - points[i % len(points)]['z'])**2
                                )
                                speed_cm_s = (dist_to_next * 100) / 0.2  # meters to cm, divide by 0.2s
                            else:
                                speed_cm_s = 0  # Last point, stopped
                            
                            # Display in terminal
                            print(f"{time_at_cmd:8.1f} {cmd['distance_from_origin']:10.3f} "
                                  f"{cmd['distance_to_end']:10.3f} {speed_cm_s:10.1f}  "
                                  f"SERVO2:{cmd['servo2']} MOTOR1:{cmd['motor1']} MOTOR2:{cmd['motor2']}")
                            
                            # Send to Quest 3 for 3D chart update
                            await websocket.send(json.dumps({
                                'type': 'M25_COMMAND',
                                'data': {
                                    'time': time_at_cmd,
                                    'speed_cm_s': speed_cm_s,
                                    'distance_to_dest_m': cmd['distance_to_end']
                                }
                            }))
                            
                            # Wait 0.2 seconds between commands
                    ...
                    print(f"\n{'='*80}")
                    print(f"  ROBOT TRACKING STOPPED")
                    print(f"{'='*80}\n")
                
                elif data.get('command') == 'TRACKING_LOST':
                    print(f"TRACKING LOST | Searching for triangle marker...")
