...
server <- function(input, output, session) {
  ...
  # ==============================================================================
  # SEND COMMAND TO JETSON NANO (ARDUINO CONTROL)
  # ==============================================================================
  observeEvent(input$sendCommand, {
    ...
    url <- paste0("http://", current_ip, ":", current_port, "/servo")  # Use /servo endpoint for Arduino
    
    # Create JSON payload for Arduino servo control
    payload <- list(
      angle = angle,
    ...
    tryCatch({
      ...
      if (status_code(response) == 200) {
        ...
        arduino_resp <- if(!is.null(content$arduino_response)) {
          paste0("Arduino: ", content$arduino_response)
        } else {
          toJSON(content, pretty = TRUE, auto_unbox = TRUE)
        }
        
        values$nanoResponse <- arduino_resp
        values$connected <- TRUE
        
        # Log success with Arduino response
        values$commandLog <- c(
          paste0("[", timestamp, "] ✓ SUCCESS: Servo moved to ", angle, "°"),
          paste0("[", timestamp, "] ", arduino_resp),
          values$commandLog
        )
