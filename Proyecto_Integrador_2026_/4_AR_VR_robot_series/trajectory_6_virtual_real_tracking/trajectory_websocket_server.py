...
def init_debug_log():
    ...
    global DEBUG_LOG_FILE, DEBUG_LOG_PATH
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    DEBUG_LOG_PATH = f"debug_session_{timestamp}.txt"
    DEBUG_LOG_FILE = open(DEBUG_LOG_PATH, 'w', buffering=1)  # Line buffered
    
    # Write header
    DEBUG_LOG_FILE.write("="*100 + "\n")
    DEBUG_LOG_FILE.write(f"  QUEST 3 AR ROBOT CONTROL - DEBUG SESSION\n")
    DEBUG_LOG_FILE.write(f"  Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    DEBUG_LOG_FILE.write(f"  Platform: {platform.system()} {platform.release()}\n")
    DEBUG_LOG_FILE.write("="*100 + "\n\n")
    ...
    print(f"{'='*80}")
    print(f"  File: {DEBUG_LOG_PATH}")
    print(f"  All events will be logged to this file")
    print(f"{'='*80}\n")
...
def close_debug_log():
    ...
    if DEBUG_LOG_FILE:
        ...
        DEBUG_LOG_FILE.write(f"  Session ended: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        DEBUG_LOG_FILE.write("="*100 + "\n")
        DEBUG_LOG_FILE.close()
        DEBUG_LOG_FILE = None
        print(f"\nDebug log closed: {DEBUG_LOG_PATH}")
...
# MODE 1: MANUAL CONTROL STATE
manual_control_active = {}  # websocket -> True/False
manual_control_log_files = {}  # websocket -> file handle
manual_control_start_time = {}  # websocket -> datetime
...
def start_manual_control_logging(websocket):
    """Start logging manual control data to file"""
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"robot_manual_control_{timestamp}.txt"
    
    try:
        log_file = open(filename, 'w')
        # Write header
        log_file.write("# Robot Manual Control Log\n")
        log_file.write(f"# Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        log_file.write("# Columns: timestamp, left_x_cm, left_y_cm, left_z_cm, left_heading_deg, ")
        log_file.write("joystick_forward, joystick_turn, motor_speed, servo2_angle, servo1_angle\n")
        log_file.write("#" + "="*100 + "\n")
        
        manual_control_log_files[websocket] = log_file
        manual_control_start_time[websocket] = datetime.now()
        manual_control_active[websocket] = True
        
        print(f"\n{'='*80}")
        print(f"  MODE 1: MANUAL CONTROL - LOGGING STARTED")
        print(f"{'='*80}")
        print(f"  Log file: {filename}")
        print(f"  Frequency: 5 Hz (200ms)")
        print(f"  Commands: SERVO2:XX (steering), MOTORS:XX (both motors), SERVO1:XX (gripper)")
        print(f"{'='*80}\n")
...
def stop_manual_control_logging(websocket):
    """Stop logging and close file"""
    if websocket in manual_control_log_files:
        log_file = manual_control_log_files[websocket]
        
        # Write footer
        log_file.write(f"\n# Stopped: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        
        duration = (datetime.now() - manual_control_start_time[websocket]).total_seconds()
        ...
        del manual_control_log_files[websocket]
        del manual_control_start_time[websocket]
        manual_control_active[websocket] = False
        
        print(f"\n{'='*80}")
        print(f"  MODE 1: MANUAL CONTROL - LOGGING STOPPED")
        print(f"{'='*80}")
        print(f"  Log file: {filename}")
        print(f"  Duration: {duration:.1f}s")
        print(f"{'='*80}\n")

def log_manual_control_data(websocket, data):
    """Log manual control data and send to Arduino"""
    if websocket not in manual_control_log_files:
        return
    ...
    left_x = data.get('left_x_cm', 0)
    left_y = data.get('left_y_cm', 0)
    left_z = data.get('left_z_cm', 0)
    left_heading = data.get('left_heading_deg', 0)
    joy_forward = data.get('joystick_forward', 0)
    joy_turn = data.get('joystick_turn', 0)
    motor_speed = data.get('motor_speed', 0)      # Both motors same speed
    servo2 = data.get('servo2_angle', 90)         # Steering (pin 9)
    servo1 = data.get('servo1_angle', 90)         # Gripper (pin 8)
    
    # Write to log file
    log_file.write(f"{timestamp:.3f}, {left_x:.2f}, {left_y:.2f}, {left_z:.2f}, {left_heading:.1f}, ")
    log_file.write(f"{joy_forward:.3f}, {joy_turn:.3f}, {motor_speed:.1f}, {servo2:.1f}, {servo1:.1f}\n")
    log_file.flush()  # Ensure data is written immediately
    
    # Send to Arduino using ORIGINAL working format
    if arduino_serial:
        send_manual_control_to_arduino(servo2, motor_speed, servo1)
    
    # Print to console (throttled - only every 10th command)
    if int(timestamp * 5) % 10 == 0:  # 5Hz, print every 2 seconds
        print(f"[MANUAL] Joy:({joy_forward:+.2f},{joy_turn:+.2f}) Motors:{motor_speed:.0f}% Servo2:{servo2:.0f}° Servo1:{servo1:.0f}° Left:({left_x:.0f},{left_y:.0f},{left_z:.0f}cm)")

def send_manual_control_to_arduino(servo2_angle, motor_speed, servo1_angle):
    """Send manual control command to Arduino using ORIGINAL working format"""
    if not arduino_serial:
        return
    
    try:
        # Clamp values to safe ranges (matching original working system)
        servo2_angle = max(50, min(130, int(servo2_angle)))    # SERVO2: 50-130°
        servo1_angle = max(40, min(140, int(servo1_angle)))    # SERVO1: 40-140°
        
        if motor_speed <= 0:
            motor_speed = 0
        else:
            motor_speed = max(50, min(80, int(motor_speed)))  # SAFETY: 50-80% only
        
        # Send SERVO2 command (steering - pin 9)
        servo2_cmd = f"SERVO2:{servo2_angle}\n"
        arduino_serial.write(servo2_cmd.encode())
        time.sleep(0.005)
        
        # Send MOTORS command (both motors same speed)
        if motor_speed == 0:
            motor_cmd = "STOP_ALL_MOTORS\n"
        else:
            motor_cmd = f"MOTORS:{motor_speed}\n"
        
        arduino_serial.write(motor_cmd.encode())
        time.sleep(0.005)
        
        # Send SERVO1 command (gripper - pin 8)
        servo1_cmd = f"SERVO1:{servo1_angle}\n"
        arduino_serial.write(servo1_cmd.encode())
        time.sleep(0.005)
...
async def handle_client(websocket):
    ...
    debug_log(f"Quest 3 connected from {client_ip}", "INFO")
    debug_log(f"Active connections: {len(connected_clients)}", "INFO")
    ...
    try:
        async for message in websocket:
            try:
                ...
                # ── MODE 4: CONTROLLER TRACKING ──────────────────────────────────
                    ...
                    # Log to file every 10th update (~2 seconds at 5Hz)
                    if random.random() < 0.1:
                        debug_log(f"Mode 4 tracking: X={pos_data.get('x_cm', 0):.1f}cm, "
                                f"Z={pos_data.get('z_cm', 0):.1f}cm, "
                                f"Heading={pos_data.get('heading_deg', 0):.1f}°", "TRACK")
                ...
                # ── MODE SELECTION ────────────────────────────────────────────────
                    ...
                    print(f"{'='*80}\n")
                    debug_log(f"MODE {mode} ACTIVATED: {mode_name}", "MODE")
                ...
                # ── MODE 1: START MANUAL CONTROL ──────────────────────────────────
                elif data.get('command') == 'START_MANUAL_CONTROL':
                    start_manual_control_logging(websocket)
                
                # ── MODE 1: STOP MANUAL CONTROL ───────────────────────────────────
                elif data.get('command') == 'STOP_MANUAL_CONTROL':
                    stop_manual_control_logging(websocket)
                
                # ── MODE 1: MANUAL CONTROL DATA ───────────────────────────────────
                elif data.get('command') == 'MANUAL_CONTROL':
                    control_data = data.get('data', {})
                    log_manual_control_data(websocket, control_data)
        ...
        # Close manual control log if active
        if websocket in manual_control_log_files:
            stop_manual_control_logging(websocket)
...
async def main():
    ...
    init_debug_log()
    debug_log("Server starting...", "INIT")
    
    # Detect platform
    current_platform = platform.system()
    debug_log(f"Platform: {current_platform}", "INIT")
    
    # Initialize Arduino (skipped on Windows)
    arduino_connected = init_arduino()
    debug_log(f"Arduino connected: {arduino_connected}", "INIT")
...
if __name__ == '__main__':
    try:
        ...
        debug_log("Server stopped by user (Ctrl+C)", "SHUTDOWN")
        
        # Stop robot on exit
        if arduino_serial:
            print("  Stopping robot...")
            debug_log("Stopping robot and closing Arduino", "SHUTDOWN")
            send_command_to_arduino(SERVO2_CENTER, 0, 0)
