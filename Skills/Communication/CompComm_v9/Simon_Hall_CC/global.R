...
# progress_bar_item() and timeline_entry() are used by modules/conclusion.R.
# They were missing from this file (only existed in the never-sourced R/utils_common.R),
# which meant the Conclusion tab crashed the whole app on load. Defined here using the
# already-styled pct-bar / toc-item components so they render consistently with the rest of the app.
progress_bar_item <- pct_bar
timeline_entry <- function(number, title, detail) {
  toc_item(number, tagList(tags$b(paste0(title, " - ")), detail))
}
