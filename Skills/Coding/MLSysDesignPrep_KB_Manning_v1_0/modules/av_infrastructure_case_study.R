...
window.addEventListener('load', function(){
  ...
  tagList(
    ...
    div(class="meta-hero",
        ...
        tags$h2("Atera Analytics / CAM Pathfinder One — K&B Manning 2025 Framework"),
  ...
  <text x="857" y="63" text-anchor="middle" fill="#6b7280" font-size="7">WP1-WP8</text>
    ...
    fluidRow(
      box(title="Box 1 — Ch.1–2: Requirements & System Scoping (K&B)", status="primary", solidHeader=TRUE, width=12,
          id="av-box1",
          div(avBtn("av-box1","av1p1","K&B 6-Step Applied"),
          ...
          avPanel("av1p2",
            div(class="section-heading-dark", "ML Task Decomposition — K&B Ch.1"),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("K&B: Why Not a Single Model?"),
                  tags$p("K&B recommend task decomposition rather than one monolithic model:"),
                  tags$ul(
                    ...
                    tags$li(tags$b("Isaac Sim is deterministic:"), " Not ML — it is a physics engine. K&B pattern: use simulation to generate labels and features where real-world collection is infeasible or dangerous."),
          ...
          avPanel("av1p3",
            ...
            fluidRow(
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("K&B Constraint Taxonomy Applied"),
          ...
          avPanel("av1p4",
            div(class="section-heading-dark", "Innovate UK Work Package Mapping to K&B Framework"),
            tags$table(class="table table-hover",
              tags$thead(tags$tr(tags$th("WP"), tags$th("Title"), tags$th("K&B Chapter"), tags$th("ML Deliverable"))),
              tags$tbody(
    ...
    fluidRow(
      box(title="Box 2 — Ch.3: Data Pipeline Design (K&B)", status="warning", solidHeader=TRUE, width=12,
          ...
          avPanel("av2p1",
            fluidRow(
              column(6,
                div(class="section-heading-dark", "Five Data Sources — K&B Ch.3 Analysis"),
                tags$table(class="table table-sm",
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("K&B Data Source Analysis — AV Context"),
                  tags$ul(
                    tags$li(tags$b("OSM quality variance:"), " Cambridge city centre OSM is excellent. Rural roads have missing attributes (no speed limit, missing lane count). K&B: always profile data quality per geographic slice before modelling."),
                    tags$li(tags$b("Street View vintage:"), " Images may be 1-3 years old. Construction zones, new road markings not captured. K&B: document data freshness constraints in requirements."),
                    tags$li(tags$b("Expert label scarcity:"), " 500 labels is the hard constraint. K&B: with small datasets, cross-validation and SHAP overfitting detection are mandatory."),
                    tags$li(tags$b("Isaac Sim as data generator:"), " K&B pattern — use simulation to generate training data for scenarios where real-world collection is infeasible (CRITICAL hazards cannot be deliberately created).")
                  )
                )
              )
            )
          ...
          avPanel("av2p2",
            div(class="section-heading-dark", "Batch Pipeline Architecture — K&B Ch.3"),
            div(style="overflow-x:auto;",
              HTML('
<svg viewBox="0 0 800 230" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  <text x="400" y="16" text-anchor="middle" fill="#e8410a" font-size="12" font-weight="bold">AV Infrastructure — Batch Data Pipeline (K&B Ch.3)</text>
  <!-- Step 1 OSM -->
  ...
  <text x="405" y="160" text-anchor="middle" fill="#6b7280" font-size="7">~10 min</text>
          ...
          avPanel("av2p3",
            div(class="warn-box", HTML("<strong>K&B Ch.3 — The AV Data Catch-22:</strong> You cannot collect real-world CRITICAL scenario training data safely (driving AV into tunnels/roundabouts/extreme gradients to gather failure data would be dangerous). But without CRITICAL scenario data, the model cannot learn to classify them accurately. K&B document simulation as the principled solution.")),
            br(),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Sim-to-Real Gap — K&B Validation"),
                  tags$table(class="table table-sm",
                ...
                div(class="tip-box", tags$small(HTML("<strong>K&B interview point:</strong> Sim-to-real gap validation is mandatory before using simulation data in production ML. Document the gap quantitatively and flag features where sim diverges from reality.")))
              )
            )
          ),

          avPanel("av2p4",
            div(class="section-heading-dark", "Data Quality & Schema Management — K&B Ch.3"),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Schema & Versioning — K&B Pattern"),
                  tags$ul(
    ...
    fluidRow(
      box(title="Box 3 — Ch.4: Feature Engineering & Feature Store (K&B)", status="success", solidHeader=TRUE, width=12,
          ...
          avPanel("av3p1",
            div(class="section-heading-dark", "50-Feature Vector — Three Source Groups (K&B Ch.4)"),
            fluidRow(
              column(4,
                div(class="framework-card",
...
<text x="350" y="258" fill="#10b981" font-size="8">&#9632; Physics</text>
            ...
            div(class="tip-box", HTML("<strong>K&B + AV Act 2024:</strong> SHAP values are not just for debugging — they are regulatory evidence. The safety case submitted to DVSA must include SHAP explanations showing which features drove each CRITICAL classification. A score of 0.30 caused by roundabout_binary (-0.22) + yolo_roundabout_conf (-0.18) is interpretable. A neural network score of 0.30 without explanation would not be accepted."))
          ),

          avPanel("av3p3",
            div(class="warn-box", HTML("<strong>K&B Ch.4 — Train-Serve Skew in AV Context:</strong> The AV readiness model is trained on features computed from one vintage of OSM + Street View + Isaac Sim. When the model is applied to a new route, features must be computed identically to training or the prediction is invalid.")),
            br(),
            tags$table(class="table table-hover",
          ...
          avPanel("av3p4",
            div(class="section-heading-dark", "Feature Store Design — K&B Pattern for Small-Scale AV System"),
            fluidRow(
              column(6,
                div(class="framework-card",
                  tags$h5("Current Feature Store (AteraSuite v3.0)"),
                  tags$p("At 500 segments, a full Vertex AI Feature Store is not warranted. K&B principle: right-size your infrastructure."),
                  tags$ul(
                    ...
                    tags$li(tags$b("Limitation:"), " reactiveValues in R Shiny used as in-session feature cache — not persistent. K&B recommendation: migrate to proper file-based store for reproducibility.")
                  )
                )
              ),
              column(6,
                div(class="framework-card",
                  ...
                  tags$p("As the system scales to UK-wide ODD (millions of segments), K&B recommend:"),
                  tags$ul(
                    ...
                    tags$li(tags$b("K&B build vs buy:"), " Feature pipeline is bespoke (AV-specific transformations). Storage is commodity — use GCS.")
                  )
                )
              )
            )
    ...
    fluidRow(
      box(title="Box 4 — Ch.5: Modelling & Training (K&B)", status="info", solidHeader=TRUE, width=12,
          ...
          avPanel("av4p1",
            div(class="section-heading-dark", "K&B Baseline Hierarchy — AV Context"),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                ...
                div(class="warn-box", tags$small(HTML("<strong>K&B key:</strong> In the interview, the baseline hierarchy demonstrates you don't over-engineer. Each step must justify its complexity. Going straight to XGBoost without baselines is a red flag.")))
              )
            )
          ),

          avPanel("av4p2",
            div(class="section-heading-dark", "YOLOv8n Fine-Tuning — K&B Ch.5 Applied"),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Hardware Constraint — K&B Compute Budget"),
                  div(class="warn-box", tags$small(HTML("<strong>T500 4GB VRAM constraint:</strong> K&B emphasise matching model size to available hardware. YOLOv8n is the correct choice. YOLOv8s requires 8GB VRAM and would fail silently or OOM."))),
                  br(),
                  tags$table(class="table table-sm",
          ...
          avPanel("av4p3",
            div(class="section-heading-dark", "XGBoost AV Readiness Model — K&B Ch.5"),
            fluidRow(
              column(6,
                div(class="framework-card",
                ...
                div(class="framework-card",
                  tags$h5("Vehicle Type Differences — K&B Slicing"),
                  tags$p("K&B: evaluate separately for each vehicle type in the ODD."),
                  tags$ul(
                    ...
                    tags$li("K&B pattern: separate model per vehicle class, or add vehicle_type as input feature")
                  )
                )
              ),
              column(6,
                div(class="framework-card",
                  ...
                  br(),
                  div(class="tip-box", tags$small(HTML("<strong>K&B interview point:</strong> The weights (0.50/0.30/0.15/0.05) are a business decision, not a ML decision. They should be set by the AV operator and Innovate UK stakeholders, not by the data scientist. Your job is to make the weights explicit and tunable.")))
                )
              )
            )
          ),
          ...
          avPanel("av4p4",
            div(class="section-heading-dark", "HPO & Experiment Tracking — K&B Ch.5"),
            fluidRow(
              column(6,
                div(class="framework-card",
                  ...
                  div(class="warn-box", tags$small("K&B: With n=500 segments, HPO search space must be constrained. Use k=5 cross-validation for every HPO trial. SHAP overfitting detection: if top feature SHAP on train >> validation, regularise more."))
                )
              ),
              column(6,
                div(class="framework-card",
    ...
    fluidRow(
      box(title="Box 5 — Ch.6: Evaluation, Slicing & Safety Case (K&B)", status="danger", solidHeader=TRUE, width=12,
          id="av-box5",
          ...
          avPanel("av5p1",
            div(class="warn-box", HTML("<strong>K&B Ch.6 — Spatial Split Mandatory:</strong> Adjacent road segments are spatially correlated. A random train/test split would place segment A and its neighbouring segment B in different sets — the model memorises local geography rather than learning transferable features. A spatial hold-out is mandatory.")),
            br(),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Metric Selection — K&B Ch.6"),
                  tags$table(class="table table-sm",
          ...
          avPanel("av5p2",
            div(class="section-heading-dark", "Sliced Evaluation — K&B Ch.6 Mandatory for Regulatory"),
            tags$table(class="table table-hover",
            ...
            div(class="warn-box", HTML("<strong>Critical finding:</strong> Segments without Street View coverage have CRITICAL Recall = 0.68 — unacceptably low for AV safety. K&B fix: flag these segments with a 'low_visual_confidence' indicator; route planner defaults them to CRITICAL until surveyed."))
          ),
          ...
          avPanel("av5p3",
            ...
            fluidRow(
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Route Interleaving — K&B More Efficient Alternative"),
          ...
          avPanel("av5p4",
            div(class="section-heading-dark", "Calibration & AV Act 2024 Safety Case — K&B Ch.6"),
            fluidRow(
              column(6,
                div(class="framework-card",
...
<svg viewBox="0 0 280 180" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  ...
  <text x="237" y="168" text-anchor="middle" fill="#6b7280" font-size="6">0.9</text>
    ...
    # ── BOX 6: Ch.7 Serving ──
    fluidRow(
      box(title="Box 6 — Ch.7: Serving, Deployment & Phased Rollout (K&B)", status="warning", solidHeader=TRUE, width=12,
          id="av-box6",
          ...
          avPanel("av6p1",
            div(class="section-heading-dark", "Serving Architecture — K&B Ch.7 (Batch Pre-Deployment)"),
            div(class="tip-box", HTML("<strong>K&B Ch.7 Key Point:</strong> Not every ML system requires real-time inference. AV infrastructure assessment is a pre-deployment analysis — the route is assessed before the vehicle departs. Batch processing with a <20 min total SLO is entirely appropriate, and far simpler and cheaper than online serving.")),
            br(),
            tags$table(class="table table-hover",
          ...
          avPanel("av6p2",
            div(class="section-heading-dark", "Phased AV Deployment — K&B Shadow → Canary → Full"),
            fluidRow(
              column(4, div(class="framework-card", style="border-left:3px solid #3b82f6;",
                ...
                tags$ul(
                  ...
                  tags$li(tags$b("K&B:"), " Never skip shadow mode for safety-critical systems")
                )
              )),
              column(4, div(class="framework-card", style="border-left:3px solid #f59e0b;",
                ...
                tags$ul(
                  ...
                  tags$li(tags$b("K&B:"), " Canary metrics gate progression — no automatic cutover")
                )
              )),
              column(4, div(class="framework-card", style="border-left:3px solid #10b981;",
                ...
                tags$ul(
                  ...
                  tags$li(tags$b("K&B:"), " ODD-Out behaviour must be tested as explicitly as ODD-In")
                )
              ))
            )
          ),
          ...
          avPanel("av6p3",
            div(class="section-heading-dark", "Edge Deployment & Model Compression — K&B Ch.7"),
            fluidRow(
              column(6,
                div(class="framework-card",
          ...
          avPanel("av6p4",
            div(class="section-heading-dark", "Flask HTTP API Pattern — K&B Serving Fix"),
            div(class="warn-box", HTML("<strong>K&B Ch.7 — Do Not Use WebSockets in R Shiny:</strong> WebSocket connections to external services always fail in Shiny's reactive model. The correct pattern is HTTP REST with polling — a standard pattern K&B document for integrating ML backends with frontend applications.")),
            br(),
            fluidRow(
              column(6,
                div(class="framework-card",
    ...
    fluidRow(
      box(title="Box 7 — Ch.8: Monitoring, Drift & Responsible AV AI (K&B)", status="success", solidHeader=TRUE, width=12,
          id="av-box7",
          ...
          avPanel("av7p1",
            div(class="section-heading-dark", "Distribution Shift in AV Data — K&B Ch.8"),
            fluidRow(
              column(6,
                div(class="framework-card",
                  tags$h5("Covariate Shift Sources"),
                  tags$ul(
                    tags$li(tags$b("Winter snow:"), " Lane markings hidden under snow — Street View images all daytime/summer. YOLO lane_marking_quality feature collapses to zero. K&B fix: flag seasonal operation; winter ODD requires separate evaluation."),
              ...
              column(6,
                div(class="framework-card",
                  ...
                  tags$ul(
                    tags$li(tags$b("Mini-roundabouts:"), " Different YOLO signature from full roundabouts. May classify as MEDIUM instead of CRITICAL. K&B: monitor CRITICAL recall specifically on roundabout-containing segments after any YOLO architecture change."),
          ...
          avPanel("av7p2",
            div(class="section-heading-dark", "AV Degenerate Feedback Loop — K&B Ch.8"),
            div(class="warn-box", HTML("<strong>K&B Ch.8 — AV-Specific Feedback Loop:</strong> AV deployed only on LOW risk segments → telemetry data accumulates only from easy routes → model never sees hard examples in production → CRITICAL features never validated against real AV behaviour → Phase 2 expansion delayed indefinitely. K&B document this as a particularly dangerous feedback loop in safety-critical ML.")),
            br(),
            HTML('
<svg viewBox="0 0 700 170" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  <text x="350" y="15" text-anchor="middle" fill="#e8410a" font-size="11" font-weight="bold">AV Degenerate Feedback Loop — K&B Ch.8</text>
  ...
  <text x="350" y="153" text-anchor="middle" fill="#6ee7b7" font-size="9">K&B Fix: Isaac Sim generates synthetic CRITICAL data → breaks catch-22</text>
</svg>'
            ),
            br(),
          ...
          avPanel("av7p3",
            div(class="section-heading-dark", "Responsible AV AI — K&B Ch.8 + AV Act 2024"),
            fluidRow(
              column(6,
                div(class="framework-card",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Fairness requirement:"), " Model must not give a false LOW score to a rural segment simply because it lacks training data. K&B fix: explicit 'low_visual_confidence' flag when Street View unavailable — default MEDIUM, not LOW."),
                ...
                div(class="framework-card",
                  ...
                  tags$ul(
                    ...
                    tags$li("K&B: document data retention policy in model card; specify maximum storage duration per image batch")
                  )
                )
              ),
              column(6,
          ...
          avPanel("av7p4",
            div(class="section-heading-dark", "AteraSuite MLOps Stack — K&B Ch.8 Infrastructure Mapping"),
            tags$table(class="table table-hover",
              tags$thead(tags$tr(tags$th("Layer"), tags$th("K&B Category"), tags$th("Current Tool"), tags$th("Recommended (Phase 2)"))),
              tags$tbody(
            ...
            br(),
            div(class="framework-card",
              tags$h5("K&B Build vs Buy — AteraSuite Decision"),
              fluidRow(
                column(6,
    ...
    observeEvent(input$save_self, {
      ...
      output$self_result <- renderUI({
        ...
        div(class=if(pct>=80)"success-box" else "tip-box",
            ...
            if(pct < 50)  tags$p("Focus on K&B Ch.6 (spatial split + calibration for safety case) and Ch.3 (Isaac Sim as synthetic data generator). These are the most distinctive aspects of AV ML vs standard ML."),
            ...
            if(pct >= 80) tags$p("\u2705 Excellent. You can walk through the full AV infrastructure ML system with K&B framework, including regulatory constraints and responsible AI implications."))
      })
