...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(id = "tabs",
      ...
      menuItem("Ch 10 \u00b7 Deployment",                      tabName = "ch10",     icon = icon("rocket"))
    )
  ),
  dashboardBody(
    tags$head(
...
server <- function(input, output, session) {
  volvo_creds <- reactiveValues(username = "", password = "")
  overview_server("overview")
  chapter1_server("ch1", volvo_creds)
  chapter2_server("ch2")
  chapter3_server("ch3", volvo_creds)
  chapter4_server("ch4")
  chapter5_server("ch5", volvo_creds)
  chapter6_server("ch6")
  chapter7_server("ch7")
  chapter8_server("ch8", volvo_creds)
  chapter9_server("ch9", volvo_creds)
