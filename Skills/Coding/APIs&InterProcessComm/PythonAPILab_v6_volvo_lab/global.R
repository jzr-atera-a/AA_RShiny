# global.R
# Python API Development Fundamentals — Jack Chan, Ray Chung, Jack Huang (Packt, 2019)
# Shared helpers, Python execution engine, UI component builders
...
run_python_safe <- function(code_str, module_files = list()) {
  if (!python_available) {
    return("Python / reticulate not available.\nInstall Python 3 and run: install.packages('reticulate')")
  }
  tryCatch({
    ...
    result
  }, error = function(e) paste("Reticulate error:", conditionMessage(e)))
}
...
py_code_display <- function(ns) {
  tagList(
    div(class = "code-display-wrap",
      ...
      div(class = "code-scroll", verbatimTextOutput(ns("code_display"))))
  )
}
...
terminal_ui <- function(ns) {
  tagList(
    ...
    div(class = "terminal-wrap",
      ...
      div(class = "terminal-body", verbatimTextOutput(ns("py_output"))))
  )
}
...
code_lab_server <- function(input, output, session, files_list) {
  ...
  output$code_filename <- renderUI({ span(class = "code-fname", sel_file()$name) })
  output$file_desc     <- renderUI({ HTML(paste0("<div class='file-desc-text'>", sel_file()$description, "</div>")) })
  output$code_display  <- renderText({ sel_file()$code })
  output$term_label    <- renderText({ paste("output \u2014", sel_file()$name) })
  ...
  py_result <- eventReactive(input$run_btn, {
    ...
    mods <- setNames(lapply(files_list, `[[`, "code"), sapply(files_list, `[[`, "name"))
