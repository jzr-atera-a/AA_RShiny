    ...
    fluidRow(
      box(title="🧠 Semantic Kernel Memory — Full API Migration (Ch.8)", status="primary", solidHeader=TRUE, width=6,
          ...
          tags$table(class="table table-hover",
            ...
            tags$tbody(
              compare_row("sk.memory.VolatileMemoryStore", "from semantic_kernel.memory.volatile_memory_store import VolatileMemoryStore"),
              compare_row("kernel.memory", "Explicit SemanticTextMemory(storage=, embeddings_generator=)"),
              compare_row("sk.core_skills.TextMemorySkill", "TextMemoryPlugin(memory)"),
              compare_row("save_information_async()", "save_information() (no _async suffix)"),
              compare_row("TextMemorySkill.COLLECTION_PARAM", 'arguments["collection"]')
            )
          ),
          br(),
      ...
      box(title="🔗 LangChain 1.x Import Migration (Ch.8)", status="info", solidHeader=TRUE, width=6,
          ...
          tags$table(class="table table-hover",
            ...
            tags$tbody(
              compare_row("langchain.document_loaders", "langchain_community.document_loaders"),
              compare_row("langchain.text_splitter", "langchain_text_splitters"),
              compare_row("langchain.embeddings", "langchain_openai"),
              compare_row("langchain.vectorstores", "langchain_chroma"),
              compare_row(".get_relevant_documents()", ".invoke()")
            )
          ),
          br(),
