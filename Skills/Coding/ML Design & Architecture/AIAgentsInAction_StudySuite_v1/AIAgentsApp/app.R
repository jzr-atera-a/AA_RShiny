# app.R — AI Agents in Action Study Suite
# Based on: AI Agents in Action — Michael Lanham (Manning Publications)
# Modelled after the ML Design Prep Suite pattern by Chip Huyen

cat("\n╔══════════════════════════════════════════════════════╗\n")
cat("║  AI AGENTS IN ACTION — STUDY SUITE v1.0 — STARTING ║\n")
cat("╚══════════════════════════════════════════════════════╝\n\n")
...
app <- shinyApp(
  ...
  server = function(input, output, session) {
    study_mgr <- StudyManager$new()

    tryCatch({
      create_server(module_loader, study_mgr)
  ...
  options = list(
    port           = getOption("shiny.port", 3840),
