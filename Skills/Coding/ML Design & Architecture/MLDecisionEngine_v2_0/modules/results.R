    ...
    output$results_body <- renderUI({
      ...
      make_method_card <- function(m, rank, score) {
        ...
        div(class="framework-card",
          ...
          # Design patterns cross-reference
          {
            linked_pats <- Filter(function(p) m$id %in% p$ml_system_ids, DESIGN_PATTERNS)
            if(length(linked_pats) > 0) {
              fluidRow(column(12,
                div(style="background:#ede9fe22;border:1px solid #c4b5fd;border-left:4px solid #7c3aed;border-radius:8px;padding:10px;margin-top:8px;",
                  tags$b(style="color:#7c3aed;", "📐 Relevant Design Patterns: "),
                  div(style="margin-top:4px;",
                    lapply(head(linked_pats, 5), function(pat)
                      span(style=paste0("display:inline-block;background:", pat$group_colour, "22;border:1px solid ", pat$group_colour, "55;color:", pat$group_colour, ";border-radius:12px;padding:2px 10px;font-size:10px;font-weight:700;margin:2px;font-family:'JetBrains Mono',monospace;"),
                        paste0(pat$id, " ", pat$name))
                    )
                  )
                )
              ))
