...
PrepManager <- R6Class("PrepManager",
  public = list(
    ...
    initialize = function() {
      ...
      tabs <- c("intro","requirements","data_pipeline","feature_store",
                "modelling","experiment_tracking","evaluation","serving","monitoring","end_to_end","practice")
      for (t in tabs) {
    ...
    update_progress = function(tab, pct) {
      self$tab_progress[[tab]] <- min(100, max(0, as.numeric(pct)))
      self$trigger_progress()
    },

    get_progress = function(tab) { self$tab_progress[[tab]] %||% 0 },
    ...
    get_scores  = function(tab) { self$practice_scores[[tab]] %||% list() },
    save_note   = function(tab, text) { self$notes[[tab]] <- text },
    get_note    = function(tab) { self$notes[[tab]] %||% "" },

    trigger_progress = function() {
      if (!is.null(self$progress_trigger)) {
        self$progress_trigger(isolate(self$progress_trigger()) + 1)
      }
    },
    trigger_score = function() {
      if (!is.null(self$score_trigger)) {
        self$score_trigger(isolate(self$score_trigger()) + 1)
      }
    }
  )
)
