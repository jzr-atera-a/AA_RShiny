  ...
  tagList(
    ...
    tabsetPanel(
      ...
      tabPanel("📖 Theory",
        ...
        fluidRow(
          box(title = "🎓 Bridging From Traditional ML: What Actually Changes", status = "success", solidHeader = TRUE, width = 12,
              div(class = "framework-card",
                  tags$h5("1. The core workflow inversion"),
                  tags$p("In a traditional supervised pipeline, you start with a business problem, collect and label task-specific data, engineer features, train a model from a random (or lightly pretrained) initialization, then validate against a held-out test set with a fixed metric like accuracy or AUC. The model IS the deliverable — you own its weights end to end.")),
              div(class = "framework-card",
                  tags$h5("2. What replaces 'training' in AI engineering"),
                  tags$p("With foundation models, the expensive supervised-learning-like work (pretraining) has already been done by someone else, on a general task (next-token prediction) over a vastly larger and broader corpus than any single company could label. Your job shifts from 'produce a model' to 'condition an existing model correctly': via prompting (showing it examples/instructions at inference time — no gradient updates), via RAG (giving it retrieved facts as extra input), or via finetuning (a light supervised-training pass ON TOP of the pretrained weights, not from scratch — this part IS still classic supervised learning, and Ch.7 goes deep on exactly this).")),
              div(class = "framework-card",
                  tags$h5("3. Why 'is it working?' becomes harder, not easier"),
                  tags$p("In supervised classification, correctness is usually a closed, well-defined question: the predicted label either matches the true label or it doesn't, so validation is arithmetic. Foundation models produce open-ended text or actions — a helpful, well-formed answer and a subtly wrong one can look almost identical on the surface. This is precisely why Ch.3–4 exist as full chapters: evaluation is no longer 'compute accuracy on a held-out set,' it becomes its own engineering discipline.")),
              div(class = "framework-card",
                  tags$h5("4. A concrete before/after"),
                  tags$p("Traditional: 'build a spam classifier' → label 50k emails spam/not-spam → train a gradient-boosted tree or fine-tuned BERT → deploy, monitor accuracy/F1 drift. AI-engineering: 'build an inbox triage assistant' → write/version a prompt (+ few-shot examples) that conditions a pretrained model → retrieve relevant thread history via RAG → evaluate output quality with a rubric-driven judge (Ch.3) because there's no single 'correct class' to check against → only finetune later, and only on the narrow sub-parts that need it (Ch.7).")),
              div(class = "warn-box", HTML("<strong>⚠️ Common misconception to avoid in interview:</strong> AI engineering is NOT 'supervised learning got easier because we skip training.' It's a different failure surface — you trade away labeled-data/training risk for prompt-design risk, evaluation-methodology risk, and non-determinism risk. A VP of Research needs to speak fluently about all three, not just the ones that map cleanly onto prior ML experience."))
          )
        )
      ),
      ...
      tabPanel("📔 Glossary",
        br(),
        fluidRow(
          box(title = "Key Terms — Chapter 1", status = "info", solidHeader = TRUE, width = 12,
              tags$table(class = "table table-hover",
                tags$thead(tags$tr(tags$th("Term"), tags$th("Definition"), tags$th("How It Relates to What You Already Know"))),
                tags$tbody(
                  tags$tr(tags$td(tags$b("Foundation Model")), tags$td("A large model pretrained on broad, general data (usually via self-supervised next-token prediction) that can be adapted to many downstream tasks."), tags$td("Like a massively pretrained feature extractor (think ImageNet-pretrained CNN) — except the 'features' it learned are general-purpose language/reasoning patterns, and adaptation often needs no retraining at all.")),
                  tags$tr(tags$td(tags$b("AI Engineering")), tags$td("The discipline of building applications on top of foundation models — emphasising adaptation (prompting/RAG/finetuning), evaluation, and system design over training models from scratch."), tags$td("A sibling discipline to ML engineering, shifting effort from 'produce a model' to 'condition and evaluate an existing one.'")),
                  tags$tr(tags$td(tags$b("Prompting")), tags$td("Providing natural-language instructions and/or examples as input to steer a foundation model's output, without updating its weights."), tags$td("No direct traditional-ML equivalent — closest analogy is providing a very rich, dynamic feature vector at inference time instead of training on fixed features.")),
                  tags$tr(tags$td(tags$b("In-Context Learning")), tags$td("The model's ability to perform a new task based purely on examples/instructions given in the prompt, without any gradient update."), tags$td("Sometimes loosely compared to k-nearest-neighbours: the model 'looks at' the examples in context and pattern-matches, rather than storing them in updated weights.")),
                  tags$tr(tags$td(tags$b("RAG (Retrieval-Augmented Generation)")), tags$td("Retrieving relevant external content at inference time and inserting it into the model's context before generation."), tags$td("Conceptually like a traditional information-retrieval system (search engine) feeding results into a downstream model, rather than the model needing to 'know' everything from training.")),
                  tags$tr(tags$td(tags$b("Finetuning")), tags$td("Continuing supervised training on a pretrained model's weights using a smaller, task-specific labeled dataset."), tags$td("This IS classic transfer learning / supervised learning, just starting from a foundation-model checkpoint instead of random initialization — see Ch.7.")),
                  tags$tr(tags$td(tags$b("AI Engineering Stack")), tags$td("The layered architecture of an AI application: application layer (prompting/orchestration), model development layer (selection/finetuning), infrastructure layer (serving/monitoring)."), tags$td("Analogous to a traditional ML stack's feature pipeline / training pipeline / serving infra split, but with new layers (prompt management, guardrails) that traditional ML pipelines didn't need.")),
                  tags$tr(tags$td(tags$b("MVP (Minimum Viable Product)")), tags$td("The smallest version of a product that reliably delivers value, used to validate direction before expanding scope."), tags$td("Same product-management concept as in traditional software/ML products, but in AI engineering it also means deliberately limiting model autonomy, not just feature scope.")),
                  tags$tr(tags$td(tags$b("Human-in-the-Loop")), tags$td("A design pattern where a human reviews or approves AI-generated output/actions before they take effect."), tags$td("Similar in spirit to a traditional ML system with a human review queue for low-confidence predictions, except here 'confidence' itself is harder to calibrate (Ch.2's non-determinism)."))
                )
              )
          )
        )
