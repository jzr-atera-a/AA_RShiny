...
# Use Case: Social Media Recommender (Meta-scale) — Chang O'Reilly Framework
...
window.addEventListener('load', function(){
  ...
  tagList(
    ...
    div(class="meta-hero",
        ...
        tags$h2("Meta-Scale Two-Tower + DLRM — Chang O'Reilly Framework"),
...
<svg viewBox="0 0 900 200" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:900px;font-family:Inter,sans-serif;">
  ...
  <text x="647" y="148" text-anchor="middle" fill="#10b981" font-size="8">20ms budget</text>
    ...
    fluidRow(
      box(title="Box 1 — Chang Ch.1+4: Requirements & Multi-Objective Framing", status="primary", solidHeader=TRUE, width=12,
          id="sr-box1",
          div(srBtn("sr-box1","sr1p1","Multi-Objective Framing"),
              srBtn("sr-box1","sr1p2","Scale & SLOs"),
              srBtn("sr-box1","sr1p3","Chang 6-Step Applied"),
              srBtn("sr-box1","sr1p4","Regulatory Constraints")),
          hr(style="border-color:#2d3748;margin:8px 0;"),

          srPanel("sr1p1",
            div(class="warn-box", HTML("<strong>Chang Ch.1 — The Multi-Objective Trap:</strong> A naive recommender optimising only for watch-time drives radicalisation funnels, diversity collapse, and creator monopolies. Chang treat this as the canonical example of why business-ML alignment is a safety issue, not just a performance issue.")),
            br(),
            fluidRow(
              column(6,
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Metric Conflict Resolution — Chang Constrained Optimisation"),
                  tags$p("Chang recommends treating as a constrained optimisation problem:"),
                  ...
                  br(),
                  div(class="tip-box", tags$small(HTML("<strong>Chang insight:</strong> In interviews, showing you understand metric conflicts — and have a principled resolution strategy — immediately signals senior-level ML thinking.")))
                )
              )
            )
          ),
          ...
          srPanel("sr1p2",
            div(class="section-heading-dark", "Scale Requirements & SLO Breakdown — Chang Ch.4"),
            fluidRow(
              column(4,
                div(class="framework-card",
          ...
          srPanel("sr1p3",
            div(class="framework-card",
              tags$h5("Chang ML Design Framework Loop Applied — Social Recommender"),
          ...
          srPanel("sr1p4",
            fluidRow(
              ...
              column(6,
                div(class="warn-box", HTML("<strong>Chang Interview Point:</strong> The Facebook emotional contagion study (2014) is Chang case study for why A/B testing on social platforms requires ethics review. Any experiment that could alter emotional state must go through IRB-equivalent review. Mentioning this in an interview shows awareness of responsible AI at scale.")),
                div(class="framework-card",
    ...
    fluidRow(
      box(title="Box 2 — Chang Ch.4: Petabyte-Scale Data Pipeline", status="warning", solidHeader=TRUE, width=12,
          id="sr-box2",
          ...
          srPanel("sr2p1",
            fluidRow(
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Chang Streaming Architecture — Flink on Kafka"),
                  tags$ul(
                ...
                div(class="tip-box", tags$small(HTML("<strong>Chang Ch.4 Interview Point:</strong> At 100B events/day (~1.2M events/second), even Kafka requires careful capacity planning: partition count, consumer group lag monitoring, backpressure handling.")))
              )
            )
          ),

          srPanel("sr2p2",
            div(class="warn-box", HTML("<strong>Chang Ch.4 Critical Issue — Negative Sampling:</strong> 99.99% of (user, item) pairs are unobserved. A non-impression is NOT a negative — it may mean the item was never shown, not that the user dislikes it. Treating all non-interactions as negatives creates a catastrophically biased training set.")),
            br(),
            fluidRow(
              column(6,
                div(class="framework-card",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang recommendsation:"), " Mix: 70% random + 20% popularity-corrected + 10% hard negatives")
                  )
                )
              ),
              column(6,
                ...
                div(class="framework-card",
                  ...
                  div(class="warn-box", tags$small("Chang fix: Exploration budget — reserve 5-10% of impressions for random/exploratory recommendations. Collect unbiased data. Use IPS (Inverse Propensity Scoring) to debias training."))
                )
              )
            )
          ),
          ...
          srPanel("sr2p3",
            div(class="section-heading-dark", "Class Imbalance at Social Scale — Chang Ch.4"),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Chang Handling Strategy"),
                  tags$ul(
...
<svg viewBox="0 0 820 260" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  <text x="410" y="18" text-anchor="middle" fill="#e8410a" font-size="12" font-weight="bold">Social Recommender Data Pipeline — Chang Ch.4</text>
  <!-- Sources -->
  ...
  <line x1="315" y1="135" x2="315" y2="175" stroke="#ef4444" stroke-width="1" stroke-dasharray="3,3"/>
    ...
    fluidRow(
      box(title="Box 3 — Chang Ch.4: Feature Engineering & Two-Tower Design", status="success", solidHeader=TRUE, width=12,
          id="sr-box3",
...
<svg viewBox="0 0 300 200" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  ...
  <text x="150" y="178" text-anchor="middle" fill="#6b7280" font-size="8">Updated hourly via Flink embedding job</text>
            ...
            fluidRow(
              ...
              column(6,
                div(class="framework-card",
                  ...
                  div(class="warn-box", tags$small(HTML("<strong>Chang Ch.4 Cold Start Problem:</strong> A new post has no engagement history. Without engagement features, the item tower produces a generic embedding that the ANN retrieval will not surface."))),
                  br(),
                  tags$ul(
              ...
              column(6,
                div(class="framework-card", style="border-left:3px solid #3b82f6;",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang's warning:"), " Point-in-time join is expensive at this scale. Pre-join during nightly Spark job, not at training time.")
                  )
                )
              ),
              column(6,
          ...
          srPanel("sr3p4",
            div(class="warn-box", HTML("<strong>Chang Ch.4 — Train-Serve Skew at Social Scale:</strong> The two-tower model is trained on (user_features_at_T, item_features_at_T, label). At serving time, user features come from Redis (up to 1hr stale), item features from FAISS index (updated hourly). Any divergence = train-serve skew.")),
            br(),
    ...
    fluidRow(
      box(title="Box 4 — Chang Ch.3+4: Two-Tower + DLRM Architecture", status="info", solidHeader=TRUE, width=12,
          id="sr-box4",
          ...
          srPanel("sr4p1",
            fluidRow(
              column(6,
                div(class="framework-card",
                  tags$h5("Why Two-Tower? — Chang Ch.3+4"),
...
<svg viewBox="0 0 340 220" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  ...
  <text x="265" y="185" text-anchor="middle" fill="#6b7280" font-size="7">ANN search: 8ms / 10B items</text>
            ...
            fluidRow(
              column(6,
                div(class="framework-card",
                  ...
                  tags$p("Chang explain Wide-and-Deep as the dominant architecture for ranking tasks requiring both memorisation of specific patterns and generalisation."),
                  tags$ul(
              ...
              column(6,
                div(class="framework-card",
                  ...
                  br(),
                  div(class="tip-box", tags$small(HTML("<strong>Chang key:</strong> The weights w1-w5 are tuned to reflect business value, not just frequency. Shares are 10× weighted vs clicks — they are far rarer but far more valuable as engagement quality signals.")))
                )
              )
            )
          ),
          ...
          srPanel("sr4p3",
            div(class="section-heading-dark", "Multi-Task Learning — Chang Ch.3+4"),
            div(class="tip-box", HTML("<strong>Chang's principle:</strong> Multi-task learning is essential for social recommenders. A single-task model optimising only clicks will ignore safety, diversity, and long-term retention. Multi-task allows the model to learn shared representations while predicting each signal.")),
            br(),
            fluidRow(
              column(6,
                div(class="framework-card",
...
<svg viewBox="0 0 300 200" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  ...
  <text x="150" y="182" text-anchor="middle" fill="#9ca3af" font-size="7">(P_unsafe guardrail applied)</text>
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Task Interference — Chang Warning"),
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang recommendsation:"), " Start with soft parameter sharing, add task weights to loss, monitor per-task validation metrics separately")
                  )
                )
              )
            )
          ...
          srPanel("sr4p4",
            div(class="section-heading-dark", "Training Strategy — Chang Ch.3+4"),
            fluidRow(
              column(6,
                div(class="framework-card",
    ...
    fluidRow(
      box(title="Box 5 — Chang Ch.4: Evaluation, A/B Testing & Network Effects", status="danger", solidHeader=TRUE, width=12,
          id="sr-box5",
          ...
          srPanel("sr5p1",
            fluidRow(
              column(6,
                div(class="framework-card",
                  tags$h5("Offline Metric Selection — Chang Ch.4"),
                  tags$table(class="table table-sm",
          ...
          srPanel("sr5p2",
            div(class="section-heading-dark", "Sliced Evaluation — Chang Ch.4 Mandatory"),
            tags$table(class="table table-hover",
            ...
            div(class="warn-box", HTML("<strong>Chang Ch.4 Critical Finding:</strong> Safety AUC = 0.78 for low-resource languages means harmful content in these languages is significantly more likely to be surfaced. This is both a safety risk and a fairness issue — users in those language communities receive lower-quality safety protection."))
          ),

          srPanel("sr5p3",
            div(class="warn-box", HTML("<strong>Chang Ch.4 — Network Interference: The Biggest A/B Testing Challenge in Social ML.</strong> In social networks, users are connected. Treatment group users share content with control group users. This violates the SUTVA assumption (Stable Unit Treatment Value Assumption) underlying standard A/B tests.")),
            br(),
            fluidRow(
              column(6,
                div(class="framework-card",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang's framework documentsed example:"), " Meta found a 20% underestimate of feed ranking improvements due to network interference."),
              ...
              column(6,
                div(class="framework-card",
                  ...
                  tags$p("Chang recommends interleaving for ranking evaluation: merge ranked lists from control and treatment into a single feed, then measure which model's items get more engagement."),
                  tags$ul(
                ...
                div(class="tip-box", tags$small(HTML("<strong>Chang Interview Signal:</strong> Knowing that interleaving exists and why it's more efficient than A/B for ranking — while understanding its limitations — immediately signals FAANG-level system design experience.")))
              )
            )
          ),

          srPanel("sr5p4",
            div(class="section-heading-dark", "Offline-Online Metric Gap — Chang Root Cause Analysis"),
            tags$table(class="table table-hover",
              tags$thead(tags$tr(tags$th("Gap Source"), tags$th("Mechanism"), tags$th("Social Example"), tags$th("Chang Fix"))),
              tags$tbody(
    ...
    # ── BOX 6: Ch.7 Serving ──
    fluidRow(
      box(title="Box 6 — Chang Ch.6: Deployment & Serving", status="warning", solidHeader=TRUE, width=12,
          id="sr-box6",
          ...
          srPanel("sr6p1",
            div(class="section-heading-dark", "Two-Stage Serving Architecture (Chang Ch.6)"),
            fluidRow(
              column(6,
                div(class="framework-card",
          ...
          srPanel("sr6p2",
            div(class="section-heading-dark", "Model Compression — Chang Ch.6"),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="framework-card",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang:"), " PQ is the standard technique for billion-scale ANN. Know FAISS IVF-PQ configuration.")
                  )
                )
              )
            )
          ...
          srPanel("sr6p3",
            div(class="section-heading-dark", "Canary Deployment & Rollback Strategy (Chang Ch.6)"),
            fluidRow(
              ...
              column(4, div(class="framework-card",
                ...
                tags$ul(
                  ...
                  tags$li("Chang: never delete old model artifact for at least 30 days post-rollout"),
          ...
          srPanel("sr6p4",
            div(class="section-heading-dark", "Hourly Embedding Update Pipeline — Chang Stateful Serving"),
            div(class="tip-box", HTML("<strong>Chang Ch.6 Key Point:</strong> At 100B events/day, it is infeasible to retrain the full model hourly. Instead, Chang advocate for <em>stateful serving</em>: keep model weights fixed, but update embeddings incrementally using new engagement data.")),
            br(),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="framework-card",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang recommendsation:"), " Weekly full stateless retrain + hourly stateful embedding updates")
                  )
                )
              )
            )
    ...
    fluidRow(
      box(title="Box 7 — Chang Ch.6: Monitoring, Drift & Responsible AI", status="success", solidHeader=TRUE, width=12,
          id="sr-box7",
          ...
          srPanel("sr7p1",
            div(class="section-heading-dark", "Distribution Shift Monitoring — Chang Ch.6"),
            fluidRow(
              column(6,
                div(class="framework-card",
          ...
          srPanel("sr7p2",
            div(class="section-heading-dark", "Feedback Loop Detection — Chang Ch.6 Degenerate Case"),
            div(class="warn-box", HTML("<strong>Chang Ch.6 — The Recommender Feedback Loop:</strong> The model recommends popular content → popular content gets more engagement → model learns it's even better → less popular content never gets a chance. Over time, content diversity collapses.")),
            br(),
            fluidRow(
              column(6,
                div(class="framework-card",
...
<svg viewBox="0 0 300 180" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  ...
  <text x="18" y="95" fill="#9ca3af" font-size="7" transform="rotate(-90,18,95)">Gini Index</text>
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Mitigation Strategies (Chang Ch.6)"),
                  tags$ul(
          ...
          srPanel("sr7p3",
            div(class="section-heading-dark", "Creator Fairness & EU DSA Compliance — Chang Ch.6"),
            fluidRow(
              column(6,
                div(class="framework-card",
                ...
                div(class="warn-box", HTML("<strong>Chang Responsible AI Point:</strong> A recommender that is biased toward large creators and high-resource languages is not just a fairness concern — it is a business risk. Creator attrition and regulatory fines (DSA) can be more expensive than the engineering cost of fixing the bias."))
              ),
              column(6,
                div(class="framework-card",
          ...
          srPanel("sr7p4",
            div(class="section-heading-dark", "MLOps Stack — Chang Ch.6 Infrastructure"),
            tags$table(class="table table-hover",
              tags$thead(tags$tr(tags$th("Layer"), tags$th("Chang Category"), tags$th("Tool (Meta-scale)"), tags$th("Notes"))),
              tags$tbody(
    ...
    observeEvent(input$save_self, {
      ...
      output$self_result <- renderUI({
        ...
        div(class=if(pct>=80)"success-box" else "tip-box",
            ...
            if(pct < 50)  tags$p("Focus on Chang Ch.4 (negative sampling) and Ch.6 (network interference in A/B testing) — these are the most differentiated interview topics for social ML."),
            ...
            if(pct >= 80) tags$p("\u2705 Excellent. You can walk through a Meta-scale recommendation system end-to-end with the Chang framework."))
      })
