...
# Use Case: UK Banking Customers ML — Chang O'Reilly Framework
...
window.addEventListener('load', function(){
  ...
  tagList(
    ...
    div(class="meta-hero",
        ...
        tags$h2("PEGA + GCP + BigQuery + Vertex AI — Chang O'Reilly Framework"),
  ...
  <text x="400" y="245" text-anchor="middle" fill="#6b7280" font-size="8">Feedback Loop (click/conversion signals)</text>
    ...
    fluidRow(
      box(title="Box 1 — Chang Ch.1+4: ML Role Requirements & Problem Framing", status="primary", solidHeader=TRUE, width=12,
          id="bk-box1",
          div(bkBtn("bk-box1","bk1p1","Chang ML Design Framework Loop",TRUE),
          ...
          bkPanel("bk1p1",
                  div(class="success-box", HTML("<strong>Chang Ch.1 Principle:</strong> Every ML design starts with translating business language into a precise ML specification using the 6-step loop. For banking, the business goal is <em>'deliver the right message via the right channel at the right time'</em> — Chang's framework insists this must be decomposed before touching any data.")),
                  fluidRow(
                    column(6,
                           div(class="framework-card",
                               tags$h5("Chang-aligned ML Design Loop Applied to Banking"),
                    ...
                    column(6,
                           div(class="warn-box", HTML("<strong>Chang Ch.1 Warning:</strong> Do NOT jump to model selection. Spend 20% of interview time on requirements. 'What does success look like in business terms?' is always the first question.")),
                           div(class="tip-box",
          ...
          bkPanel("bk1p2",
                  div(class="section-heading-dark", "Chang ML Task Type Analysis — One Model per Problem"),
                  fluidRow(
                    column(4,
                           div(class="framework-card",
                               tags$h5("Propensity Models (Per-Product)"),
                               tags$p("Chang recommends separate binary classifiers per product, not multiclass. Why?"),
                    ...
                    column(4,
                           div(class="framework-card",
                               ...
                               tags$ul(tags$li("Input: customer features + eligible actions"), tags$li("Output: channel probability vector"), tags$li("PEGA overrides with contact cap policy on top"), tags$li("Chang's pattern: ML scores → business rules → final decision"))
                           )
                    ),
                    column(4,
                           div(class="framework-card",
                               ...
                               tags$ul(tags$li("Epsilon-greedy → Thompson Sampling (more Bayesian)"), tags$li("Reward: click within 4-hour window"), tags$li("Cold start for new customers: prior from segment"), tags$li("Chang: bandits beat static send-time rules by 15-30%"))
                           )
                    )
                  )
          ),
          ...
          bkPanel("bk1p3",
                  ...
                  fluidRow(
                    ...
                    column(6,
                           div(class="warn-box", HTML("<strong>Regulatory Constraints (Chang Ch.4 — Constraint Taxonomy):</strong>")),
                           tags$ul(
    ...
    fluidRow(
      box(title="Box 2 — Chang Ch.4: Data Acquisition & Pipeline Design", status="warning", solidHeader=TRUE, width=12,
          ...
          bkPanel("bk2p2",
                  div(class="section-heading-dark", "Chang Batch vs Streaming Decision Framework"),
                  fluidRow(
                    column(4,
                           div(class="framework-card", style="border-left:3px solid #3b82f6;",
                               ...
                               div(class="success-box", tags$small("Chang: Batch is always cheaper and simpler. Only add streaming when freshness requirement genuinely demands it."))
                           )
                    ),
                    column(4,
                           div(class="framework-card", style="border-left:3px solid #10b981;",
                               ...
                               div(class="warn-box", tags$small("Chang: Streaming introduces at-least-once vs exactly-once semantics. Use idempotent writes to BigQuery."))
                           )
                    ),
                    column(4,
                           div(class="framework-card", style="border-left:3px solid #f59e0b;",
                               ...
                               tags$ul(
                                 ...
                                 tags$li(tags$b("Chang's warning:"), " Train/serve skew risk at merge point")
                               )
                           )
                    )
                  )
          ...
          bkPanel("bk2p3",
                  div(class="section-heading-dark", "Data Quality — Chang Production Concerns (Ch.3)"),
                  fluidRow(
                    column(6,
                           div(class="framework-card",
                               ...
                               tags$ul(
                                 ...
                                 tags$li(tags$b("Chang's pattern:"), " Data contracts between producing and consuming teams. Breaking changes require 30-day notice."),
                    ...
                    column(6,
                           div(class="warn-box", HTML("<strong>Chang Data Quality Checklist (Ch.3):</strong>")),
                           tags$ul(
...
<svg viewBox="0 0 800 280" xmlns="http://www.w3.org/2000/svg" style="width:100%%;font-family:Inter,sans-serif;">
  <text x="400" y="20" text-anchor="middle" fill="#e8410a" font-size="13" font-weight="bold">Banking ML Data Pipeline — Chang Ch.4 Architecture</text>
  <!-- Batch path -->
  ...
  <line x1="430" y1="170" x2="430" y2="215" stroke="#374151" stroke-width="1" stroke-dasharray="4,3"/>
    ...
    fluidRow(
      box(title="Box 3 — Chang Ch.4: Feature Engineering & Feature Store", status="success", solidHeader=TRUE, width=12,
          ...
          bkPanel("bk3p1",
                  div(class="section-heading-dark", "Feature Taxonomy for Banking ML (Chang Ch.4 Framework)"),
                  fluidRow(
          ...
          bkPanel("bk3p2",
                  div(class="section-heading-dark", "Offline vs Online Feature Store — Chang Pattern"),
                  fluidRow(
                    column(6,
                           div(class="framework-card", style="border-left:3px solid #3b82f6;",
                    ...
                    column(6,
                           div(class="framework-card", style="border-left:3px solid #10b981;",
                               ...
                               tags$ul(
                                 ...
                                 tags$li(tags$b("Consistency:"), " Chang critical point — online store may lag offline by up to 1 hour. Accept staleness or implement dual write."),
          ...
          bkPanel("bk3p3",
                  div(class="warn-box", HTML("<strong>Chang Ch.4 — Train-Serve Skew: The Silent Model Killer.</strong> The model was trained on one distribution of feature values; at serving time, the feature computation logic differs. For banking, this is a critical production risk.")),
                  br(),
                  fluidRow(
                    column(6,
                           div(class="framework-card",
                    ...
                    column(6,
                           div(class="success-box", HTML("<strong>Chang Prevention Strategy:</strong>")),
                           tags$ul(
...
<text x="%d" y="%d" fill="%s" font-size="8">%.3f</text>',
    ...
    fluidRow(
      box(title="Box 4 — Chang Ch.3+4: Model Selection & Training", status="info", solidHeader=TRUE, width=12,
          ...
          bkPanel("bk4p1",
                  div(class="section-heading-dark", "Chang Model Selection Framework (Ch.5)"),
                  fluidRow(
                    column(6,
                           tags$table(class="table table-hover",
                    ...
                    column(6,
                           div(class="tip-box",
                               tags$h6("Chang's Key Insight: One Model Per Product"),
                               tags$p("Chang's framework strongly recommends separate models per prediction task rather than one multi-output model. For banking:"),
                               tags$ul(
                           ...
                           div(class="framework-card",
                               ...
                               tags$p("ISA propensity: ~3% conversion rate (97:3 imbalance). Chang recommendsations:"),
                               tags$ul(
                  ...
                  fluidRow(
                    column(6,
                           div(class="framework-card",
                               ...
                               tags$p("Chang's framework insists on temporal splits for any time-series-contaminated data:"),
                               tags$ul(
                    ...
                    column(6,
                           div(class="success-box", HTML("<strong>Chang Distributed Training Note:</strong> For banking, XGBoost on 10M rows x 500 features fits on a single n1-highmem-32 GCP instance (256GB RAM). No distributed training needed. Chang: don't over-engineer — add distributed training only when single-machine training exceeds 4 hours.")),
                           br(),
                           div(class="framework-card",
          ...
          bkPanel("bk4p3",
                  div(class="section-heading-dark", "Experiment Tracking — Chang Ch.3+4"),
                  tags$table(class="table table-hover",
                  ...
                  div(class="tip-box", HTML("<strong>Chang Experiment Tracking Requirements:</strong> Log every run with: data snapshot hash, feature list, hyperparameters, evaluation metrics per slice (not just aggregate), training duration, model artifact path, and the hypothesis being tested. MLflow on Vertex AI Managed MLflow."))
          ),
          ...
          bkPanel("bk4p4",
                  ...
                  fluidRow(
                    column(6,
                           div(class="framework-card",
                               tags$h5("Chang Baseline Hierarchy"),
                               tags$ol(
                               ...
                               div(class="warn-box", tags$small("Chang: Each step must justify its complexity. If LR achieves 0.71 and XGBoost achieves 0.72, the operational cost of XGBoost is not justified."))
                           )
                    ),
                    column(6,
                           div(class="framework-card",
    ...
    fluidRow(
      box(title="Box 5 — Chang Ch.4: Model Evaluation & Testing", status="danger", solidHeader=TRUE, width=12,
          id="bk-box5",
          ...
          bkPanel("bk5p1",
                  fluidRow(
                    column(6,
                           div(class="section-heading-dark", "Metric Selection — Chang Ch.4"),
                           div(class="tip-box", HTML("<strong>Chang's principle:</strong> Choose metrics that reflect the real cost of errors, not just statistical elegance. In banking, a false positive (contacting a customer with a product they don't want) costs a marketing budget. A false negative (missing a ready-to-buy customer) costs revenue.")),
                           tags$table(class="table table-sm",
          ...
          bkPanel("bk5p2",
                  div(class="section-heading-dark", "Sliced Evaluation — Chang Must-Do (Ch.6)"),
                  div(class="warn-box", HTML("<strong>Chang Ch.4 Warning:</strong> Aggregate AUC can look great while performance on a specific regulatory segment (e.g., vulnerable customers) is poor. Sliced evaluation is mandatory for FCA-regulated models.")),
                  br(),
                  tags$table(class="table table-hover",
          ...
          bkPanel("bk5p3",
                  div(class="section-heading-dark", "A/B Test Design — Chang Ch.4"),
                  fluidRow(
                    column(6,
                           div(class="framework-card",
                               tags$h5("Test Design"),
          ...
          bkPanel("bk5p4",
                  div(class="warn-box", HTML("<strong>Chang Ch.4 — The Offline-Online Metric Gap:</strong> A model with high offline AUC may not improve online business metrics. Chang's framework documents three root causes, all present in this banking system.")),
                  br(),
                  tags$table(class="table table-hover",
    ...
    fluidRow(
      box(title="Box 6 — Chang Ch.6: Deployment & Serving", status="warning", solidHeader=TRUE, width=12,
          ...
          bkPanel("bk6p1",
                  div(class="section-heading-dark", "Serving Architecture — Batch + Online Hybrid (Chang Ch.6)"),
                  fluidRow(
                    column(6,
                           div(class="framework-card", style="border-left:3px solid #3b82f6;",
                               ...
                               tags$ul(
                                 ...
                                 tags$li("Chang: pre-compute for all likely served users, not just active ones")
                               )
                           )
                    ),
                    column(6,
          ...
          bkPanel("bk6p2",
                  div(class="section-heading-dark", "Deployment Strategy — Chang Ch.6 (Shadow → Canary → Blue-Green)"),
                  fluidRow(
          ...
          bkPanel("bk6p3",
                  div(class="section-heading-dark", "PEGA Integration — Chang Serving Contract"),
                  div(class="tip-box", HTML("<strong>Chang Ch.6 Pattern:</strong> The ML system and the decisioning system (PEGA) are explicitly decoupled. ML provides <em>signals</em>, PEGA provides <em>decisions</em>. This is a critical interview point.")),
                  br(),
                  fluidRow(
                    column(6,
                           div(class="framework-card",
...
{"customer_id": "UK-123456",
 ...
 "feature_timestamp": "2025-01-15T00:00:00Z"}'
          ...
          bkPanel("bk6p4",
                  div(class="section-heading-dark", "Model Compression — Chang Ch.6 Applied"),
                  fluidRow(
                    column(6,
                           tags$table(class="table table-sm",
                    ...
                    column(6,
                           div(class="framework-card",
                               tags$h5("Chang Latency Budget Breakdown"),
                               tags$table(class="table table-sm",
    ...
    fluidRow(
      box(title="Box 7 — Chang Ch.6: Monitoring & Reliability", status="success", solidHeader=TRUE, width=12,
          id="bk-box7",
          ...
          bkPanel("bk7p1",
                  div(class="section-heading-dark", "Distribution Shift Detection — Chang Ch.6"),
                  fluidRow(
                    column(6,
                           div(class="framework-card",
                               tags$h5("PSI (Population Stability Index)"),
                               tags$p("Chang recommends PSI as the primary drift indicator for tabular models:"),
                               tags$ul(
          ...
          bkPanel("bk7p2",
                  div(class="section-heading-dark", "Alerting & Automated Retraining (Chang Ch.6)"),
                  fluidRow(
                    column(6,
                           div(class="framework-card",
                    ...
                    column(6,
                           div(class="warn-box", HTML("<strong>Chang's Warning — Retraining is NOT Free:</strong>")),
                           tags$ul(
                             ...
                             tags$li("Chang: automate as much as possible but never skip offline evaluation before redeployment"),
...
<circle cx="%d" cy="%.0f" r="3" fill="#10b981" opacity="0.8"/>',
    x+27, months[i], x+27, ay, x+27, py, x+27, ry)
          ...
          bkPanel("bk7p4",
                  div(class="section-heading-dark", "GCP MLOps Stack — Chang Ch.6 Infrastructure"),
                  tags$table(class="table table-hover",
                    tags$thead(tags$tr(tags$th("Layer"), tags$th("Chang Ch.6 Category"), tags$th("Tool (Banking)"), tags$th("Notes"))),
                    tags$tbody(
    ...
    fluidRow(
      box(title="Self-Assessment — Banking ML Case Study", status="primary", solidHeader=TRUE, width=12,
          fluidRow(
            column(4, sliderInput(ns("sc1"),"Requirements & SLOs (Chang Ch.1+4)",1,10,5)),
            column(4, sliderInput(ns("sc2"),"Data Pipeline & Features (Chang Ch.4)",1,10,5)),
            column(4, sliderInput(ns("sc3"),"Modelling & Evaluation (Chang Ch.3+4)",1,10,5))
          ),
          fluidRow(
            column(4, sliderInput(ns("sc4"),"Serving & Deployment (Chang Ch.6)",1,10,5)),
            column(4, sliderInput(ns("sc5"),"Monitoring & Drift (Chang Ch.6)",1,10,5)),
    ...
    observeEvent(input$save_self, {
      ...
      output$self_result <- renderUI({
        ...
        div(class=if(pct>=80)"success-box" else "tip-box",
            ...
            if(pct < 50) tags$p("Focus: re-read Chang Ch.1-2 and Ch.7. Requirements and serving are the most commonly tested interview areas for production banking ML."),
