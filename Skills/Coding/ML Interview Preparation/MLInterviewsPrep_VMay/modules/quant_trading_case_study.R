...
# Chang O'Reilly Framework Applied
...
window.addEventListener('load', function(){
  ...
  tagList(
    ...
    div(class="meta-hero",
        ...
        tags$h2("Mid-Frequency Systematic Equities · Alpha Research · Execution-Aware · Chang O'Reilly Framework"),
        div(
...
<svg viewBox="0 0 900 195" xmlns="http://www.w3.org/2000/svg" style="width:100%;max-width:900px;font-family:Inter,sans-serif;">
  ...
  <text x="400" y="207" text-anchor="middle" fill="#374151" font-size="7">Signal decay monitoring · Live P&amp;L feedback · Regime detection</text>
    ...
    fluidRow(
      box(title="Box 1 — Chang Ch.1+4: Requirements & Problem Scoping", status="primary", solidHeader=TRUE, width=12,
          id="qt-box1",
          div(qtBtn("qt-box1","qt1p1","Chang 6-Step Applied"),
          ...
          qtPanel("qt1p1",
            div(class="success-box", HTML("<strong>Chang Ch.1 Applied:</strong> The key translation is business goal → precise ML specification. In quant trading, the business goal is <em>generating risk-adjusted excess returns (alpha)</em>. Chang insist: before touching any data, define exactly what you are predicting, over what horizon, with what evaluation metric.")),
            br(),
            div(class="framework-card",
              tags$h5("Chang ML Design Framework Loop — Mid-Frequency Equity Alpha"),
          ...
          qtPanel("qt1p2",
            div(class="section-heading-dark", "ML Task Decomposition — Chang Ch.1 Business→ML Translation"),
            fluidRow(
              column(6,
              ...
              column(6,
                ...
                div(class="framework-card",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang freshness implication:"), " Mid-frequency allows daily batch signal generation. Real-time streaming not required for signal, only for execution.")
                  )
                )
              )
            )
          ...
          qtPanel("qt1p3",
            ...
            fluidRow(
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Chang Constraint Taxonomy Applied to Quant Trading"),
                  tags$ul(
          ...
          qtPanel("qt1p4",
            ...
            fluidRow(
              ...
              column(6,
                div(class="warn-box", HTML("<strong>Chang Responsible AI in Quant Context:</strong> Model risk in quant trading is direct financial and systemic risk. A miscalibrated model doesn't just harm a user — it can move markets, harm other participants, and create systemic risk (see: August 2007 Quant Meltdown — factor crowding caused simultaneous drawdowns across systematic funds when one fund deleveraged).")),
                div(class="framework-card",
    ...
    fluidRow(
      box(title="Box 2 — Chang Ch.4: Data Pipeline — Point-in-Time Correctness", status="warning", solidHeader=TRUE, width=12,
          id="qt-box2",
          ...
          qtPanel("qt2p1",
            fluidRow(
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Alternative Data — Chang Data Sourcing Framework"),
                  tags$p("Chang distinguish between data that is available before prediction time vs data that is revised after. In quant finance this is called the", tags$b("as-reported vs point-in-time"), "distinction."),
                  tags$ul(
                ...
                div(class="tip-box", HTML("<strong>Chang Ch.3 interview point:</strong> In quant ML, lookahead bias is the #1 backtest inflation risk. A model that appears to have Sharpe 2.5 in backtest but Sharpe 0.3 live is almost always suffering from lookahead — point-in-time data infrastructure is the fix."))
              )
            )
          ),

          qtPanel("qt2p2",
            div(class="warn-box", HTML("<strong>Chang Ch.3 — The Quant Lookahead Bias Problem:</strong> This is the most destructive data quality issue in quant ML. It is a form of train-serve skew (Chang Ch.4) applied at the data ingestion layer. There are four distinct types, each requiring a different fix.")),
            br(),
            fluidRow(
              column(6,
                tags$table(class="table table-hover",
              ...
              column(6,
                div(class="framework-card",
                  ...
                  div(class="success-box", HTML("<strong>Chang pattern:</strong> The point-in-time feature store is the single most important infrastructure investment in quant ML. Everything else is downstream."))
                )
              )
            )
          ),
          ...
          qtPanel("qt2p3",
            div(class="section-heading-dark", "Data Pipeline Architecture — Chang Batch Pattern"),
            div(style="overflow-x:auto;",
              HTML('
<svg viewBox="0 0 800 240" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  <text x="400" y="16" text-anchor="middle" fill="#e8410a" font-size="11" font-weight="bold">Quant ML Data Pipeline — Chang Ch.4 Architecture</text>
  <!-- Raw data sources -->
  ...
  <line x1="295" y1="188" x2="450" y2="188" stroke="#6b7280" stroke-width="1" stroke-dasharray="4,3" marker-end="url(#qt-arr)"/>
          ...
          qtPanel("qt2p4",
            div(class="section-heading-dark", "Data Quality — Chang Ch.4 Applied to Financial Data"),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="warn-box", HTML("<strong>Chang Exactly-Once Semantics in Quant Context:</strong> When an EOD price file is delivered and partially processed before a crash, re-processing must not double-count prices. Use idempotent feature computation keyed by (ticker, date, data_version). Critical: duplicate rows in training data create phantom factor exposures.")),
                div(class="framework-card",
                  ...
                  tags$ul(
                    ...
                    tags$li("Chang data contract: vendor schema changes require 30-day notice + parallel validation")
                  )
                )
              )
            )
    ...
    fluidRow(
      box(title="Box 3 — Chang Ch.4: Feature Engineering — Alpha Factors", status="success", solidHeader=TRUE, width=12,
          id="qt-box3",
          ...
          qtPanel("qt3p1",
            div(class="section-heading-dark", "Alpha Factor Taxonomy — Chang Ch.4 Feature Groups"),
            fluidRow(
              column(3,
                div(class="framework-card",
              ...
              column(6,
                div(class="framework-card",
                  ...
                  tags$ul(
                    ...
                    tags$li("Chang's principle: features must be comparable across the prediction universe")
                  ),
                  br(),
                  tags$table(class="table table-sm",
          ...
          qtPanel("qt3p3",
            div(class="section-heading-dark", "Feature Store Design — Chang Offline/Online Pattern"),
            fluidRow(
              column(6,
                div(class="framework-card", style="border-left:3px solid #3b82f6;",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang's pattern:"), " Feature hash in every training run — ensures reproducibility")
                  )
                )
              ),
              column(6,
                div(class="framework-card", style="border-left:3px solid #10b981;",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang's warning:"), " Training used daily OHLCV; live system uses intraday — check normalisation consistency")
                  )
                )
              )
            )
          ...
          qtPanel("qt3p4",
            div(class="section-heading-dark", "Factor Decay — Chang Concept Drift Applied to Alpha Factors"),
            div(class="tip-box", HTML("<strong>Chang Ch.4 / Ch.8:</strong> Alpha factors decay as they become crowded (more funds trade them) or as market regimes change. IC decay analysis is the quant equivalent of concept drift monitoring.")),
            br(),
            HTML('
<svg viewBox="0 0 700 200" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  ...
  <text x="205" y="187" text-anchor="middle" fill="#9ca3af" font-size="7.5">Mid-freq sweet spot: NLP decays by 5d; Momentum persists to 21d+</text>
    ...
    fluidRow(
      box(title="Box 4 — Chang Ch.3+4: Modelling in Low-SNR Environments", status="info", solidHeader=TRUE, width=12,
          id="qt-box4",
          ...
          qtPanel("qt4p1",
            div(class="section-heading-dark", "Chang Model Selection Framework — Quant Low-SNR Context"),
            div(class="warn-box", HTML("<strong>Chang Ch.5 Critical Warning:</strong> In low SNR environments (expected IC ~0.03-0.06), complex models do NOT necessarily outperform simple ones. The regularisation requirement increases with model complexity. An overfit LightGBM with IC=0.05 in backtest may deliver IC=-0.01 live. Start with interpretable baselines.")),
            br(),
            fluidRow(
              column(7,
                tags$table(class="table table-hover",
                  tags$thead(tags$tr(tags$th("Model"), tags$th("IC (backtest)"), tags$th("IC (live est.)"), tags$th("Pros"), tags$th("Chang Verdict"))),
                  tags$tbody(
              ...
              column(5,
                div(class="framework-card",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang baseline hierarchy:"), " Rank IC → Ridge → LightGBM → ensemble. Each step must justify additional complexity with out-of-sample improvement.")
                  )
                )
              )
            )
          ...
          qtPanel("qt4p2",
            div(class="section-heading-dark", "Purged Walk-Forward Cross-Validation — Chang Ch.3+4 Time Series"),
            fluidRow(
              column(6,
                div(class="warn-box", HTML("<strong>Chang Ch.5 — Standard k-Fold CV Is INVALID for financial time series.</strong> Standard k-fold shuffles data randomly. Financial data is serially correlated — a training sample from 2019 and a test sample from 2018 creates information leakage through overlapping return horizons.")),
                br(),
                div(class="framework-card",
...
<svg viewBox="0 0 380 200" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  ...
  <text x="190" y="183" text-anchor="middle" fill="#6b7280" font-size="7.5">Critical: never test on data chronologically before training data. No shuffling.</text>
          ...
          qtPanel("qt4p3",
            div(class="section-heading-dark", "Ensemble Architecture — Chang Ch.3+4"),
            fluidRow(
              column(6,
                div(class="framework-card",
                  tags$h5("Signal Ensemble (Stacking Approach)"),
                  tags$p("Chang: ensembles reduce variance at the cost of interpretability. In quant, the key question is whether the ensemble preserves factor orthogonality."),
                  tags$ol(
              ...
              column(6,
                div(class="framework-card",
                  ...
                  tags$p("Chang Ch.3+4: model selection should be informed by the deployment context. Quant markets have non-stationary regimes:"),
                  tags$table(class="table table-sm",
          ...
          qtPanel("qt4p4",
            div(class="section-heading-dark", "Experiment Tracking — Chang Ch.3+4 Applied to Quant Research"),
            tags$table(class="table table-hover",
    ...
    fluidRow(
      box(title="Box 5 — Chang Ch.4: Evaluation — Financial Metrics & Walk-Forward", status="danger", solidHeader=TRUE, width=12,
          id="qt-box5",
          ...
          qtPanel("qt5p1",
            fluidRow(
              column(6,
                div(class="section-heading-dark", "Chang Metric Selection Applied to Quant"),
                div(class="tip-box", HTML("<strong>Chang Ch.6:</strong> Choose metrics that reflect the real cost of errors. In quant ML, the cost of a bad prediction is financial loss and opportunity cost. Standard ML metrics (RMSE, AUC) are necessary but not sufficient — you must translate to financial metrics.")),
                tags$table(class="table table-sm",
              ...
              column(6,
                div(class="framework-card",
                  tags$h5("Core Quant Evaluation Metrics (Chang Financial Translation)"),
                  tags$table(class="table table-sm",
          ...
          qtPanel("qt5p2",
            div(class="section-heading-dark", "Sliced Evaluation — Chang Ch.4 Applied to Equities"),
            div(class="warn-box", HTML("<strong>Chang Ch.6 Mandatory:</strong> Aggregate Sharpe can look strong while signal completely fails in certain sectors (e.g., financials during crisis) or market-cap segments. Sliced evaluation is required for risk management.")),
            br(),
            tags$table(class="table table-hover",
          ...
          qtPanel("qt5p3",
            div(class="section-heading-dark", "Backtest vs Live Gap — Chang Offline-Online Gap in Quant Context"),
            div(class="warn-box", HTML("<strong>Chang Ch.6 — The Quant Offline-Online Gap is the industry's biggest problem.</strong> The average backtest Sharpe at hedge funds is ~2.5. The average live Sharpe of launched strategies is ~0.8. This factor-of-3 gap has well-documented sources.")),
            br(),
            tags$table(class="table table-hover",
              tags$thead(tags$tr(tags$th("Gap Source"), tags$th("Chang Parallel"), tags$th("Mechanism"), tags$th("Magnitude"), tags$th("Fix"))),
              tags$tbody(
          ...
          qtPanel("qt5p4",
            div(class="section-heading-dark", "Transaction Cost Modelling — Chang Serving Constraints"),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="framework-card",
                  ...
                  div(class="success-box", HTML("<strong>Chang Ch.6 practical implication:</strong> A signal with IC=0.05 generates gross Sharpe ~1.5 at low turnover. After 15-25 bps round-trip costs at typical participation, net Sharpe drops to ~0.8-1.0. A signal must be strong enough to survive realistic costs."))
                )
              )
            )
          )
    ...
    # ── Box 6: Ch.7 Serving ───────────────────────────────────────────────────
    fluidRow(
      box(title="Box 6 — Chang Ch.6: Execution-Aware Signal Deployment", status="warning", solidHeader=TRUE, width=12,
          id="qt-box6",
          ...
          qtPanel("qt6p1",
            div(class="section-heading-dark", "Signal Generation Pipeline — Chang Batch + Streaming Hybrid"),
            fluidRow(
              column(6,
                div(class="framework-card", style="border-left:3px solid #3b82f6;",
              ...
              column(6,
                div(class="framework-card", style="border-left:3px solid #10b981;",
                  ...
                  tags$ul(
                    ...
                    tags$li(tags$b("Chang freshness SLO:"), " Intraday signal lag < 5 minutes. Not sub-millisecond — mid-frequency does not need HFT infrastructure.")
                  )
                )
              )
            )
          ...
          qtPanel("qt6p2",
            div(class="section-heading-dark", "Portfolio Construction — Chang Serving with Business Constraints"),
            fluidRow(
              column(5,
                div(class="framework-card",
              ...
              column(7,
                div(class="framework-card",
                  tags$h5("Chang Constraint Taxonomy Applied to Portfolio Construction"),
                  tags$table(class="table table-sm",
                    tags$thead(tags$tr(tags$th("Constraint"), tags$th("Chang Category"), tags$th("Rationale"))),
                    tags$tbody(
          ...
          qtPanel("qt6p3",
            div(class="section-heading-dark", "Execution Integration — Chang Serving Architecture for Quant"),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                div(class="tip-box", HTML("<strong>Chang Ch.6 — Execution Awareness:</strong> The signal tells you WHAT to buy/sell. The execution algorithm tells you HOW. A strong alpha signal with poor execution can deliver negative net P&amp;L. Chang's lesson: serving constraints (cost, latency) are as important as model quality.")),
                div(class="framework-card",
                  tags$h5("Capacity Analysis"),
                  tags$p("Chang: every model has a capacity limit beyond which market impact overwhelms alpha."),
                  tags$ul(
          ...
          qtPanel("qt6p4",
            div(class="section-heading-dark", "Deployment Strategy — Chang Shadow → Paper → Live"),
            div(class="phase-cards",
              div(class="phase-card phase-blue",
                tags$h5("Phase 1 — Paper Trading (Shadow)"),
                tags$p(tags$b("Duration:"), " 3-6 months"),
                tags$ul(tags$li("Signal generated and logged but no real orders"), tags$li("All costs modelled (bid-ask, market impact)"), tags$li("Risk model validated against hypothetical P&L"), tags$li("Chang: shadow mode is mandatory for new models"))
              ),
    ...
    fluidRow(
      box(title="Box 7 — Chang Ch.6: Monitoring — Signal Health & Drift", status="success", solidHeader=TRUE, width=12,
          id="qt-box7",
          ...
          qtPanel("qt7p1",
            div(class="section-heading-dark", "Signal Health Monitoring — Chang Ch.6 Drift Detection"),
            fluidRow(
              column(6,
                tags$table(class="table table-hover",
...
<svg viewBox="0 0 320 140" xmlns="http://www.w3.org/2000/svg" style="width:100%;font-family:Inter,sans-serif;">
  ...
  <text x="270" y="120" fill="#6b7280" font-size="6.5">2024</text>
          ...
          qtPanel("qt7p2",
            div(class="section-heading-dark", "Factor Crowding Detection — Chang Feedback Loop in Quant Markets"),
            div(class="warn-box", HTML("<strong>Chang Ch.8 — The Quant Feedback Loop (Crowding):</strong> When many systematic funds trade the same alpha signal, the signal erodes. Worse: in a market stress event (like the August 2007 Quant Meltdown), crowded positions unwind simultaneously — creating correlated losses across all funds trading the same factors. This is systemic risk caused by model homogeneity.")),
            br(),
            fluidRow(
              column(6,
                div(class="framework-card",
              ...
              column(6,
                ...
                div(class="success-box", HTML("<strong>Chang diversification principle:</strong> Signal diversification — combining price, fundamental, sentiment, and microstructure factors reduces crowding risk because fewer competing funds will have identical factor exposure. No single factor should contribute &gt;30% of signal variance."))
              )
            )
          ),

          qtPanel("qt7p3",
            div(class="section-heading-dark", "Regime Drift — Chang Distribution Shift in Financial Markets"),
            fluidRow(
              column(6,
                div(class="framework-card",
                  tags$h5("Quant Drift Types — Chang Ch.6 Applied"),
                  tags$table(class="table table-sm",
                    tags$thead(tags$tr(tags$th("Chang Drift Type"), tags$th("Quant Equivalent"), tags$th("Example"))),
                    tags$tbody(
              ...
              column(6,
                ...
                div(class="tip-box", HTML("<strong>Chang Stateless vs Stateful:</strong> Full stateless retrain every quarter (best for regime-change resilience). Stateful incremental update on recent 30 days (best for fast signal decay adaptation). Chang recommendation: use both — quarterly stateless + monthly incremental fine-tune."))
              )
            )
          ),

          qtPanel("qt7p4",
            div(class="section-heading-dark", "Quant ML MLOps Stack — Chang Ch.6 Infrastructure"),
            tags$table(class="table table-hover",
              tags$thead(tags$tr(tags$th("Layer"), tags$th("Chang Category"), tags$th("Tool"), tags$th("Notes"))),
              tags$tbody(
                ...
                tags$tr(tags$td("Portfolio optimiser"), tags$td("Serving layer"), tags$td("CVXPY / custom solver"), tags$td("Constraints as Chang serving limits")),
    ...
    observeEvent(input$save_self, {
      ...
      output$self_result <- renderUI({
        ...
        div(class = if(pct >= 80) "success-box" else "tip-box",
            ...
            if(pct < 50) tags$p("Focus: Chang Ch.4 (point-in-time data — the #1 quant interview test) and Ch.5 (purged walk-forward CV — standard k-fold is invalid for financial time series)."),
            if(pct >= 50 && pct < 80) tags$p("Good foundation. Deepen on: backtest-to-live gap sources, transaction cost modelling, and factor crowding as Chang concept drift."),
            if(pct >= 80) tags$p("\u2705 Strong quant ML command. You can connect Chang framework chapters to production equity alpha signal systems end-to-end.")
        )
      })
