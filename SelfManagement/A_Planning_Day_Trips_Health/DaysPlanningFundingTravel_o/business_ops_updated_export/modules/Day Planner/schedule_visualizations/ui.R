# modules/Day Planner/schedule_visualizations/ui.R

schedule_visualizations_ui <- function(id) {
  ns <- NS(id)
  ...
  tagList(
    fluidRow(
      box(title = "Select a Day to Visualize", status = "primary", solidHeader = TRUE, width = 12,
          ...
          fluidRow(
            ...
            column(3, selectInput(ns("filter_row_type"), "Filter by Row Type (Optional):", choices = c("All Rows" = "all")))
          ),
    ...
    fluidRow(
      box(title = "Itinerary Timeline", status = "success", solidHeader = TRUE, width = 12, collapsible = TRUE,
          htmlOutput(ns("timeline_html")))
    ),

    fluidRow(
      box(title = "Time Allocation Trends", status = "warning", solidHeader = TRUE, width = 12, collapsible = TRUE,
