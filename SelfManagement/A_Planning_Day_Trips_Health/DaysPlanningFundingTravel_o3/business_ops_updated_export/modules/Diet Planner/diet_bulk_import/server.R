# modules/Diet Planner/diet_bulk_import/server.R

diet_bulk_import_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {

    parsed_data <- reactiveVal(NULL)

    observeEvent(api_manager$pending_bulk_text_diet(), {
      incoming_text <- api_manager$pending_bulk_text_diet()
      if (nchar(incoming_text) > 0) updateTextAreaInput(session, "diet_text", value = incoming_text)
    }, ignoreInit = TRUE)

    observeEvent(input$parse, {
      if (trimws(input$diet_text) == "") {
        output$status <- renderUI({ tags$div(class = "status-error", tags$i(class = "fa fa-exclamation-triangle"), " Please paste a diet log to parse") })
        return()
      }
      output$status <- renderUI({ tags$div(class = "status-info", tags$i(class = "fa fa-spinner fa-spin"), " Parsing...") })

      tryCatch({
        parsed_df <- parse_diet_text(input$diet_text)
        parsed_data(parsed_df)

        output$parse_info <- renderUI({
          tags$div(class = "status-success", tags$i(class = "fa fa-check-circle"),
                   sprintf(" Successfully parsed %d rows for %s (%s)", nrow(parsed_df), parsed_df$log_date[1], parsed_df$diet_type[1]))
        })
    ...
    observeEvent(input$upload, {
      ...
      if (is.null(parsed_data())) { showNotification("Please parse the diet log first!", type = "error"); return() }

      output$status <- renderUI({ tags$div(class = "status-info", tags$i(class = "fa fa-spinner fa-spin"), " Uploading...") })

      tryCatch({
        rows_uploaded <- api_manager$bq_insert_diet(parsed_data())
    ...
    observeEvent(input$clear, {
      updateTextAreaInput(session, "diet_text", value = "")
      parsed_data(NULL)
      output$parse_info <- renderUI({})
      output$preview_table <- DT::renderDataTable({})
