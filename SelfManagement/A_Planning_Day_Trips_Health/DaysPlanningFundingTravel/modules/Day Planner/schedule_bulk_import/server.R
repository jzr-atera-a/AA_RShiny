# modules/Day Planner/schedule_bulk_import/server.R

schedule_bulk_import_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {

    parsed_data <- reactiveVal(NULL)

    observeEvent(api_manager$pending_bulk_text_schedule(), {
      incoming_text <- api_manager$pending_bulk_text_schedule()
      if (nchar(incoming_text) > 0) updateTextAreaInput(session, "schedule_text", value = incoming_text)
    }, ignoreInit = TRUE)

    observeEvent(input$parse, {
      if (trimws(input$schedule_text) == "") {
        output$status <- renderUI({ tags$div(class = "status-error", tags$i(class = "fa fa-exclamation-triangle"), " Please paste a schedule to parse") })
        return()
      }
      ...
      tryCatch({
        ...
        output$parse_info <- renderUI({
          tags$div(class = "status-success", tags$i(class = "fa fa-check-circle"),
                   sprintf(" Successfully parsed %d rows for %s (%s)", nrow(parsed_df), parsed_df$schedule_date[1], parsed_df$day_type[1]))
        })
    ...
    observeEvent(input$upload, {
      ...
      if (is.null(parsed_data())) { showNotification("Please parse the schedule first!", type = "error"); return() }

      output$status <- renderUI({ tags$div(class = "status-info", tags$i(class = "fa fa-spinner fa-spin"), " Uploading...") })

      tryCatch({
        rows_uploaded <- api_manager$bq_insert_schedule(parsed_data())
