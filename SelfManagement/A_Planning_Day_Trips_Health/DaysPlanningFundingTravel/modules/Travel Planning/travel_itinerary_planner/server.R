  ...
  moduleServer(id, function(input, output, session) {
      ...
      content = function(file) {
        tryCatch({
          ...
          kml <- c(
            ...
            "    <Style id=\"redPin\"><IconStyle><Icon><href>http://maps.google.com/mapfiles/ms/icons/red-dot.png</href></Icon></IconStyle></Style>"
          )
          ...
          for (day_idx in seq_along(rv$itinerary_data)) {
            ...
            for (idx in seq_along(day$attractions)) {
              ...
              kml <- c(kml,
                ...
                paste0("      <description>", attr$description, "</description>"),
                "      <Point>",
          ...
          writeLines(kml, file)
        }, error = function(e) { writeLines("<?xml version=\"1.0\"?><kml></kml>", file) })
      }
    )
    ...
    output$download_gpx <- downloadHandler(
      ...
      content = function(file) {
        tryCatch({
          if (is.null(rv$itinerary_data) || length(rv$itinerary_data) == 0) {
            writeLines("<?xml version=\"1.0\"?><gpx></gpx>", file)
            return()
          }
          
          gpx <- c("<?xml version=\"1.0\" encoding=\"UTF-8\"?>",
          ...
          for (day_idx in seq_along(rv$itinerary_data)) {
            ...
            for (idx in seq_along(day$attractions)) {
              attr <- day$attractions[[idx]]
              gpx <- c(gpx,
                paste0("  <wpt lat=\"", attr$latitude, "\" lon=\"", attr$longitude, "\">"),
                paste0("    <name>", idx, ". ", attr$name, "</name>"),
                paste0("    <desc>", attr$description, "</desc>"),
                "  </wpt>"
              )
            }
          }
          gpx <- c(gpx, "</gpx>")
          writeLines(gpx, file)
        }, error = function(e) { writeLines("<?xml version=\"1.0\"?><gpx></gpx>", file) })
      }
    )
      ...
      content = function(file) {
        tryCatch({
          ...
          for (day in rv$itinerary_data) {
            if (!is.null(day$attractions)) {
              for (attr in day$attractions) {
                coords <- c(coords, paste0(attr$latitude, ",", attr$longitude))
              }
            }
          }
          ...
          origin <- coords[1]
          dest <- coords[length(coords)]
          waypts <- if(length(coords) > 2) paste(coords[2:(length(coords)-1)], collapse = "|") else ""
          
          url <- paste0("https://www.google.com/maps/dir/?api=1&origin=", origin, "&destination=", dest)
          if (waypts != "") url <- paste0(url, "&waypoints=", waypts)
          url <- paste0(url, "&travelmode=driving")
          
          writeLines(c(
            paste0("Google Maps Navigation: ", input$destination),
        ...
        }, error = function(e) { writeLines("Error", file) })
      }
    )
    # ==================
