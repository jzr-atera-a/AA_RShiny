# modules/Exercise Tracker/exercise_browse/server.R

exercise_browse_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$refresh, {
      ...
      tryCatch({
        query <- sprintf("SELECT * FROM `%s` ORDER BY workout_date DESC, row_sequence ASC LIMIT %d",
                        api_manager$bq_full_table_exercise, input$max_rows)
    ...
    output$download <- downloadHandler(
      filename = function() paste0("exercise_log_", format(Sys.Date(), "%Y%m%d"), ".csv"),
    ...
    output$download_calendar <- downloadHandler(
      filename = function() paste0("workout_schedule_", format(Sys.Date(), "%Y%m%d"), ".ics"),
      content = function(file) {
        tryCatch({
          data <- browse_data()
          if (is.null(data) || nrow(data) == 0) {
            showNotification("No workout entries to export", type = "warning")
            return()
          }

          # Rename columns for calendar export
          cal_data <- data
          if ("workout_date" %in% names(cal_data)) names(cal_data)[names(cal_data) == "workout_date"] <- "date"
          ...
          if ("exercise_type" %in% names(cal_data)) names(cal_data)[names(cal_data) == "exercise_type"] <- "name"
          ...
          ics_lines <- generate_calendar_ics(
            ...
            duration_minutes = 60,
            description = "Workout session - Exported from Business Operations Suite",
            item_col = if ("name" %in% names(cal_data)) "name" else "activity"
          )

          export_calendar_file(ics_lines, file)
          showNotification("✓ Workout schedule exported successfully", type = "message")
