# modules/Gantt to Tickets/gantt_email_contacts/ui.R

gantt_email_contacts_ui <- function(id) {
  ns <- NS(id)
  ...
  tagList(
    fluidRow(
      box(title = "Filter Contacts", status = "primary", solidHeader = TRUE, width = 12,
          fluidRow(
            column(3, selectInput(ns("filter_country"), "Filter by Country:", choices = c("All" = ""))),
            column(3, selectInput(ns("filter_city"), "Filter by City:", choices = c("All" = ""))),
            column(3, selectInput(ns("filter_org"), "Filter by Organization:", choices = c("All" = ""))),
    ...
    fluidRow(
      box(title = "Compose Email", status = "success", solidHeader = TRUE, width = 12,
          ...
          hr(),
          checkboxInput(ns("include_contact_name"), "Personalize with name (use {NAME} in body)", value = TRUE),
          hr(),
