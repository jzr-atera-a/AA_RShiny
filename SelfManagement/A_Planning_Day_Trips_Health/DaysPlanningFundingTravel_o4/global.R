...
# BUSINESS OPERATIONS SUITE: Day Planner + Events Scheduling + Funding Programmes
# =================================================================================
# One app, one shared BigQuery connection and one shared Claude API connection
# (grouped under "API Configuration" in the sidebar), with each of the three
# original standalone apps' tabs grouped under their own named section
# ("Day Planner", "Events Scheduling", "Funding Programmes"). See
# R/utils_api.R for how one shared APIManager class serves three separate
# BigQuery tables, and R/module_loader.R for the recursive module-discovery
# that lets modules live inside these grouped subfolders
# (modules/"Day Planner"/generate_schedule/manifest.yml etc) instead of a
# flat modules/<id>/ layout.
#
# NOTE: this is a SEPARATE unified app from the Book Summary / Flex Table /
# Mind Map / Knowledge Graph unified suite - same architectural pattern,
# different deployment, intentionally not merged together.

cat("\n╔══════════════════════════════════════════════╗\n")
cat("║  BUSINESS OPERATIONS SUITE - INIT             ║\n")
cat("║  Day Planner + Events + Funding Programmes    ║\n")
cat("╚══════════════════════════════════════════════╝\n\n")
...
# library()/pkg::fn() calls, not packages named inside YAML manifests, so
# every package any module needs (across all three suites) is declared here
# explicitly. `curl` is required for the streaming Claude API implementation
# (curl::curl_fetch_stream) shared by every suite's Generate/Scan tab.
suppressPackageStartupMessages({
  library(shinyjs)
  library(httr)
  library(curl)
...
create_ui <- function(module_loader) {
  ...
  # Groups are defined as a top-level `groups:` list in the registry (each
  # with an explicit `modules: [id, id, ...]` member list). For each group,
  # filter its declared member ids against the currently-enabled modules,
  # build one menuSubItem() per surviving member, then build the group's
  ...
  dashboardPage(
    dashboardHeader(title = module_loader$registry$app$name %||% "Business Operations Suite"),
