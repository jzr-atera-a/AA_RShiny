...
ModuleLoader <- R6::R6Class(
  ...
  public = list(
    ...
    discover_modules = function() {
      # Recursive search: modules can now live inside GROUP subfolders
      # (e.g. modules/"Flex Table"/generate_table/manifest.yml), not
      # just directly under modules/<module_id>/manifest.yml. Finding
      # every manifest.yml anywhere under modules/, regardless of
      # nesting depth, works for both the flat and grouped layouts.
      manifest_paths <- list.files("modules", pattern = "^manifest\\.yml$",
                                    recursive = TRUE, full.names = TRUE)

      for (manifest_path in manifest_paths) {
        dir <- dirname(manifest_path)
        if (grepl("^_", basename(dir))) next
