...
browse_events_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    load_data <- function() {
      ...
      tryCatch({
        query <- sprintf(
          ...
          api_manager$bq_full_table_events, input$max_rows
        )
    ...
    observeEvent(api_manager$state_trigger_events(), {
    ...
    output$download_calendar <- downloadHandler(
      filename = function() paste0("events_", format(Sys.Date(), "%Y%m%d"), ".ics"),
      content = function(file) {
        tryCatch({
          data <- if (!is.null(filter_data())) filter_data() else all_data()
          if (is.null(data) || nrow(data) == 0) {
            showNotification("No events to export", type = "warning")
            return()
          }

          # Rename columns for calendar export
          cal_data <- data
          if ("event_date" %in% names(cal_data)) names(cal_data)[names(cal_data) == "event_date"] <- "date"
          if ("event_time" %in% names(cal_data)) names(cal_data)[names(cal_data) == "event_time"] <- "time"
          if ("event_name" %in% names(cal_data)) names(cal_data)[names(cal_data) == "event_name"] <- "name"
          if ("venue_name" %in% names(cal_data)) names(cal_data)[names(cal_data) == "venue_name"] <- "location"
          ...
          ics_lines <- generate_calendar_ics(
            ...
            description = "Event - Exported from Business Operations Suite",
            location = if ("location" %in% names(cal_data)) cal_data$location[1] else "",
            item_col = if ("name" %in% names(cal_data)) "name" else "event_name"
          )

          export_calendar_file(ics_lines, file)
          showNotification("✓ Events calendar exported successfully", type = "message")
