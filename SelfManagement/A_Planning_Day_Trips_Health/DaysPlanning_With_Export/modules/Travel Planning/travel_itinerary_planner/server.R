...
# FIXED: PDF generation + HTML with map + Geo file exports
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$trip_map <- plotly::renderPlotly({
      ...
      tryCatch({
        ...
        # Store for geo exports
        rv$all_attractions_df <- all_attractions
        ...
        tryCatch({
          ...
          if (!is.null(rv$itinerary_data) && length(rv$itinerary_data) > 0) {
            ...
            for (day_idx in seq_along(rv$itinerary_data)) {
              ...
              for (attr in day$attractions) {
                day_color <- day_colors[((day$day - 1) %% length(day_colors)) + 1]
                html_content <- c(html_content,
                  paste0("        { x: ", attr$longitude, ", y: ", attr$latitude, ", 
                          name: '", attr$name, "', marker: { color: '", day_color, "', size: 12 } },")
                )
              }
            }
          }
          ...
          html_content <- c(html_content,
            "      ];",
            "      var layout = {",
            "        mapbox: { style: 'open-street-map', center: { lon: 0, lat: 0 }, zoom: 10 },",
            "        showlegend: false,",
            "        margin: { l: 0, r: 0, t: 0, b: 0 }",
            "      };",
            "      Plotly.newPlot('map', [{x: mapData.map(d => d.x), y: mapData.map(d => d.y), mode: 'markers', type: 'scattermapbox', marker: {size: 12}}], layout, {responsive: true});",
            "    </script>"
          )
      ...
      content = function(file) {
        tryCatch({
          ...
          result <- tryCatch({
            ...
            cat("rmarkdown error:", e$message, "\n")
            "failed"
          })
          
          if (result == "success") {
            cat("✅ PDF generated via rmarkdown\n")
          } else {
            # Fallback: save as text
            cat("⚠️ PDF generation failed, saving as text\n")
        ...
        tryCatch({
          ...
          for (day_idx in seq_along(rv$itinerary_data)) {
            ...
            for (idx in seq_along(day$attractions)) {
              ...
              kml <- c(kml,
                "    <Placemark>",
                paste0("      <name>", idx, ". ", attr$name, " (", day$day_label, ")</name>"),
                paste0("      <description>", attr$description, " | Time: ", attr$time_start, "-", attr$time_end, "</description>"),
                paste0("      <styleUrl>#", if(idx %% 2 == 0) "bluePin" else "redPin", "</styleUrl>"),
                "      <Point>",
                paste0("        <coordinates>", attr$longitude, ",", attr$latitude, ",0</coordinates>"),
                "      </Point>",
                "    </Placemark>"
              )
            }
        ...
        tryCatch({
          ...
          for (day_idx in seq_along(rv$itinerary_data)) {
            ...
            for (idx in seq_along(day$attractions)) {
              attr <- day$attractions[[idx]]
              gpx <- c(gpx,
                "  <wpt lat=\"" , attr$latitude, "\" lon=\"", attr$longitude, "\">",
                paste0("    <name>", idx, ". ", attr$name, "</name>"),
                paste0("    <desc>", attr$description, " (", attr$time_start, "-", attr$time_end, ")</desc>"),
                paste0("    <type>", day$day_label, "</type>"),
                "  </wpt>"
              )
            }
          }
    ...
    # ==================
    # GEO EXPORTS - GOOGLE MAPS LINK
    # ==================
    
    output$download_gmaps_link <- downloadHandler(
      filename = function() { paste0(input$destination, "_gmaps_link.txt") },
      content = function(file) {
        tryCatch({
          if (is.null(rv$itinerary_data) || length(rv$itinerary_data) == 0) {
            writeLines("No coordinates available", file)
            return()
          }
          ...
          for (day in rv$itinerary_data) {
            if (!is.null(day$attractions)) {
              for (attr in day$attractions) {
                waypoints <- c(waypoints, paste0(attr$latitude, ",", attr$longitude))
              }
            }
          }
          ...
          # First point as origin, last as destination, middle as waypoints
          origin <- waypoints[1]
          destination <- waypoints[length(waypoints)]
          
          if (length(waypoints) > 2) {
            waypoints_str <- paste(waypoints[2:(length(waypoints)-1)], collapse = "|")
            gmaps_url <- paste0(
          ...
          link_content <- c(
            ...
            gmaps_url,
            "",
            "Note: This link includes all attractions as waypoints."
          )
          
          writeLines(link_content, file)
