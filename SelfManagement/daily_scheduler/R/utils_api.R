  ...
  public = list(
    ...
    authenticate_bigquery = function(json_path = NULL, json_text = NULL) {
      tryCatch({
        ...
        # Create table if not exists - day_scheduler schema
    ...
    # Get distinct day_type/country/city combinations, used to build the
    # cascading Type of Day and Country -> City dropdowns in generate_schedule,
    # add_single, and visualizations.
    bq_get_taxonomy = function() {
      empty_result <- data.frame(day_type = character(), country = character(),
      ...
      query <- sprintf(
        "SELECT DISTINCT day_type, country, city, schedule_date FROM `%s` ORDER BY day_type, country, schedule_date",
        self$bq_full_table_id
      )
