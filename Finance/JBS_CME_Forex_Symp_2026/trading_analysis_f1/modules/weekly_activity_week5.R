...
w5_node_bounce <- function(kind, seed) {
  ...
  approach <- syn_path(10, start = tail(sess$Close,1), drift = approach_dir * 0.4, vol = 0.6, seed = seed + 40)
...
w5_mp_respect <- function(kind, seed) {
  ...
  outc <- syn_outcome_tail(level, react_dir, diff(range(sess$Close)) * 0.6, TRUE, seed + 200, n = 14)
...
# ══════════════════════════════════════════════════════════════════════════
# CONFLUENCE — aggressive bounce off 3+ levels stacked together
# ══════════════════════════════════════════════════════════════════════════

w5_confluence_bounce <- function(seed) {
  set.seed(seed)
  lead <- syn_path(10, start = 100, drift = 0.05, vol = 1, seed = seed)
  sess <- syn_path(24, start = tail(lead$Close,1), drift = 0.02, vol = 1.2, seed = seed + 10)
  sess$Volume <- round(runif(24, 500, 900))
  ib_lo <- min(sess$Low[1:4])
  ...
  nearest_hvn <- vp$hvn[which.min(abs(vp$hvn - vp$val))]
  level <- mean(c(vp$val, ib_lo, nearest_hvn))
  approach <- syn_path(10, start = tail(sess$Close,1), drift = -0.5, vol = 0.7, seed = seed + 40)
  outc <- syn_outcome_tail(level, 1, diff(range(sess$Close)) * 0.9, TRUE, seed + 200, n = 16)

  df <- syn_concat(lead, sess, approach, outc$df)
  outc_dates <- syn_seg(df, 4)
  vp_shapes <- syn_vp_shapes(df, vp)
  shapes <- c(vp_shapes, list(
    syn_hline(df$Date[1], tail(df$Date,1), vp$val, "#9b59b6", "dash", 1.5),
    syn_hline(df$Date[1], tail(df$Date,1), ib_lo, "#7f8c8d", "dash", 1.5),
    syn_hline(df$Date[1], tail(df$Date,1), nearest_hvn, "#3498db", "dash", 1.5),
    syn_hline(df$Date[1], tail(df$Date,1), level, "#f39c12", "solid", 2)
  ))
  ann <- list(syn_tag(outc_dates[1], level, "Confluence of VAL + Prior IB Low + HVN \u2192 Aggressive Bounce", "#27ae60", 8))
  syn_chart(df, "Aggressive Bounce off Multi-Level Confluence (VAL + IB Low + HVN)", shapes, ann)
}
