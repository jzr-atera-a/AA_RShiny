...
w2_fib_retracement <- function(direction, touch_level, seed) {
  ...
  # corrected post-concat date — this is both the Fib-touch point AND where the
  # trendline bounce below is anchored, so the two coincide exactly as the sheet asks.
  ...
  # Downtrend/uptrend line: anchored on an early high (downtrend, sloping down as
  # resistance) or low (uptrend, sloping up as support) within leg1, extended through
  # the retracement's touch point and a little further into the continuation leg so it
  # visibly acts as the line price bounces off.
  anchor_idx <- max(2, round(nrow(leg1) * 0.12))
  anchor_date <- leg1$Date[anchor_idx]
  anchor_price <- if (down) leg1$High[anchor_idx] + rng * 0.03 else leg1$Low[anchor_idx] - rng * 0.03
  x_anchor <- as.numeric(anchor_date); x_touch <- as.numeric(touch_date)
  slope <- (touch_price - anchor_price) / (x_touch - x_anchor)
  extend_date <- cont_dates[min(6, length(cont_dates))]
  x_extend <- as.numeric(extend_date)
  y_extend <- touch_price + slope * (x_extend - x_touch)
  trend_color <- if (down) "#e74c3c" else "#27ae60"
  shapes[[length(shapes) + 1]] <- syn_line(anchor_date, extend_date, anchor_price, y_extend, trend_color, "solid", 2)
  
  ann <- list(syn_tag(touch_date, touch_price,
                       paste0(touch_level * 100, "% Fib Touch, Bounce off ", ifelse(down, "Downtrend", "Uptrend"),
                              " Line\n\u2192 ", ifelse(down, "Downtrend", "Uptrend"), " Resumes"),
                       trend_color, 9))
