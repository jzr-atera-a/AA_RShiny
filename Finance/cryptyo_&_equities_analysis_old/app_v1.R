  # Multi-Asset Analysis Dashboard - Cryptocurrencies & Private Equity
  ...
  server <- function(input, output, session) {
    ...
    output$classComparison <- renderText({
      ...
      if (nrow(crypto_data) == 0 || nrow(equity_data) == 0) {
        return("Insufficient data for comparison")
      }
      
      crypto_ret <- mean(crypto_data$returns) * 252 * 100
      equity_ret <- mean(equity_data$returns) * 252 * 100
      
      crypto_vol <- sd(crypto_data$returns) * sqrt(252) * 100
      equity_vol <- sd(equity_data$returns) * sqrt(252) * 100
      
      paste(
        "Class Comparison:",
        paste("Return Diff:", round(crypto_ret - equity_ret, 2), "%"),
        paste("Vol Diff:", round(crypto_vol - equity_vol, 2), "%"),
        "",
        ifelse(crypto_ret > equity_ret, "Crypto outperforming", "Equity outperforming"),
        ifelse(crypto_vol > equity_vol, "Crypto more volatile", "Equity more volatile"),
        sep = "\n"
      )
    })
  }
