    ...
    tabItems(
      ...
      tabItem(tabName = "hydrocarbons",
              ...
              fluidRow(
                ...
                box(
                  title = "Oil Price ARIMA Forecast", status = "primary", solidHeader = TRUE, width = 6,
      ...
      tabItem(tabName = "battery",
              ...
              fluidRow(
                ...
                box(
                  title = "Lithium Price Exponential Smoothing Forecast", status = "primary", solidHeader = TRUE, width = 6,
...
server <- function(input, output, session) {
  ...
  output$ev_heatmap_plot <- renderPlotly({
    ...
    plot_ly(z = demand_matrix, x = hours, y = uk_regions, type = "heatmap",
            colorscale = "Viridis", hovertemplate = "Hour: %{x}<br>Region: %{y}<br>Demand: %{z:.1f} MW<extra></extra>") %>%
      layout(title = "UK Regional EV Charging Demand - 24 Hour Profile",
             xaxis = list(title = "Hour of Day"),
             yaxis = list(title = "UK Region"))
  })
  
  # Time Series Forecast for Electricity Prices
  output$electricity_forecast <- renderPlotly({
    ...
    seasonal <- 10 * sin(2 * pi * (1:n) / 12) # Annual seasonality
    noise <- rnorm(n, 0, 5)
    prices <- trend + seasonal + noise
    
    # Create time series
    ts_data <- ts(prices, start = c(2020, 1), frequency = 12)
    ...
    plot_ly() %>%
      add_trace(x = historical_dates, y = as.numeric(ts_data), 
                type = "scatter", mode = "lines", name = "Historical Prices",
                line = list(color = "#3498db", width = 2)) %>%
      add_trace(x = forecast_dates, y = as.numeric(forecast_result$mean), 
                type = "scatter", mode = "lines", name = "Forecast",
                line = list(color = "#e74c3c", width = 2, dash = "dash")) %>%
      add_ribbons(x = forecast_dates, 
                  ymin = as.numeric(forecast_result$lower[,2]), 
                  ymax = as.numeric(forecast_result$upper[,2]),
                  name = "95% CI", fillcolor = "rgba(231, 76, 60, 0.2)",
                  line = list(color = "transparent")) %>%
      layout(title = "UK Electricity Price Forecast (ETS Model)",
             xaxis = list(title = "Date"),
             yaxis = list(title = "Price (£/MWh)"))
  })
  
  # Oil Price ARIMA Forecast
  output$oil_price_forecast <- renderPlotly({
    ...
    # Create time series
    ts_data <- ts(prices, frequency = 52)
    
    # Fit ARIMA model
    fit <- auto.arima(ts_data)
    forecast_result <- forecast(fit, h = 26) # 6 months weekly forecast
    ...
    plot_ly() %>%
      ...
      add_ribbons(x = forecast_dates, 
                  ymin = as.numeric(forecast_result$lower[,2]), 
                  ymax = as.numeric(forecast_result$upper[,2]),
                  name = "95% CI", fillcolor = "rgba(243, 156, 18, 0.2)",
                  line = list(color = "transparent")) %>%
      layout(title = "Brent Crude Oil Price Forecast (ARIMA Model)",
             xaxis = list(title = "Date"),
             yaxis = list(title = "Price ($/barrel)"))
  })
  
  # Lithium Price Forecast
  output$lithium_forecast <- renderPlotly({
    # Generate sample lithium price data
    dates <- seq(as.Date("2020-01-01"), as.Date("2024-06-01"), by = "month")
    n <- length(dates)
    
    # Create realistic lithium price time series with high volatility
    base_price <- 15000
    growth <- seq(0, 2.5, length.out = n)
    bubble <- ifelse(dates > as.Date("2021-01-01") & dates < as.Date("2023-01-01"), 
                     sin(pi * (as.numeric(dates - as.Date("2021-01-01")) / 730)) * 40000, 0)
    noise <- rnorm(n, 0, 2000)
    prices <- base_price * exp(growth) + bubble + noise
    
    # Ensure positive prices
    prices[prices < 5000] <- 5000 + abs(rnorm(sum(prices < 5000), 0, 1000))
    ...
    plot_ly() %>%
      ...
      add_trace(x = forecast_dates, y = as.numeric(forecast_result$mean), 
                type = "scatter", mode = "lines", name = "ETS Forecast",
                line = list(color = "#e67e22", width = 2, dash = "dash")) %>%
      add_ribbons(x = forecast_dates, 
                  ymin = as.numeric(forecast_result$lower[,2]), 
                  ymax = as.numeric(forecast_result$upper[,2]),
                  name = "95% CI", fillcolor = "rgba(230, 126, 34, 0.2)",
                  line = list(color = "transparent")) %>%
      layout(title = "Lithium Carbonate Price Forecast (ETS Model)",
             xaxis = list(title = "Date"),
             yaxis = list(title = "Price ($/tonne)"))
  })
