# modules/Strategic Analysis/diagram_recommender/server.R

diagram_recommender_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$recommend, {
      ...
      prompt <- generate_diagram_recommendation_prompt(input$problem_description, n)

      # ---- DEBUG ----
      cat("========================================================\n")
      cat("🧠 [Strategy Recommender][DEBUG] Requesting framework recommendations from Claude\n")
      cat(sprintf("    num_recommendations : %d\n", n))
      cat(sprintf("    problem_description  : %s\n", input$problem_description))
      cat(sprintf("    catalog_size          : %d frameworks offered\n", length(DIAGRAM_TYPES)))
      ...
      tryCatch({
        ...
        df <- parse_diagram_recommendations(response_text, n)
        recommendations(df)

        cat(sprintf("🧠 [Strategy Recommender][DEBUG] Got %d valid recommendation(s) in %ss: %s\n",
        ...
        shinyjs::hide("loading_spinner")
        cat(sprintf("❌ [Strategy Recommender][DEBUG] Failed: %s\n", e$message))
        output$status <- renderUI({
    ...
    #     switch to the Generate Diagram tab, where an observer on
    #     api_manager$pending_diagram_selection() auto-selects the
    #     matching Group + Framework dropdowns. ----
    observeEvent(input$tool_clicked, {
      dtype <- input$tool_clicked
      if (is.null(dtype) || !dtype %in% DIAGRAM_TYPES) return()

      cat(sprintf("🖱️  [Strategy Recommender][DEBUG] Recommended tool clicked: %s -> jumping to Generate Diagram\n", dtype))
      api_manager$set_pending_diagram_selection(dtype)
      updateTabItems(session$rootScope(), "sidebar_menu", selected = "generate_diagram")
      showNotification(sprintf("✓ %s selected - jumping to Generate Diagram", DIAGRAM_TYPE_LABELS[[dtype]]), type = "message")
    })
