# modules/visualize_kg_cytoscape/ui.R

visualize_kg_cytoscape_ui <- function(id) {
  ns <- NS(id)
  ...
  tagList(
        ...
        fluidRow(
          ...
          column(6,
                 actionButton(ns("load_graph"), "Load Graph", class = "btn-success btn-lg",
                             icon = icon("share-nodes"), style = "width: 100%;")
          )
        ),
    ...
    fluidRow(
      box(
        title = "Knowledge Graph (Cytoscape)",
        status = "success",
        solidHeader = TRUE,
        width = 8,
        collapsible = TRUE,
        ...
        export_controls_ui(ns, ns("export_target"), "knowledge_graph_cytoscape"),
        uiOutput(ns("network"))
      ),
      box(
