  ...
  tagList(
    fluidRow(
      box(
        ...
        fluidRow(
          column(3,
                 numericInput(ns("expected_rows"), "Expected Rows (approx.):",
                              value = 5, min = 1, max = 50, step = 1),
                 p(style = "color: #7f8c8d; font-size: 12px; margin-top: -10px;",
                   "Used only to budget the response size - your description's wording ",
                   "(e.g. 'top 5') still decides the actual count.")
          ),
          column(3,
                 numericInput(ns("expected_columns"), "Expected Columns (approx.):",
                              value = 5, min = 1, max = 50, step = 1)
          ),
          column(3,
                 radioButtons(ns("include_latex"), "Include LaTeX in cell values?",
                              choices = c("No (plain text only)" = "no", "Yes" = "yes"),
                              selected = "no")
          ),
          column(3,
                 sliderInput(ns("words_per_cell"), "Max Words per Cell:",
