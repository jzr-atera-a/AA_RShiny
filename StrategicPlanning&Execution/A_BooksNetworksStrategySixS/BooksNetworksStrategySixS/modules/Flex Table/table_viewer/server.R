...
table_viewer_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    #
    # Uses observe() rather than observeEvent(input$viz_category, ...):
    # observeEvent isolates everything in its handler body except the
    # event expression itself, so a read of viz_taxonomy() inside it is
    # NOT a real dependency. That means if a new table is uploaded under
    # a Category string that happens to already be selected, the Topic
    # (and downstream Row/Column Label) dropdowns would never refresh to
    # show it. observe() tracks every reactive read in its body, so it
    # correctly re-cascades whenever EITHER the taxonomy changes OR the
    # relevant parent selection changes.
    # ------------------------------------------------------------
    viz_taxonomy <- reactive({
      api_manager$state_trigger_flex()
      if (!api_manager$bq_authenticated) return(api_manager$empty_flex_taxonomy())
      tryCatch(api_manager$bq_get_flex_taxonomy(), error = function(e) api_manager$empty_flex_taxonomy())
    })
    ...
    #
    # This level queries BigQuery directly rather than filtering
    # viz_taxonomy() (row/column dimension labels aren't part of that
    # taxonomy pull) - so it must explicitly depend on
    # api_manager$state_trigger_flex() itself to know when to re-query,
    # in addition to depending on the Category/Topic selections.
    # ------------------------------------------------------------
    observe({
      api_manager$state_trigger_flex()
      cat_val <- input$viz_category
      ...
      if (!api_manager$bq_authenticated || is.null(cat_val) || is.null(top_val) ||
          cat_val == "" || top_val == "") {
      ...
      tryCatch({
        safe_category <- gsub("'", "''", cat_val)
        safe_topic <- gsub("'", "''", top_val)
        query <- sprintf(
          "SELECT DISTINCT row_dimension_label FROM `%s` WHERE category = '%s' AND topic = '%s' ORDER BY row_dimension_label",
          api_manager$bq_full_table_flex, safe_category, safe_topic
        )
        rows <- api_manager$bq_query(query)

        if (nrow(rows) > 0) {
          current <- isolate(input$row_label_select)
          choices <- setNames(rows$row_dimension_label, rows$row_dimension_label)
          selected <- if (!is.null(current) && current %in% rows$row_dimension_label) current else rows$row_dimension_label[1]
          updateSelectInput(session, "row_label_select", choices = choices, selected = selected)
        } else {
    ...
    observe({
      ...
      cat_val <- input$viz_category
      top_val <- input$viz_topic
      row_val <- input$row_label_select

      if (!api_manager$bq_authenticated || is.null(cat_val) || is.null(top_val) ||
          is.null(row_val) || cat_val == "" || top_val == "" || row_val == "") {
      ...
      tryCatch({
        ...
        query <- sprintf(
          ...
          api_manager$bq_full_table_flex, safe_category, safe_topic, safe_row_label
        )
        cols <- api_manager$bq_query(query)

        if (nrow(cols) > 0) {
          current <- isolate(input$column_label_select)
          choices <- setNames(cols$column_dimension_label, cols$column_dimension_label)
          selected <- if (!is.null(current) && current %in% cols$column_dimension_label) current else cols$column_dimension_label[1]
          updateSelectInput(session, "column_label_select", choices = choices, selected = selected)
        } else {
    ...
    observeEvent(input$load_table, {
      ...
      tryCatch({
        ...
        query <- sprintf(
          ...
          api_manager$bq_full_table_flex, safe_category, safe_topic, safe_row_label, safe_col_label
        )
