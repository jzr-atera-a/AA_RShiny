# modules/Six Sigma Analysis/sixsigma_browse/ui.R

sixsigma_browse_ui <- function(id) {
  ns <- NS(id)

  tagList(
    fluidRow(
      box(title = "Saved Six Sigma Diagrams (one row per diagram)", status = "primary", solidHeader = TRUE, width = 12,
          fluidRow(
    ...
    fluidRow(
      box(title = "Raw Component Rows (six_sigma_diagrams table)", status = "info", solidHeader = TRUE, width = 12,
          fluidRow(
