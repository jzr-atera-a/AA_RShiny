...
sixsigma_visualizations_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    #
    # IMPORTANT: every level below uses observe() and reads viz_taxonomy()
    # directly, rather than observeEvent(input$viz_category, ...) reacting
    # only to the PARENT INPUT'S VALUE changing. That distinction matters:
    # if a fresh diagram is uploaded under a Category/Domain string that
    # was already selected (e.g. re-using a Domain string under a
    # different Category than before), the dropdown's SELECTED VALUE never
    # actually changes even though the underlying data behind it did - so
    # observeEvent(input$viz_domain, ...) would never re-fire, leaving
    # Topic/Diagram silently stuck showing OLD data that happened to share
    # that Domain string. observe() has no such gap: it re-runs whenever
    # ANY reactive value read inside it changes, including viz_taxonomy()
    # itself, so a fresh BigQuery pull always recascades all the way down
    # regardless of whether any dropdown's selected string happens to
    # coincide with its previous value.
    # ------------------------------------------------------------
    ...
    observe({
      ...
      topics <- sort(unique(tax$topic[tax$category == cat_val & tax$domain == dom_val & nchar(trimws(tax$topic)) > 0]))
      if (length(topics) == 0) {
    ...
    observe({
      ...
      if (is.null(cat_val) || is.null(dom_val) || is.null(top_val) ||
          cat_val == "" || dom_val == "" || top_val == "") {
        updateSelectInput(session, "viz_diagram_id", choices = c("(select a topic first)" = ""))
        return()
      }

      diagrams <- tax[tax$category == cat_val & tax$domain == dom_val & tax$topic == top_val, , drop = FALSE]
      if (nrow(diagrams) == 0) {
        ...
        current <- isolate(input$viz_diagram_id)
        choices <- setNames(diagrams$diagram_id, labels)
        selected <- if (!is.null(current) && current %in% diagrams$diagram_id) current else diagrams$diagram_id[1]
        updateSelectInput(session, "viz_diagram_id", choices = choices, selected = selected)
      }
    })
