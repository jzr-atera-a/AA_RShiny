  ...
  public = list(
    ...
    initialize = function() {
      ...
      self$bq_full_table_id  <- paste0(self$bq_project_id, ".",
                                        self$bq_dataset_id, ".",
                                        self$bq_table_id)
      cat("🔌 [APIManager] Initialized\n")
      cat("   Project :", self$bq_project_id,  "\n")
      cat("   Dataset :", self$bq_dataset_id,  "\n")
      cat("   Table   :", self$bq_table_id,    "\n")
      cat("   Full ref:", self$bq_full_table_id, "\n\n")
    },

    trigger_state_update = function() {
      current <- self$state_trigger()
      self$state_trigger(current + 1)
      cat("🔔 [APIManager] state_trigger fired →", current + 1, "\n")
    },

    set_pending_bulk_text = function(text) {
      self$pending_bulk_text(text)
      cat("📋 [APIManager] pending_bulk_text set (", nchar(text), "chars)\n")
    },
    ...
    set_claude_credentials = function(api_key, model = NULL, max_tokens = NULL, timeout = NULL) {
      ...
      cat("🤖 [Claude] Credentials set\n")
      cat("   Model     :", self$claude_model,      "\n")
      cat("   Max tokens:", self$claude_max_tokens, "\n")
      cat("   Timeout   :", self$claude_timeout, "s\n")
    },

    test_claude_connection = function() {
      cat("🤖 [Claude] Testing connection...\n")
      ...
      tryCatch({
        ...
        cat("🤖 [Claude] Test response HTTP", status_code(response), "\n")
        if (status_code(response) == 200) {
          self$claude_authenticated <- TRUE
          self$trigger_state_update()
          cat("✅ [Claude] Connection OK\n")
          return(TRUE)
        }
        ...
        cat("❌ [Claude] Test failed:", e$message, "\n")
    ...
    call_claude = function(prompt, max_tokens = NULL, progress_callback = NULL) {
      ...
      cat("   Timeout    :", self$claude_timeout, "s\n")
      cat("   Prompt size:", nchar(prompt), "chars\n")
      cat("   Prompt preview:\n")
      cat(substr(prompt, 1, 200), "...\n")
      cat("─────────────────────────────────────────────────────\n")

      if (!is.null(progress_callback)) progress_callback("Sending to Claude API...")
      ...
      tryCatch({
        ...
        cat("🤖 [Claude] Response received — HTTP", status_code(response),
            "in", elapsed, "s\n")
        ...
        cat("✅ [Claude] OK — stop_reason:", stop_reason,
            "| output:", nchar(text), "chars",
            if (truncated) "⚠️  TRUNCATED" else "", "\n")
        cat("   Usage — input:", result$usage$input_tokens,
            "| output:", result$usage$output_tokens, "tokens\n\n")
        ...
        elapsed <- round(proc.time()[["elapsed"]] - t_start, 1)
        msg <- e$message
        cat("❌ [Claude] Request failed after", elapsed, "s:", msg, "\n\n")
        if (grepl("timeout|Timeout|Operation was aborted", msg, ignore.case = TRUE)) {
          msg <- paste0("Request timed out after ", self$claude_timeout,
                        "s. Try reducing Top-N or increasing timeout in Claude API Config.")
    ...
    authenticate_bigquery = function(json_path = NULL, json_text = NULL) {
      ...
      tryCatch({
        ...
        if (!is.null(json_path)) {
          ...
          cat("   Auth via : JSON file —", json_path, "\n")
          cat("   SA email :", json_content$client_email, "\n")
        ...
        cat("   Verifying: bq_project_datasets(", self$bq_project_id, ")...\n")
        datasets <- bq_project_datasets(self$bq_project_id)
        cat("✅ [BigQuery] Auth OK —", length(datasets), "datasets visible in project\n")

        # Check if target table exists — do NOT auto-create (needs bigquery.tables.create)
        cat("   Checking table exists:", self$bq_full_table_id, "...\n")
        table_exists <- tryCatch({
          tbl <- bq_table(self$bq_project_id, self$bq_dataset_id, self$bq_table_id)
          bq_table_exists(tbl)
        }, error = function(e) {
          cat("   ⚠️  Table check error:", e$message, "\n")
          FALSE
        })

        if (table_exists) {
          cat("✅ [BigQuery] Table found:", self$bq_full_table_id, "\n")
        } else {
          cat("⚠️  [BigQuery] Table NOT found:", self$bq_full_table_id, "\n")
          cat("   → Run the provided SQL script in BigQuery console to create it first.\n")
          cat("   → Then reconnect here.\n")
          # Still mark as authenticated so the user can see the error in the UI
        }
        cat("─────────────────────────────────────────────────────\n\n")
    ...
    bq_query = function(query) {
      ...
      elapsed <- round(proc.time()[["elapsed"]] - t_start, 1)
      cat("✅ [BigQuery] Query returned", nrow(data), "rows in", elapsed, "s\n")
      data
    },
    ...
    bq_insert = function(data_frame) {
      ...
      cat("\n📤 [BigQuery] ── Inserting rows ──────────────────────\n")
      cat("   Target   :", self$bq_full_table_id, "\n")
      cat("   Rows in  :", nrow(data_frame), "\n")
      cat("   Token user at insert:", tryCatch(bigrquery::bq_user(), error = function(e) paste("ERROR:", e$message)), "\n")
      ...
      if (length(missing_cols) > 0) {
        cat("   Filling missing cols:", paste(missing_cols, collapse = ", "), "\n")
        for (col in missing_cols) data_frame[[col]] <- ""
      }

      # ONE connection — get MAX(id) then write, exactly as Contact Manager does
      ...
      start_id <- tryCatch({
        ...
        cat("   ⚠️  MAX(id) failed, starting from 1:", e$message, "\n")
        1L
      })
      ...
      cat("   Uploading via dbWriteTable (append=TRUE)...\n")
      t_start <- proc.time()[["elapsed"]]
      ...
      elapsed <- round(proc.time()[["elapsed"]] - t_start, 1)
      cat("✅ [BigQuery] Inserted", nrow(data_frame), "rows in", elapsed, "s\n")
      cat("   IDs:", start_id, "to", start_id + nrow(data_frame) - 1L, "\n")
      cat("─────────────────────────────────────────────────────\n\n")
