...
visualizations_events_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
      ...
      tryCatch({
        ...
        output$event_cards <- renderUI({
          cards <- lapply(seq_len(nrow(data)), function(i) {
            ...
            addr_str  <- if (val(ev$address))    paste0("🗺 ", ev$address)    else ""
            ...
            div(
              ...
              if (nchar(addr_str)  > 0) tags$p(style = "color:#666; font-size:0.85em; margin:2px 0;", addr_str),
        ...
        # ── TAB 4: MAP — check GPS data first ────────────────
        map_data <- data[
          !is.na(data$latitude) & !is.na(data$longitude) &
          data$latitude  != "N/A" & data$longitude != "N/A" &
          nchar(trimws(as.character(data$latitude)))  > 0 &
          nchar(trimws(as.character(data$longitude))) > 0, ]

        valid_lats <- suppressWarnings(as.numeric(as.character(map_data$latitude)))
        valid_lngs <- suppressWarnings(as.numeric(as.character(map_data$longitude)))
        has_gps    <- sum(!is.na(valid_lats) & !is.na(valid_lngs)) > 0

        output$map_or_message <- renderUI({
          if (!has_gps) {
            div(
              style = "text-align:center; padding:60px 20px;",
              tags$i(class = "fa fa-map-marker-alt fa-3x", style = "color:#ccc; margin-bottom:15px;"),
              tags$h4(style = "color:#888;", "GPS Data Not Available"),
              tags$p(style = "color:#aaa;",
                     "No latitude/longitude coordinates were included in this scan.",
                     tags$br(),
                     "Re-scan with ", tags$strong("Core + Location"), " or ", tags$strong("Full"),
                     " optional fields selected to enable the map view.")
            )
          } else {
            leaflet::leafletOutput(session$ns("map"), height = "550px")
          }
        })
          ...
          output$map <- leaflet::renderLeaflet({
            m <- leaflet::leaflet() %>%
              ...
              leaflet::addLegend(
                ...
                colors   = unname(category_colors[c("Music","Tech","Art","Food","Sports","Family","Business","Other")]),
                labels   = c("Music","Tech","Art","Food","Sports","Family","Business","Other"),
                title    = "Category", opacity = 0.9
              )

            valid <- !is.na(valid_lats) & !is.na(valid_lngs)
            map_valid <- map_data[valid, ]

            for (i in seq_len(nrow(map_valid))) {
              row <- map_valid[i, ]
              lat <- valid_lats[which(valid)[i]]
              lng <- valid_lngs[which(valid)[i]]
              col <- get_color(as.character(row$category))

              price_str  <- if (has_real_value(row$price_range) && row$price_range != "N/A")
                paste0(" · ", row$price_range) else ""
              ticket_link <- if (has_real_value(row$ticket_url) && row$ticket_url != "N/A")
                sprintf('<br><a href="%s" target="_blank" style="color:#008A82;font-weight:bold;">🎟 Buy Tickets</a>', row$ticket_url) else ""

              popup_html <- sprintf(
                '<div style="min-width:220px;font-family:sans-serif;">
                   <strong style="color:#002C3C;font-size:1.1em;">%s</strong><br>
                   <span style="background:%s;color:white;padding:2px 8px;border-radius:10px;font-size:.8em;">%s</span>
                   <span style="color:#555;font-size:.8em;"> %s</span><br><br>
                   <span>📅 %s %s%s</span><br>
                   <span>📍 %s</span><br>
                   <span style="font-size:.85em;color:#555;">%s</span>%s
                 </div>',
                as.character(row$event_name), col, as.character(row$category),
                if (has_real_value(row$subcategory)) as.character(row$subcategory) else "",
                as.character(row$event_date), as.character(row$event_time), price_str,
                as.character(row$venue_name),
                substr(as.character(row$description), 1, 120),
                ticket_link
              )

              m <- m %>% leaflet::addCircleMarkers(
                lng = lng, lat = lat, color = col, fillColor = col,
                fillOpacity = 0.8, opacity = 1, weight = 2, radius = 10,
                popup = popup_html, label = as.character(row$event_name)
              )
            }

            m %>% leaflet::fitBounds(
              lng1 = min(valid_lngs[valid]) - 0.1, lat1 = min(valid_lats[valid]) - 0.1,
              lng2 = max(valid_lngs[valid]) + 0.1, lat2 = max(valid_lats[valid]) + 0.1
            )
          })
        }
        ...
        output$status <- renderUI({
          tags$div(class = "status-success",
                   ...
                   if (!has_gps) tags$span(style = "color:#e67e22; margin-left:10px;",
                                           "⚠️ No GPS data — Map tab shows guidance") else NULL)
        })
    ...
    output$map_or_message  <- renderUI({
      tags$div(style = "color:#999; padding:20px;", "Load visualizations to see the map.")
    })
    output$map             <- leaflet::renderLeaflet({ leaflet::leaflet() %>% leaflet::addTiles() })
