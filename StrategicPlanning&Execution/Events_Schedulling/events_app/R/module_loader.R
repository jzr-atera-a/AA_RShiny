# R/module_loader.R
# ModuleLoader R6 Class — base R only, no purrr dependency
# =========================================================
...
ModuleLoader <- R6::R6Class(
  ...
  public = list(
    ...
    discover_modules = function() {
      ...
      # Sort by priority using base R
      priorities <- sapply(self$modules, function(m) {
        self$registry$modules[[m$module$id]]$priority %||% 99
      })
      self$modules <- self$modules[order(priorities)]
      cat("✓ Discovered", length(self$modules), "modules\n")
    },

    get_enabled_modules = function() {
      # base R Filter instead of purrr::keep
      Filter(function(m) isTRUE(m$module$enabled), self$modules)
    },
    ...
    print = function() {
      ...
      disabled <- Filter(function(m) !isTRUE(m$module$enabled), self$modules)
