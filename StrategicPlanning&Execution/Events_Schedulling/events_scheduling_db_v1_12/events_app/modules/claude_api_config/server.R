...
claude_api_config_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
      ...
      tryCatch({
        ...
        output$status <- renderUI({
          tags$div(class = "status-success",
                   tags$i(class = "fa fa-check-circle"), " Connection Successful!",
                   tags$br(), tags$small("Model: ", input$model),
                   tags$br(), tags$small("Timeout: ", input$timeout, " seconds"),
                   tags$br(), tags$small("Status: Ready to scan events"))
        })
        showNotification("✓ Claude API connection successful!", type = "message")
      }, error = function(e) {
        output$status <- renderUI({
          tags$div(class = "status-error",
                   tags$i(class = "fa fa-times-circle"), " Connection Failed:",
    ...
    observeEvent(input$save_credentials, {
      ...
      output$status <- renderUI({
        tags$div(class = "status-success",
                 tags$i(class = "fa fa-check-circle"), " Credentials Saved",
                 tags$br(), tags$small("Model: ", input$model),
                 tags$br(), tags$small("Max Tokens: ", input$max_tokens),
                 tags$br(), tags$small("Timeout: ", input$timeout, " seconds"))
      })
    })
