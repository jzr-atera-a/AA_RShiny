...
send_email_server <- function(id, contact_manager, parent_session) {
  moduleServer(id, function(input, output, session) {
    
    # Observer to populate email fields when coming from customise_communication
    observe({
      contact_manager$state_trigger()  # Create dependency
      
      # Only update if we have both email and message
      if (!is.null(contact_manager$selected_contact_email) && 
          !is.null(contact_manager$generated_message)) {
        
        isolate({
          # Only update if fields are currently empty to avoid overwriting user edits
          if (input$email_to == "") {
            updateTextInput(session, "email_to", value = contact_manager$selected_contact_email)
          }
          if (input$email_body == "") {
            updateTextAreaInput(session, "email_body", value = contact_manager$generated_message)
          }
          if (input$email_subject == "") {
            # Generate subject from contact manager data if available
            if (!is.null(contact_manager$selected_contact)) {
              subject <- paste("Following up -", contact_manager$selected_contact$full_name)
