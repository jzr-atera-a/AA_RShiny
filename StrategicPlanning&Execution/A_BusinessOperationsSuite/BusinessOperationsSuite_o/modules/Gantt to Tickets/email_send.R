# modules/Gantt to Tickets/email_send.R
# Subtab: Send Email Notifications
  ...
  tagList(
    ...
    fluidRow(
      box(title = "Preview Emails", status = "info", solidHeader = TRUE, width = 12,
          DT::dataTableOutput(ns("email_preview_table")), br(),
          p(strong("Summary:")), verbatimTextOutput(ns("email_summary")))
    ),
    fluidRow(
      box(title = "Email Options", status = "warning", solidHeader = TRUE, width = 6,
          ...
          checkboxInput(ns("include_attachments"), "Include task list as CSV attachment", value = FALSE),
      ...
      box(title = "Send Emails", status = "success", solidHeader = TRUE, width = 6,
          ...
          br(),
          actionButton(ns("send_test_email"), "Send Test Email to Myself", class = "btn-info btn-block"),
          br(), verbatimTextOutput(ns("email_send_result")))
    )
  )
}
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$email_preview_table <- DT::renderDataTable({
      ...
      if ("Assignee" %in% names(api_manager$gantt_data)) {
        ...
        DT::datatable(email_preview, options = list(scrollX = TRUE, pageLength = 10))
      }
    })
    ...
    output$email_summary <- renderText({
      req(api_manager$gantt_data)
      if ("Assignee" %in% names(api_manager$gantt_data)) {
        tasks_with_assignees <- api_manager$gantt_data %>% dplyr::filter(!is.na(Assignee) & Assignee != "")
        unique_assignees <- unique(tasks_with_assignees$Assignee)

        if (input$group_by_assignee) {
          paste0("Total tasks to notify: ", nrow(tasks_with_assignees), "\n",
    ...
    observeEvent(input$send_test_email, {
      req(api_manager$gantt_data, api_manager$gantt_email_connected)
      if (nrow(api_manager$gantt_data) == 0) { showNotification("No tasks to preview", type = "warning"); return() }

      sample_task <- api_manager$gantt_data[1, ]
      subject <- "Test Email - Task Assignment"
      body <- paste0("**THIS IS A TEST EMAIL**\n\n",
      ...
      tryCatch({
        api_manager$send_gantt_email(to = api_manager$gantt_smtp_config$user, subject = subject, body = body)
    ...
    observeEvent(input$send_emails, {
      ...
      tasks_with_assignees <- api_manager$gantt_data %>% dplyr::filter(!is.na(Assignee) & Assignee != "")
      if (nrow(tasks_with_assignees) == 0) { showNotification("No tasks with assignees found", type = "warning"); return() }
      ...
      tryCatch({
        if (input$group_by_assignee) {
          ...
          withProgress(message = 'Sending emails...', value = 0, {
            for (assignee in unique_assignees) {
              assignee_tasks <- tasks_with_assignees %>% dplyr::filter(Assignee == assignee)

              task_list <- ""
              for (i in 1:nrow(assignee_tasks)) {
                task <- assignee_tasks[i, ]
                task_list <- paste0(task_list, "\n\n--- Task ", i, " ---\n",
                                    "Task: ", task$Task_Name, "\n",
              ...
              body <- paste0("Hello ", assignee, ",\n\n",
                             "You have been assigned ", nrow(assignee_tasks), " task(s):\n", task_list,
                             "\n\nPlease review and confirm.\n\nBest regards")

              to_email <- parse_email(assignee)
              api_manager$send_gantt_email(to = to_email, subject = subject, body = body)
          ...
          withProgress(message = 'Sending emails...', value = 0, {
            for (i in 1:nrow(tasks_with_assignees)) {
              ...
              body <- paste0("Hello ", task$Assignee, ",\n\n",
        ...
        showNotification(paste("Successfully sent", length(results), "email(s)!"), type = "message", duration = 10)

      }, error = function(e) {
        showNotification(paste("Error sending emails:", e$message), type = "error")
        output$email_send_result <- renderText(paste("Error:", e$message, "\n\nResults so far:\n", paste(results, collapse = "\n")))
      })
    })
