# modules/Receipt Processor/receipt_to_pdf_converter.R
# Subtab: Convert to PDF
# (Same <- vs <<- error-handler bug as receipt_pdf_converter.R, fixed the same way.)

receipt_to_pdf_converter_ui <- function(id) {
  ns <- NS(id)

  tagList(
    fluidRow(
      box(title = "Convert Images to PDF", status = "primary", solidHeader = TRUE, width = 12,
          ...
          hr(),
          radioButtons(ns("pdf_input_type"), "Select Input Type:",
                       choices = c("Individual Files" = "files", "Entire Folder" = "folder"), selected = "files"),
          conditionalPanel(condition = sprintf("input['%s'] == 'files'", ns("pdf_input_type")), ns = ns,
                            fileInput(ns("image_files_to_pdf"), "Choose Image Files:", multiple = TRUE,
                                      accept = c(".jpg", ".jpeg", ".png", ".bmp", ".tiff", ".gif", "image/*"),
                                      placeholder = "Select one or more image files")),
          conditionalPanel(condition = sprintf("input['%s'] == 'folder'", ns("pdf_input_type")), ns = ns,
                            shinyDirButton(ns("image_folder_browser"), "Browse for Input Folder", "Select folder containing images",
                                           class = "btn-primary", icon = icon("folder-open")),
                            hr(), verbatimTextOutput(ns("selected_input_folder")), verbatimTextOutput(ns("input_folder_status"))),
          hr(),
          selectInput(ns("pdf_page_size"), "PDF Page Size:", choices = c("A4" = "A4", "Letter" = "letter"), selected = "A4"),
          p(class = "text-muted", "A4: 210 x 297 mm (International) | Letter: 8.5 x 11 inches (US)"),
          hr(),
          shinyDirButton(ns("pdf_output_browser"), "Browse for Output Folder", "Select folder to save PDF files",
                         class = "btn-primary", icon = icon("folder-open")),
          hr(), verbatimTextOutput(ns("selected_output_folder")), verbatimTextOutput(ns("output_folder_status")),
          hr(),
          actionButton(ns("convert_to_pdf"), "Convert to PDF", class = "btn-success", icon = icon("file-pdf")),
          hr(),
...
receipt_to_pdf_converter_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {

    rv <- reactiveValues(pdf_conversion_results = NULL, selected_input_folder = NULL, selected_output_folder = NULL)
      ...
      if (!is.integer(input$image_folder_browser)) {
        ...
        if (length(folder_path) > 0) {
          ...
          if (dir.exists(folder_path)) {
            ...
            output$selected_input_folder <- renderText(paste0("Selected folder:\n", folder_path))
            ...
            if (length(image_files) == 0) {
              ...
              showNotification(paste("Found", length(image_files), "image files"), type = "message", duration = 3)
            }
          }
        }
      }
    ...
    observeEvent(input$pdf_output_browser, {
      if (!is.integer(input$pdf_output_browser)) {
        ...
        if (length(folder_path) > 0) {
          ...
          output$selected_output_folder <- renderText(paste0("Selected folder:\n", folder_path))
    ...
    observeEvent(input$convert_to_pdf, {
      if (!requireNamespace("magick", quietly = TRUE)) {
        showModal(modalDialog(title = "Package Required",
          HTML("The 'magick' package is required for PDF conversion.<br><br>Please install it by running:<br><code>install.packages('magick')</code>"),
      ...
      if (input$pdf_input_type == "files") {
        ...
        files_to_convert <- data.frame(name = basename(image_files), datapath = image_files, stringsAsFactors = FALSE)
      }
      ...
      if (page_size == "A4") { page_width <- 210; page_height <- 297 } else { page_width <- 215.9; page_height <- 279.4 }
      ...
      withProgress(message = 'Converting images to PDF...', value = 0, {
        ...
        for (i in 1:nrow(files_to_convert)) {
          ...
          incProgress(1 / nrow(files_to_convert), detail = paste("Converting", file_info$name))
          ...
          tryCatch({
            ...
            img <- magick::image_resize(img, geometry = paste0(page_width_px, "x", page_height_px))
            ...
            results_list[[i]] <- data.frame(image_file = file_info$name, pdf_file = output_filename,
                                             page_size = page_size, status = "Success", stringsAsFactors = FALSE)
          }, error = function(e) {
            results_list[[i]] <<- data.frame(image_file = file_info$name, pdf_file = "", page_size = page_size,
                                              status = paste("Error:", e$message), stringsAsFactors = FALSE)
          })
        }

        if (length(results_list) > 0) rv$pdf_conversion_results <- do.call(rbind, results_list)
      })
        ...
        output$pdf_conversion_status <- renderUI({
          if (error_count > 0) {
            tags$div(class = "alert alert-success", tags$strong("Completed! "),
                     sprintf("Successfully converted %d image(s) to PDF. %d error(s) occurred.", success_count, error_count),
                     tags$br(), tags$small(paste("PDF files saved to:", output_folder)))
          } else {
            tags$div(class = "alert alert-success", tags$strong("✓ Success! "),
                     sprintf("Successfully converted all %d image(s) to PDF format (%s) at 300 DPI.", total_count, page_size),
                     tags$br(), tags$small(paste("PDF files saved to:", output_folder)))
          }
        })
    ...
    output$pdf_conversion_results_table <- DT::renderDataTable({
      ...
      DT::datatable(rv$pdf_conversion_results, options = list(pageLength = 10, scrollX = TRUE, dom = 'frtip'), rownames = FALSE)
    })
  })
}
