# modules/Gantt to Tickets/email_configuration.R
# Subtab: Email Configuration (SMTP, blastula-based - separate from the
# Communications suite's curl-based SMTP Configuration tab; see api_manager.R
# comment for why these are kept distinct)
...
email_configuration_ui <- function(id) {
  ...
  tagList(
    fluidRow(
      box(title = "SMTP Email Configuration", status = "primary", solidHeader = TRUE, width = 12,
          p("Configure your email settings to send task notifications directly to team members."),
          hr(),
          selectInput(ns("email_provider"), "Email Provider:", choices = c("Gmail", "Outlook/Office365", "Custom SMTP")),
          textInput(ns("smtp_server"), "SMTP Server:", placeholder = "smtp.gmail.com"),
          numericInput(ns("smtp_port"), "SMTP Port:", value = 587, min = 1, max = 65535),
          textInput(ns("smtp_username"), "Email Address:", placeholder = "your.email@example.com"),
          passwordInput(ns("smtp_password"), "Password/App Password:", ""),
          checkboxInput(ns("use_ssl"), "Use SSL/TLS", value = TRUE),
          actionButton(ns("test_email"), "Test Email Connection", class = "btn-success"),
          br(), br(), htmlOutput(ns("email_status")))
    ),
    fluidRow(
      box(title = "Email Template Settings", status = "info", solidHeader = TRUE, width = 12,
          textInput(ns("email_subject_template"), "Email Subject Template:", value = "New Task Assignment: {Task_Name}",
    ...
    fluidRow(
      box(title = "Gmail & Outlook Instructions", status = "warning", width = 12,
...
email_configuration_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$test_email, {
      ...
      tryCatch({
        api_manager$set_gantt_smtp_config(host = input$smtp_server, port = input$smtp_port,
                                           user = input$smtp_username, password = input$smtp_password, use_ssl = input$use_ssl)
        api_manager$test_gantt_email_connection()

        output$email_status <- renderUI(tags$div(class = "status-success", tags$i(class = "fa fa-check-circle"),
                                                   " ✓ Email configuration successful! Test email sent to your address."))
        showNotification("Test email sent successfully!", type = "message")
      }, error = function(e) {
        output$email_status <- renderUI(tags$div(class = "status-error", tags$i(class = "fa fa-times-circle"),
                                                   " Email configuration failed: ", tags$br(), tags$small(e$message)))
