...
server <- function(input, output, session) {
  ...
  # Function to get context from Tab 2
  get_tab2_context <- function() {
    context <- ""
  ...
  # BUSINESS CASE GENERATORS (with context from Tab 2)
  
  # Generate Business Case 1
  observeEvent(input$generate_bc1, {
    ...
    context <- get_tab2_context()
    
    prompt <- paste0(
      "CONTEXT FROM PREVIOUS SECTIONS:", context, "\n\n",
      "Based on the above context and the following main ideas, answer this question:\n\n",
      "What mobility challenge or gap are you addressing, and what is the size and timing of the opportunity which this business case can unlock?\n\n",
      "Main Ideas:\n", input$main_ideas_bc1, "\n\n",
      "Write a comprehensive response of approximately ", input$word_limit_bc1, " words. ",
      "Be specific, data-driven, and align with the project context provided above."
    )
    
    showNotification("Generating response with context from Project Details...", 
                     type = "message", 
                     duration = NULL, 
                     id = "gen_bc1")
    
    result <- call_chatgpt(prompt, input$word_limit_bc1, "Problem & Market")
  ...
  observeEvent(input$generate_bc2, {
    ...
    result <- call_chatgpt(prompt, input$word_limit_bc2, "CAM Service")
  ...
  observeEvent(input$generate_bc3, {
    ...
    result <- call_chatgpt(prompt, input$word_limit_bc3, "Readiness")
  ...
  observeEvent(input$generate_bc4, {
    ...
    result <- call_chatgpt(prompt, input$word_limit_bc4, "Feasibility")
  ...
  observeEvent(input$generate_bc5, {
    ...
    result <- call_chatgpt(prompt, input$word_limit_bc5, "Commercialisation")
  ...
  # Save Business Case to Excel (uses same file from Tab 2)
  observeEvent(input$save_business_case, {
    ...
    tryCatch({
      # Calculate word counts for business case
      ...
      # Create data frame for business case
      data_bc <- data.frame(
      ...
      # Load existing workbook and APPEND to existing sheet
      if (file.exists(file_path)) {
        wb <- loadWorkbook(file_path)
        ...
        if (input$sheet_name %in% names(wb)) {
          ...
          # Combine with new business case data
          combined_data <- rbind(existing_data, data_bc)
          
          # Remove and recreate worksheet with combined data
          removeWorksheet(wb, input$sheet_name)
          addWorksheet(wb, input$sheet_name)
