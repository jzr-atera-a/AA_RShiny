  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "timesheet",
              # Load Previous Session Box - NEW: Added at the top
              fluidRow(
                box(
                  ...
                  fluidRow(
                    column(8,
                           fileInput("load_timesheet_file", "Load Timesheet from Excel",
                                     accept = c(".xlsx", ".xls"),
                                     placeholder = "Browse for saved timesheet...")
                    ),
                    column(4,
                           br(),
                           actionButton("load_timesheet", "Load Timesheet",
                                        icon = icon("upload"),
                                        class = "btn-warning",
...
server <- function(input, output, session) {
  ...
  rv <- reactiveValues(
    ...
    gantt_data = NULL,
    work_packages = NULL,
    wp_date_ranges = NULL,  # Store date ranges for each WP
    data_version = 0  # Version counter to force reactivity updates
  )
  
  # NEW: Load timesheet from Excel file
  observeEvent(input$load_timesheet, {
    req(input$load_timesheet_file)
    
    tryCatch({
      # Read the Excel file
      loaded_df <- read_excel(input$load_timesheet_file$datapath)
      
      # Validate the structure - check for required columns
      required_cols <- c("Date", "DoW", "Task", "WorkPackage", "StartTime", "EndTime", "Hours", "Description")
      missing_cols <- setdiff(required_cols, names(loaded_df))
      
      if (length(missing_cols) > 0) {
        output$load_status <- renderUI({
          div(class = "alert alert-danger",
              icon("times-circle"), 
              sprintf(" Invalid file format. Missing columns: %s", paste(missing_cols, collapse = ", ")))
        })
        return()
      }
      
      # Convert Date column to Date type
      loaded_df$Date <- as.Date(loaded_df$Date)
      
      # Convert Hours to numeric
      loaded_df$Hours <- as.numeric(loaded_df$Hours)
      
      # Ensure character columns
      loaded_df$DoW <- as.character(loaded_df$DoW)
      loaded_df$Task <- as.character(loaded_df$Task)
      loaded_df$WorkPackage <- as.character(loaded_df$WorkPackage)
      loaded_df$StartTime <- as.character(loaded_df$StartTime)
      loaded_df$EndTime <- as.character(loaded_df$EndTime)
      loaded_df$Description <- as.character(loaded_df$Description)
      
      # Replace NA with empty strings for character columns
      loaded_df$StartTime[is.na(loaded_df$StartTime)] <- ""
      loaded_df$EndTime[is.na(loaded_df$EndTime)] <- ""
      loaded_df$Description[is.na(loaded_df$Description)] <- ""
      
      # Update the reactive value
      rv$timesheet_data <- as.data.frame(loaded_df)
      
      # Increment version counter to force infoBox updates
      ...
      output$load_status <- renderUI({
        div(class = "alert alert-success",
            icon("check-circle"), 
            sprintf(" Timesheet loaded successfully! %d rows loaded.", nrow(loaded_df)),
            br(),
            tags$small(sprintf("Date range: %s to %s", 
                               min(loaded_df$Date), 
                               max(loaded_df$Date))))
      })
      
      showNotification(
        sprintf("Loaded %d rows from previous session", nrow(loaded_df)),
        type = "message",
        duration = 5
      )
      ...
      output$load_status <- renderUI({
        div(class = "alert alert-danger",
            icon("times-circle"), " Error loading file: ", e$message)
      })
    })
  })
  ...
  generate_timesheet <- function() {
    ...
    # Create base dataframe with explicit numeric Hours
    df <- data.frame(
      Date = all_dates,
      ...
      Hours = as.numeric(ifelse(weekdays(all_dates) == "Sunday", 0, 8)),
      Description = "",
  ...
  # Render editable timesheet table - MODIFIED: Changed pageLength to 120
  output$timesheet_table <- renderDT({
    ...
    datatable(
      ...
      options = list(
        pageLength = 120,  # CHANGED: Display 120 rows by default
        lengthMenu = c(31, 60, 90, 120, 150),  # Added options for different page sizes
        scrollX = TRUE,
        columnDefs = list(
  ...
  observeEvent(input$timesheet_table_cell_edit, {
    ...
    temp_df <- rv$timesheet_data
    
    # Update the cell value in the copy
    temp_df[row, col] <- value
    ...
    if (col_name == "WorkPackage") {
      ...
      if (!is.null(rv$work_packages) && value %in% names(rv$work_packages)) {
        ...
        temp_df$Description[row] <- wp_title
    ...
    # If Hours column is edited, update EndTime and ensure Hours is numeric
    if (col_name == "Hours") {
      # Convert to numeric
      temp_df$Hours[row] <- as.numeric(value)
      
      start_time <- temp_df$StartTime[row]
      if (start_time != "" && !is.na(value) && as.numeric(value) > 0) {
        start_hour <- as.numeric(substr(start_time, 1, 2))
        end_hour <- start_hour + as.numeric(value)
        temp_df$EndTime[row] <- sprintf("%02d:00", end_hour)
      }
    }
    
    # Reassign the entire dataframe to trigger reactivity
    rv$timesheet_data <- temp_df
  ...
  output$total_hours_box <- renderInfoBox({
    ...
    # Force dependency on version counter to ensure updates
    rv$data_version
