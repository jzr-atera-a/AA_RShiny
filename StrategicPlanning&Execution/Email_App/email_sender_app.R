...
server <- function(input, output, session) {
  ...
  observeEvent(input$test_connection, {
    ...
    showNotification("Testing SMTP...", type = "message", duration = NULL, id = "testing")
    
    tryCatch({
      send_email_with_curl(
        from = input$smtp_username,
        to = input$smtp_username,
        subject = "SMTP Test - Email Sender App",
        body = "Your SMTP configuration is working!\n\nYou can now send emails.",
      ...
      connection_state$message <- paste0(
        "✓ TEST SUCCESS!\n\n",
        "Test email sent to: ", input$smtp_username, "\n",
        "Server: ", input$smtp_host, ":", input$smtp_port, "\n\n",
        "Click 'Open Connection'"
      )
      
      removeNotification(id = "testing")
      showNotification("✓ Test passed! Check inbox.", type = "message", duration = 5)
