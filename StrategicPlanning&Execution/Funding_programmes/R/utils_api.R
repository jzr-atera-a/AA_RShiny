...
APIManager <- R6::R6Class(
  ...
  public = list(
    ...
    claude_timeout = 180,  # 3 minute default timeout
    claude_authenticated = FALSE,
    
    # BigQuery credentials
    bq_project_id = "atera-2",
    bq_dataset_id = "business_strategy",
    bq_table_id = "funding_programmes",
    ...
    # ⭐ Cross-module data handoff: lets generate_schedule push text into
    ...
    # Push generated schedule text to the Bulk Import module
    ...
    call_claude = function(prompt, max_tokens = NULL, progress_callback = NULL) {
      ...
      tryCatch({
        ...
        # IMPORTANT: check specific network/SSL failures BEFORE the generic
        # "timeout" check. curl's own error text for a failed SSL/TLS
        # handshake is "Timeout was reached [host]: SSL/TLS connection
        # timeout" - it contains the word "Timeout" but has NOTHING to do
        # with the claude_timeout setting below. This is a connection-phase
        # failure (R/curl couldn't even establish a secure connection),
        # not a "Claude took too long to respond" situation, and it
        # typically fails in seconds, not after the full configured
        # timeout. Checking generic "timeout" first would mislabel this as
        # a slow-response timeout and falsely claim it waited the full
        # configured duration.
        if (grepl("SSL|TLS|certificate|handshake", error_msg, ignore.case = TRUE)) {
          error_msg <- paste0(
            "Network/SSL error connecting to Claude API: ", error_msg, ". ",
            "This means R/curl could not establish a secure connection to api.anthropic.com at all - ",
            "it is NOT the same as the Timeout setting below, and increasing it will not fix this. Try: ",
            "(1) Check firewall/proxy rules allow outbound HTTPS to api.anthropic.com on port 443, ",
            "(2) If behind a corporate proxy, set the https_proxy environment variable before launching R, ",
            "(3) Update the 'curl' and 'openssl' R packages and your system's CA certificate bundle, ",
            "(4) Test with httr::GET('https://api.anthropic.com') directly in the R console to confirm ",
            "whether the problem is specific to this app or affects all HTTPS requests from this machine, ",
            "(5) If running on shinyapps.io, Posit Connect, or behind a VPN, confirm outbound access to ",
            "api.anthropic.com is permitted by that environment's network policy."
          )
        } else if (grepl("timeout", error_msg, ignore.case = TRUE)) {
          error_msg <- paste0(
            "Request timeout - Claude did not respond within the configured ", self$claude_timeout, 
            " second limit. ",
            "Try: (1) Increase timeout in Claude API Config, ",
            "(2) Reduce Max Tokens / content length, or (3) Try again later."
          )
    ...
    authenticate_bigquery = function(json_path = NULL, json_text = NULL) {
      tryCatch({
        ...
        # Create table if not exists - funding_programmes schema
        create_table_query <- sprintf("
          CREATE TABLE IF NOT EXISTS `%s` (
            ...
            start_date_for_applying STRING,
            deadline STRING,
    ...
    # Get distinct category/country/city_region combinations, used to build
    # the cascading Category and Country -> City/Region dropdowns in
    # generate_programme, add_single, and visualizations.
    bq_get_taxonomy = function() {
      empty_result <- data.frame(category = character(), country = character(),
