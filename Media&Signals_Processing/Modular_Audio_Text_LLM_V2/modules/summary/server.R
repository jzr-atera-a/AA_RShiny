summary_server <- function(id, api_manager, session) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$generateBtn, {
      ...
      # CRITICAL FIX: Properly check timeout checkbox
      ...
      tryCatch({
        ...
        cat("   Max tokens:", api_manager$chatgpt_max_tokens, "\n\n")
        flush.console()
        ...
        output$status <- renderText({
          paste0(
            ...
            "✓ Max tokens: ", api_manager$chatgpt_max_tokens, "\n\n",
            "Status: Creating message body...\n"
          )
        })
        
        Sys.sleep(0.5)
        
        cat("📤 Building ChatGPT request...\n")
        flush.console()
        
        messages <- list(
          list(role = "system", content = "You are a helpful assistant that creates detailed, well-structured summaries."),
          list(role = "user", content = paste0(input$instructions, "\n\nTRANSCRIPTION TO SUMMARIZE:\n\n", values$transcript))
        )
        ...
        output$status <- renderText({
          paste0(
            ...
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n",
            "✅ Response received!\n",
            "⏱️  API call took: ", round(elapsed, 2), " seconds\n\n",
        ...
        cat("   Characters:", format(nchar(summary_text), big.mark = ","), "\n")
        flush.console()
        ...
        cat("API call time:", round(elapsed, 2), "seconds\n")
        cat(rep("=", 60), "\n\n", sep = "")
        flush.console()
        ...
        cat("Transcript loaded:", !is.null(values$transcript), "\n")
        cat(rep("=", 60), "\n\n", sep = "")
        flush.console()
