...
# Async processing setup (used by Video Audio Extractor to avoid blocking
# the UI/console during long MP4 -> MP3 extraction jobs)
if (requireNamespace("future", quietly = TRUE) && requireNamespace("promises", quietly = TRUE)) {
  suppressPackageStartupMessages({
    library(future)
    library(promises)
  })
  # multisession = separate background R process(es); works on Windows,
  # unlike multicore/forking which Windows doesn't support.
  future::plan(future::multisession, workers = 2)
  cat("Async processing enabled (future::multisession)\n")
} else {
  cat("Package not installed: future and/or promises - Video Audio Extractor will run synchronously and may block the UI during extraction\n")
}
