          ...
          div(class = "wg-card-body",
            fluidRow(
              ...
              column(3,
                div(class = "wg-field-group",
                  ...
                  conditionalPanel(
                    ...
                    div(style = "margin-top:6px;padding:8px 10px;background:#1a2d45;border-left:3px solid #1a9b9b;border-radius:3px;",
                      tags$i(class = "fa fa-info-circle", style = "color:#1a9b9b;margin-right:6px;"),
                      tags$small(style = "color:#7ec8e3;",
              ...
              column(3,
                ...
                conditionalPanel(
                  condition = sprintf("input['%s'] != 'vehicles'", ns("query_type")),
                  div(style = "margin-top:6px;padding:7px 10px;background:#1a2d45;border-left:3px solid #fd7e14;border-radius:3px;",
        ...
        div(class = "wg-card", style = "margin-top:6px;",
          ...
          div(class = "wg-card-body",
            tabsetPanel(id = ns("result_tabs"),
              tabPanel("Data Table",
                tags$br(), uiOutput(ns("data_table_ui"))
              ),
              tabPanel("Raw JSON",
                tags$br(),
                div(style = "margin-bottom:8px;display:flex;align-items:center;gap:12px;",
                  tags$span(style = "color:#7ec8e3;font-size:13px;",
                ...
                div(style = "background:#0d1921;border:1px solid #253a52;border-radius:4px;
  ...
  moduleServer(id, function(input, output, session) {
      ...
      if (length(cache) == 0) {
        ...
        make_label <- function(v) {
          ...
          as.character(tags$span(
            ...
            tags$br(),
            tags$small(style = "color:#4a7a9b;font-family:monospace;font-size:10px;",
              vin)
          ))
        }
        ...
        # Resolve any stale NAME: values that may be lingering in input$selected_vehicles
        # from before the connection was established. These are pre-connection placeholders
        # that no longer exist in the rebuilt choices — map them back to real VINs now.
        current_sel <- isolate(input$selected_vehicles) %||% character(0)
        resolved_sel <- sapply(current_sel, function(val) {
          if (startsWith(val, "NAME:")) {
            target <- sub("^NAME:", "", val)
            found  <- ""
            for (v in cache) {
              nm  <- v$customerVehicleName %||% ""
              reg <- v$volvoGroupVehicle$registrationNumber %||% ""
              if (identical(nm, target) || identical(reg, target)) {
                found <- v$vin %||% ""
                break
              }
            }
            ...
            val   # already a VIN or "ALL"
          }
        })
        resolved_sel <- unique(resolved_sel[nchar(resolved_sel) > 0])

        # Priority: resolved previous selection > EV trucks > ALL
        sel <- if ("ALL" %in% resolved_sel) {
          "ALL"
        } else if (length(resolved_sel) > 0) {
          resolved_sel
        } else if (length(ev_vins) > 0) {
          ev_vins
        } else {
          "ALL"
        }
        cat("[TAB2] vehicle_selector_ui rebuilt — EV trucks:", paste(ev_vins, collapse = ", "),
            "| resolved selection:", paste(sel, collapse = ", "), "\n")
      }
    ...
    output$resolved_vins_display <- renderUI({
      ...
      if ("ALL" %in% sel) {
        ...
        div(style = "margin-top:6px;padding:5px 8px;background:#0d1921;
                     border-left:2px solid #1a9b9b;border-radius:2px;",
          tags$small(style = "color:#4a9b8b;font-family:monospace;font-size:10px;", txt))
      } else {
        vins <- resolve_vins()
        if (is.null(vins) || length(vins) == 0) {
          div(style = "margin-top:6px;padding:5px 8px;background:#2a1a1a;border-left:2px solid #e05c5c;border-radius:2px;",
          ...
          rows <- lapply(vins, function(vin) {
            ...
            tags$div(
              tags$span(style = "color:#7ec8e3;font-size:10px;font-family:monospace;", vin),
              tags$span(style = "color:#8899aa;font-size:10px;", paste0(" (", nm, ")"))
            )
          })
          div(style = "margin-top:6px;padding:5px 8px;background:#0d1921;
                       border-left:2px solid #1a9b9b;border-radius:2px;",
            tags$small(style = "color:#4a9b8b;font-size:10px;display:block;margin-bottom:3px;",
    ...
    resolve_vins <- function() {
      ...
      for (val in sel) {
        if (startsWith(val, "NAME:")) {
          # Pre-connection placeholder — match by customerVehicleName OR registrationNumber
          # (covers cases where the two differ or contain spaces e.g. "TA70 WTL")
          target <- sub("^NAME:", "", val)
          for (v in cache) {
            nm  <- v$customerVehicleName %||% ""
            reg <- v$volvoGroupVehicle$registrationNumber %||% ""
    ...
    output$data_table_ui <- renderUI({
      if (is.null(rv$df) || nrow(rv$df) == 0) {
        ...
        tagList(
          ...
          tags$hr(style = "border-color:#253a52;margin:18px 0;"),
          # Positions section
          div(style = "padding:8px 0 4px;",
            tags$span(style = "color:#fd7e14;font-weight:600;font-size:13px;",
              tags$i(class="fa fa-map-marker", style="margin-right:6px;"),
    ...
    output$btn_download_json <- downloadHandler(
      filename = function() {
        paste0("welch_fleet_", rv$query_type %||% "data", "_",
    ...
    dark_layout <- function(title) {
      list(
        ...
        legend = list(font = list(color = "#8fa0b5")),
        font   = list(color = "#8fa0b5")
      )
    }
    ...
    output$fleet_emission <- plotly::renderPlotly({
      ...
      plotly::plot_ly(tbl, labels = ~Var1, values = ~Freq, type = "pie",
        ...
        plotly::layout(!!!dark_layout("Emission Level"), paper_bgcolor = "#131e2b",
                       legend = list(font = list(color = "#8fa0b5")),
                       font   = list(color = "#8fa0b5"))
    })
    ...
    output$statistics_ui <- renderUI({
      ...
      geo_panel <- if (qtype == "positions") {
        ...
        if (nrow(valid) > 0) {
          ...
          div(class = "wg-geo-box",
            tags$h5("Geographic Extent", style = "color:#7ec8e3;margin-bottom:10px;"),
            fluidRow(
      ...
      tagList(
        div(class = "wg-stat-section",
          tags$h5("Dataset Metadata", style = "color:#7ec8e3;margin-bottom:10px;"),
          fluidRow(
        ...
        div(class = "wg-stat-section",
          tags$h5("Numeric Field Statistics", style = "color:#7ec8e3;margin-bottom:10px;"),
          if (!is.null(smry) && nrow(smry) > 0 && "Field" %in% names(smry))
            DT::dataTableOutput(ns("stats_dt"))
          else tags$p("No numeric fields.", style = "color:#8fa0b5;")
        ),
        div(class = "wg-stat-section",
          tags$h5("Column Types", style = "color:#7ec8e3;margin-bottom:10px;"),
          div(style = "display:flex;flex-wrap:wrap;gap:8px;",
            lapply(names(df), function(col) {
              dtype  <- class(df[[col]])[1]
              colour <- switch(dtype, "numeric" = "#1a9b9b", "integer" = "#17a2b8",
                               "character" = "#fd7e14", "logical" = "#6f42c1", "#8fa0b5")
              HTML(sprintf(
                '<div style="background:#1e2a3a;border:1px solid %s;padding:4px 10px;
                 border-radius:4px;font-size:12px;">
                 <span style="color:%s;font-weight:600;">%s</span>
                 <span style="color:#8fa0b5;"> %s</span></div>',
