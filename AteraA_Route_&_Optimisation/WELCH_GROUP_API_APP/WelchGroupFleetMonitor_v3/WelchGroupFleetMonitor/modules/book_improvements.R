...
book_improvements_ui <- function(id) {
  ...
  bi_card <- function(chapter, colour, icon_class, title, body_content) {
    div(style = paste0(
          "background:#ffffff;border-left:5px solid ", colour, ";",
          "border-radius:8px;padding:18px 20px;margin-bottom:18px;box-shadow:0 4px 14px rgba(0,44,60,0.1);"
        ),
      div(style = "display:flex;align-items:center;gap:12px;margin-bottom:14px;",
        ...
        tags$h4(style = paste0("color:", colour, ";margin:0;font-size:16px;font-weight:800;"),
                chapter),
        tags$span(style = "color:#5a7a77;font-size:13px;", title)
      ),
      body_content
    )
  }
  ...
  improvement_row <- function(label, orig, improved, colour = "#1a9b9b") {
    div(style = "display:grid;grid-template-columns:200px 1fr 1fr;gap:10px;padding:8px 0;border-bottom:1px solid #253a52;",
      ...
      div(style = "background:#f8fffe;border:1px solid #b2e0dd;border-radius:4px;padding:8px 10px;",
        tags$small(style = "color:#dc3545;display:block;margin-bottom:4px;", "\u2717 Original"),
        tags$code(style = "color:#5a7a77;font-size:11px;white-space:pre-wrap;", orig)
      ),
      div(style = "background:#f8fffe;border:1px solid #b2e0dd;border-radius:4px;padding:8px 10px;",
        tags$small(style = "color:#28a745;display:block;margin-bottom:4px;", "\u2713 v2 Fixed"),
        tags$code(style = "color:#006b63;font-size:11px;white-space:pre-wrap;", improved)
      )
    )
  }

  # ── Summary metric card (uses paste0, avoids sprintf quoting issues) ──────
  make_metric_card <- function(value, label, colour) {
    HTML(paste0(
      '<div style="flex:1;min-width:130px;background:#ffffff;border-left:4px solid ', colour, ';',
      'padding:12px 16px;border-radius:6px;box-shadow:0 2px 8px rgba(0,44,60,0.12);">',
      '<div style="font-size:28px;font-weight:800;color:#1a2a35;">', value, '</div>',
      '<div style="font-size:11px;color:#5a7a77;text-transform:uppercase;letter-spacing:.5px;">', label, '</div>',
      '</div>'
    ))
  }
    ...
    fluidRow(
      column(12,
        div(style = "display:flex;flex-wrap:wrap;gap:10px;margin-bottom:24px;",
          make_metric_card("8",  "Bugs Fixed",      "#dc3545"),
          make_metric_card("3",  "Tabs Cloned",     "#1a9b9b"),
          make_metric_card("5",  "Book Chapters",   "#fd7e14"),
          make_metric_card("6",  "New Charts",      "#28a745"),
          make_metric_card("1",  "Pagination Loop", "#6f42c1")
        )
      )
    ),
      ...
      column(12,
        bi_card(
          ...
          body_content = tagList(
            ...
            tags$p(style = "color:#5a7a77;margin-bottom:14px;",
            ...
            div(style = "margin-bottom:14px;",
              ...
              div(style = "display:grid;grid-template-columns:1fr 1fr;gap:8px;",
                lapply(list(
                  list("Stateless",
                       "Every request carries Basic Auth. No session cookies. The API rejects requests with missing credentials with 401."),
                  list("Uniform Interface",
                       "Three fixed resource URIs: /vehicles, /vehiclepositions, /vehiclestatuses. All GET \u2014 read-only."),
                  list("Client-Server",
                       "The app (client) stores data; the Volvo API (server) serves it. Separation of concerns."),
                  list("Cacheable",
                       "Ch9 maps each endpoint to a cache TTL: /vehicles=86400s, /vehiclestatuses SNAPSHOT=60s.")
                ), function(r) {
                  div(style = "background:#f0faf9;border-left:2px solid #1a9b9b;padding:8px 12px;border-radius:3px;",
                    tags$b(style = "color:#006b63;font-size:12px;", r[[1]]),
                    tags$p(style = "color:#5a7a77;font-size:11px;margin:4px 0 0;", r[[2]])
                  )
                })
              )
            ),
            ...
            div(style = "margin-bottom:14px;",
              ...
              tags$p(style = "color:#5a7a77;font-size:13px;",
              ...
              div(style = "display:flex;flex-wrap:wrap;gap:6px;",
                lapply(list(
                  ...
                  HTML(paste0(
                    '<span style="background:#edfaf9;border:1px solid ', r[[2]], ';padding:4px 10px;border-radius:4px;font-size:11px;font-family:monospace;">',
                    '<b style="color:', r[[2]], ';">', r[[1]], '</b> ',
                    '<span style="color:#5a7a77;">', r[[3]], '</span></span>'
                  ))
                })
              )
            ),
            ...
            div(
              ...
              div(style = "background:#f0faf9;border-radius:4px;padding:10px 14px;font-family:monospace;font-size:11px;",
                tags$pre(style = "color:#006b63;margin:0;",
                ...
                tags$table(style = "width:100%;font-size:12px;border-collapse:collapse;",
                  ...
                  tags$tbody(
                    lapply(list(
                      ...
                      tags$tr(
                        tags$td(style = "padding:5px 10px;color:#006b63;font-family:monospace;font-size:11px;border-bottom:1px solid #c8e6e4;", r[[1]]),
                        tags$td(style = "padding:5px 10px;color:#1a2a35;border-bottom:1px solid #c8e6e4;", r[[2]]),
                        tags$td(style = "padding:5px 10px;color:#1a9b9b;font-family:monospace;border-bottom:1px solid #c8e6e4;", r[[3]]),
                        tags$td(style = "padding:5px 10px;color:#5a7a77;font-size:11px;border-bottom:1px solid #c8e6e4;", r[[4]])
                      )
                    })
                  )
                )
            ...
            fluidRow(
              column(6,
                div(style = "margin-bottom:14px;",
                  ...
                  div(style = "background:#f0faf9;border-radius:4px;padding:10px 14px;",
                    tags$pre(style = "color:#006b63;font-size:11px;margin:0;",
...
class SnapshotSchema(Schema):
    gnssPosition = fields.Nested(GNSSSchema)
    ...
    volvoGroupSnapshot = fields.Nested(VGSnapshotSchema)')
              ...
              column(6,
                div(style = "margin-bottom:14px;",
                  ...
                  lapply(list(
                    list("dump_only",
                         "receivedDateTime, requestServerDateTime",
                         "Set by Volvo server - never in request params"),
                    list("required",
                         "vin",
                         "Every response must have a VIN"),
                    list("partial=",
                         "accumulatedData, uptimeData",
                         "Not present when contentFilter=SNAPSHOT only"),
                    list("load_only",
                         "(not applicable - Volvo is read-only)",
                         "No write operations; all fields are dump_only perspective")
                  ), function(r) {
                    div(style = "padding:6px 10px;margin-bottom:6px;background:#edfaf9;border-left:2px solid #6f42c1;border-radius:3px;",
                      tags$b(style = "color:#6f42c1;font-family:monospace;font-size:11px;", r[[1]]),
                      tags$span(style = "color:#006b63;font-size:11px;margin-left:8px;", r[[2]]),
                      tags$br(),
                      tags$small(style = "color:#5a7a77;", r[[3]])
                    )
                  })
                )
              )
            ...
            div(
              ...
              tags$p(style = "color:#5a7a77;font-size:13px;",
                "The v2 Field Explorer tab shows which fields from each section",
                " (Base/Snapshot/Accumulated/Uptime/Volvo EV)",
          ...
          body_content = tagList(
            ...
            div(style = "margin-bottom:14px;",
              ...
              div(style = "display:grid;grid-template-columns:1fr 1fr;gap:12px;",
                div(style = "background:#f0faf9;border-radius:4px;padding:10px 14px;",
                  tags$small(style = "color:#fd7e14;display:block;margin-bottom:6px;",
                    "\u2717 Ch8 SQLAlchemy paginate()"),
                  tags$pre(style = "color:#006b63;font-size:11px;margin:0;",
'# Ch8 book pattern:
...
while pag.has_next:
    ...
    pag = Recipe.query.paginate(
        page=page, per_page=20
                ...
                div(style = "background:#f0faf9;border-radius:4px;padding:10px 14px;",
                  tags$small(style = "color:#28a745;display:block;margin-bottom:6px;",
                    "\u2713 Volvo API (v2 .paginated_fetch_v2)"),
                  tags$pre(style = "color:#006b63;font-size:11px;margin:0;",
'# Equivalent Volvo API pattern:
# moreDataAvailable = pag.has_next
# receivedDateTime  = pag.next_num
...
while True:
    ...
    cursor = last + timedelta(seconds=1)')
            ...
            div(style = "margin-bottom:14px;",
              ...
              div(style = "background:#fff5f5;border-left:3px solid #dc3545;border-radius:4px;padding:10px 14px;",
                tags$b(style = "color:#dc3545;", "\u2717 Original vehicle_data.R:"),
                tags$p(style = "color:#5a7a77;font-size:12px;margin:6px 0 0;",
                  "get_vehicle_positions() and get_vehicle_statuses() call the API once and return",
                  " whatever fits in one response. If moreDataAvailable=true, all subsequent pages",
                  " are silently lost. For a fleet with 14 days of history, this can mean losing",
                  " over 95% of the data."
                )
              ),
              div(style = "background:#f0fff4;border-left:3px solid #28a745;border-radius:4px;padding:10px 14px;margin-top:8px;",
                tags$b(style = "color:#28a745;", "\u2713 v2 .paginated_fetch_v2():"),
                tags$p(style = "color:#5a7a77;font-size:12px;margin:6px 0 0;",
                  "Loops until moreDataAvailable=false, advancing the receivedDateTime cursor by",
                  " +1 second each page. Respects 1.2s inter-call sleep (Ch9 rate limit).",
                  " Logs each page to the Pagination [Ch8] diagnostic tab."
                )
              )
            ),
            ...
            div(style = "margin-bottom:14px;",
              ...
              div(style = "overflow-x:auto;",
                tags$table(style = "width:100%;font-size:12px;border-collapse:collapse;",
                  tags$thead(
                    tags$tr(
                      lapply(c("Endpoint / contentFilter","Volvo cadence","Ch9 cache TTL","v2 hint shown"), function(h)
                        tags$th(style = "color:#e83e8c;padding:6px 10px;text-align:left;border-bottom:1px solid #253a52;", h))
                    )
                  ),
                  tags$tbody(
                    lapply(list(
                      ...
                      tags$tr(
                        tags$td(style = "padding:5px 10px;color:#006b63;font-family:monospace;font-size:11px;border-bottom:1px solid #c8e6e4;", r[[1]]),
                        tags$td(style = "padding:5px 10px;color:#1a2a35;border-bottom:1px solid #c8e6e4;", r[[2]]),
                        tags$td(style = "padding:5px 10px;color:#e83e8c;font-weight:700;border-bottom:1px solid #c8e6e4;", r[[3]]),
                        tags$td(style = "padding:5px 10px;color:#5a7a77;font-size:11px;border-bottom:1px solid #c8e6e4;", r[[4]])
                      )
                    })
                  )
            ...
            div(
              ...
              div(style = "display:grid;grid-template-columns:1fr 1fr;gap:10px;",
                div(style = "background:#fff5f5;border-left:3px solid #dc3545;border-radius:4px;padding:10px 14px;",
                  ...
                  tags$pre(style = "color:#5a7a77;font-size:11px;margin:6px 0 0;",
'# Original hardcodes v3.0:
accept_type =
  "application/x.volvogroup
   .com.vehiclestatuses.v3.0+json"
# Triggers 406 on every call!
# Auto-retry works but wastes
# one round-trip per request.')
                ),
                div(style = "background:#f0fff4;border-left:3px solid #28a745;border-radius:4px;padding:10px 14px;",
                  tags$b(style = "color:#28a745;", "\u2713 BUGFIX-8: datetype=received explicit"),
                  tags$pre(style = "color:#006b63;font-size:11px;margin:6px 0 0;",
'# [Ch9] v2 always sends:
q$datetype <- "received"
      ...
      column(12,
        div(style = "background:#2a1a2a;border-left:5px solid #dc3545;border-radius:8px;padding:18px 20px;margin-bottom:18px;",
          ...
          div(style = "display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px;",
            div(style = "background:#f0faf9;border-left:2px solid #dc3545;padding:8px 12px;border-radius:3px;",
...
# Overwrites vehicle_data.R silently!')
            ...
            div(style = "background:#f0faf9;border-left:2px solid #28a745;padding:8px 12px;border-radius:3px;",
...
# Registered as vehicle_data_v2 in YAML.')
    ...
    fluidRow(
      column(12,
        div(style = "background:#ffffff;border-radius:8px;padding:18px 20px;margin-bottom:18px;",
          tags$h4(style = "color:#006b63;margin-bottom:16px;",
          ...
          div(style = "overflow-x:auto;",
            tags$table(style = "width:100%;font-size:12px;border-collapse:collapse;",
              tags$thead(
                tags$tr(
                  lapply(c("Bug / Improvement","Chapter","Original","v2 Behaviour","Tab"),
                    function(h)
                      tags$th(style = "color:#006b63;padding:8px 10px;text-align:left;border-bottom:2px solid #b2e0dd;background:#f8fffe;", h))
                )
              ),
              tags$tbody(
                lapply(list(
                  list("BUGFIX-1","Architecture",
                       "vehicle_data.R + _p.R share function names",
                       "Unique names: vehicle_data_v2_ui/server","vehicle_data_v2"),
                  list("BUGFIX-2","Ch3",
                       "s$volvoGroupAccumulatedData (wrong key)",
                       "acc$volvoGroupAccumulated (correct nesting)","vehicle_data_v2"),
                  list("BUGFIX-3","Ch3",
                       "electricEnergyRecuperated (wrong name)",
                       "electricEnergyRecuperation$energy (nested)","vehicle_data_v2"),
                  list("BUGFIX-4","Ch5",
                       "estimatedDistanceToEmpty as scalar",
                       "estimatedDistanceToEmpty$electric","vehicle_data_v2"),
                  list("BUGFIX-5","Ch3",
                       "snap$grossCombinationVehicleWeight",
                       "s$grossCombinationVehicleWeight (top-level)","vehicle_data_v2"),
                  list("BUGFIX-6","Ch8",
                       "Single-page result, moreDataAvailable ignored",
                       "Full cursor loop until moreDataAvailable=false","vehicle_data_v2"),
                  list("BUGFIX-7","Ch1",
                       "Accept: v3.0 triggers 406+retry every call",
                       "Correct v1.0 type; no wasted round-trip","utils_api_manager"),
                  list("BUGFIX-8","Ch9",
                       "datetype never sent",
                       "datetype=received on all time-window queries","vehicle_data_v2"),
                  list("Cache TTL hints","Ch9",
                       "No polling frequency guidance",
                       "Dynamic TTL hint per contentFilter","vehicle_data_v2"),
                  list("Field Explorer","Ch5",
                       "No section breakdown visible",
                       "Section-by-section fill-rate audit","vehicle_data_v2"),
                  list("EV Analytics","Ch3/Ch5",
                       "No EV-specific charts",
                       "SoC timeline, charging, energy donut, range scatter","vehicle_data_v2"),
                  list("Pagination log","Ch8",
                       "No pagination visibility",
                       "Per-page log with cursor values","vehicle_data_v2"),
                  list("Endpoint tester","Ch1",
                       "Only tests /vehicles",
                       "Tests all 3 endpoints, per-endpoint HTTP status","api_connection_v2"),
                  list("JSON inspector","Ch1",
                       "No raw response in connection tab",
                       "Full JSON inspector with type annotations","api_connection_v2"),
                  list("Status code map","Ch1",
                       "Generic error messages",
                       "Per-code hints matching API spec","api_connection_v2")
                ), function(r) {
                  is_bugfix <- startsWith(r[[1]], "BUGFIX")
                  bg  <- if (is_bugfix) "#2a1a1a" else "#1a2a1a"
                  col <- if (is_bugfix) "#dc3545" else "#28a745"
                  tags$tr(
                    tags$td(style = paste0("padding:5px 10px;font-weight:700;color:", col, ";font-family:monospace;font-size:11px;border-bottom:1px solid #c8e6e4;background:", bg, ";"), r[[1]]),
                    tags$td(style = "padding:5px 10px;color:#6f42c1;font-size:11px;border-bottom:1px solid #c8e6e4;", r[[2]]),
                    tags$td(style = "padding:5px 10px;color:#5a7a77;font-size:11px;border-bottom:1px solid #c8e6e4;", r[[3]]),
                    tags$td(style = "padding:5px 10px;color:#006b63;font-size:11px;border-bottom:1px solid #c8e6e4;", r[[4]]),
                    tags$td(style = "padding:5px 10px;color:#fd7e14;font-family:monospace;font-size:11px;border-bottom:1px solid #c8e6e4;", r[[5]])
                  )
                })
              )
            )
