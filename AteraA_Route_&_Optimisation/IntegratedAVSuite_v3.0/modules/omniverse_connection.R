  ...
  tagList(
    ...
    fluidRow(
      ...
      box(title = "Route", status = "info", solidHeader = TRUE, width = 4,
          ...
          hr(),
          sliderInput(ns("numScenarios"), "Number of Scenarios:",
                      min=1, max=3, value=1, step=1, width="100%"),
          helpText(class="text-muted",
                   "1 = single scenario. 2-3 = weather variations (clear, light rain, fog).")
      )
    ),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$generateScenarios, {
      ...
      body <- list(
        ...
        limit                = as.integer(input$numScenarios),
        sensors              = sensor_cfg,
        physics              = physics_cfg
      )
    ...
    output$connectionStatus <- renderUI({
      if (rv$connected) {
        ...
        tags$div(
          tags$div(class="connection-success",
                   icon("check-circle"), " Flask server online"),
          if (isaac_up)
            tags$div(class="connection-success",
                     icon("microchip"), " Isaac Sim persistent server ready",
                     tags$br(),
                     tags$small(paste0("Ready since: ", res$isaac_sim_ready_since %||% "unknown")))
          else
            tags$div(class="connection-warning",
                     icon("exclamation-triangle"),
                     " Isaac Sim not started",
                     tags$br(),
                     tags$small("Run: C:\\isaac-sim\\python.bat isaac_sim_persistent_server.py"))
        )
      } else {
        tags$div(class="connection-error",
                 icon("times-circle"), " Not connected",
                 tags$br(),
                 tags$small("Click Test Connection to connect to Flask on localhost:5000"))
      }
    })
    ...
    output$capabilitiesStatus <- renderUI({
      ...
      tags$div(class="status-info",
               tags$b(icon("microchip"), " Confirmed Capabilities:"), tags$br(),
               tags$small(
                 paste0(sum(unlist(caps$sensors)), " sensors | ",
                        sum(unlist(caps$physics)), " physics features"), tags$br(),
                 paste0("GPU: ", caps$hardware$gpu_name,
                        " | Isaac Sim ", caps$software$isaac_sim_version), tags$br(),
                 paste0("Mode: ", caps$software$server_mode)
               ))
    })
    ...
    output$liveLog <- renderUI({
      ...
      log_div_id <- session$ns("logScroll")
      tags$div(
        tags$div(
          class = "isaac-log",
          id    = log_div_id,
    ...
    output$trajectoryMap <- renderLeaflet({
      ...
      # --- Safe blank map helper ---
      blank_map <- function(msg = "No data yet. Generate a scenario to see real GPS route.") {
        leaflet() %>%
          addProviderTiles(providers$CartoDB.Positron) %>%
          setView(lng = -0.7594, lat = 52.0406, zoom = 11) %>%
          addControl(
            html = paste0("<div style='background:rgba(255,255,255,0.92);padding:10px;",
                          "border-radius:6px;font-size:12px;border-left:4px solid #008A82;'>",
                          msg, "</div>"),
            position = "topright"
          )
      }
      ...
      if (length(rv$scenarios) == 0) return(blank_map())

      # Guard: first scenario must be a list with trajectories
      scenario <- tryCatch(rv$scenarios[[1]], error = function(e) NULL)
      if (is.null(scenario) || !is.list(scenario)) return(blank_map("Scenario data invalid."))

      trajs <- scenario$trajectories
      if (is.null(trajs) || length(trajs) == 0) return(blank_map("Scenario has no trajectory points."))

      # --- Extract vectors safely ---
      safe_num <- function(lst, key, default = 0)
        sapply(lst, function(t) { v <- t[[key]]; if (is.null(v) || length(v)==0) default else as.numeric(v[1]) })
      safe_chr <- function(lst, key, default = "")
        sapply(lst, function(t) { v <- t[[key]]; if (is.null(v) || length(v)==0) default else as.character(v[1]) })

      lats   <- safe_num(trajs, "lat")
      lons   <- safe_num(trajs, "lon")
      speeds <- safe_num(trajs, "speed_kmh")
      rtypes <- safe_chr(trajs, "road_type")
      avs    <- safe_chr(trajs, "av_readiness_status", "?")
      av_sc  <- safe_num(trajs, "av_readiness_score",  5)

      # Guard: coordinates must be valid numbers
      if (any(is.na(lats)) || any(is.na(lons))) return(blank_map("Invalid coordinates in trajectory."))
      if (length(lats) < 2)                     return(blank_map("Not enough waypoints to draw route."))

      # Guard: speed range must be non-zero for palette
      spd_range <- range(speeds, na.rm = TRUE)
      if (diff(spd_range) == 0) spd_range <- c(spd_range[1] - 1, spd_range[1] + 1)

      pal    <- colorNumeric(c("#2ecc71","#f39c12","#e74c3c"), domain = spd_range)
      ...
      tr <- function(lbl, val, bg = "") {
        ...
        paste0("<tr style='", bg_style, "'>",
               "<td style='padding:1px 6px;'><b>", lbl, "</b></td>",
               "<td style='padding:1px 6px;'>",  val,  "</td></tr>")
      }

      popups <- lapply(seq_along(trajs), function(i) {
        t   <- trajs[[i]]
        phy <- if (is.list(t$physics)) t$physics else list()
        cam <- if (is.list(t$camera))  t$camera  else list()
        lid <- if (is.list(t$lidar))   t$lidar   else list()
        ...
        if (!is.null(cam$lane_visibility))
          html <- paste0(html,
            "<tr><td colspan=2 style='color:#198754;padding-top:5px;'><b>— Camera —</b></td></tr>",
            tr("Lane vis",  paste0(round(as.numeric(cam$lane_visibility), 1), "/10")),
            tr("Sign conf", round(as.numeric(cam$sign_confidence %||% 0), 3), "#f8f9fa"))
        if (!is.null(lid$point_density))
          html <- paste0(html,
            "<tr><td colspan=2 style='color:#6f42c1;padding-top:5px;'><b>— LiDAR —</b></td></tr>",
            tr("Density",  paste0(round(as.numeric(lid$point_density), 0), " pts/m³")),
            tr("Det conf", round(as.numeric(lid$detection_confidence %||% 0), 3), "#f8f9fa"),
      ...
      tryCatch({
        leaflet() %>%
          addProviderTiles(providers$CartoDB.Positron) %>%
          addPolylines(lng = lons, lat = lats,
                       color = "#1a6eb5", weight = 4, opacity = 0.8) %>%
          ...
          addMarkers(lng = lons[1], lat = lats[1],
                     popup = paste0("<b>START</b><br>", input$queryOrigin,
                                    "<br>", round(lats[1], 5), ", ", round(lons[1], 5))) %>%
          addMarkers(lng = lons[length(lons)], lat = lats[length(lats)],
                     popup = paste0("<b>END</b><br>", input$queryDestination,
                                    "<br>", round(lats[length(lats)], 5), ", ",
                                    round(lons[length(lons)], 5))) %>%
          addLegend(pal = pal, values = speeds,
                    title = "Speed (km/h)", position = "bottomright") %>%
          fitBounds(lng1 = min(lons), lat1 = min(lats),
                    lng2 = max(lons), lat2 = max(lats))
      }, error = function(e) {
        blank_map(paste0("Map render error: ", conditionMessage(e)))
      })
    })
