...
library(htmltools)
library(tmaptools)
library(dodgr)  # Make sure this is explicitly loaded

# Source the functions file
# source("ev_route_functions.R")
...
server <- function(input, output, session) {
  ...
  # Download Road Network (Fixed Version)
    ...
    withProgress(message = 'Downloading road network...', value = 0, {
      ...
      tryCatch({
        ...
        if (is.null(bbox) || length(bbox) == 0) {
          stop("Location not found. Try 'Cambridge, UK' or 'London, UK'")
        }
        
        incProgress(0.4, detail = "Downloading OpenStreetMap data")
        
        # Download using osmdata - be specific about what we want
        ...
        # IMPORTANT: Filter to keep only LINESTRING geometries
        edges <- edges %>%
        ...
        # Keep only essential columns and remove any NA geometries
        edges <- edges %>%
        ...
        # Create the weighted streetnet with proper error handling
        graph <- tryCatch({
          ...
          # If weight_streetnet fails, try a simpler approach
          message("Trying alternative graph creation method...")
          
          # Extract coordinates
          coords_list <- st_coordinates(edges)
        ...
        output$networkStatus <- renderUI({
          div(class = "connection-error",
              ...
              tags$ul(
                ...
                tags$ul(
                  tags$li(tags$code("Cambridge, UK")),
  ...
  observeEvent(input$calculateRoute, {
    ...
    withProgress(message = 'Calculating optimal route...', value = 0, {
      ...
      tryCatch({
        ...
        # Calculate optimal path through charging points
        ...
        for (i in 1:nrow(nearest)) {
          ...
          # Calculate route through this charging point
  ...
  output$routeMap <- renderLeaflet({
    if (!values$route_calculated) {
      ...
      tryCatch({
        ...
        # Calculate and draw the actual road-based route
        if (!is.null(values$route_data$charge_point_index)) {
          ...
          withProgress(message = 'Drawing route on map...', value = 0, {
            ...
            tryCatch({
              ...
              # METHOD 1: Use dodgr_flows_aggregate to get actual route geometry
        ...
        # Fallback map with markers only
        leaflet() %>%
          addTiles() %>%
