...
route_preview_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    output$routeMap <- leaflet::renderLeaflet({
      ...
      lats <- sapply(s$trajectories, function(t) as.numeric(t$lat) %||% NA)
      lons <- sapply(s$trajectories, function(t) as.numeric(t$lon) %||% NA)
      
      # Extract sensor data from nested structure
      lane_vis <- sapply(s$trajectories, function(t) {
        if (!is.null(t$camera)) t$camera$lane_visibility %||% 5 else 5
      })
      lidar_dens <- sapply(s$trajectories, function(t) {
        if (!is.null(t$lidar)) t$lidar$point_density %||% 500 else 500
      })
      av_status <- sapply(s$trajectories, function(t) t$av_readiness_status %||% "AMBER")
      av_scores <- sapply(s$trajectories, function(t) as.numeric(t$av_readiness_score) %||% 0.5)
      ...
      # Create map (with safety checks)
      if (length(lats) == 0 || length(lons) == 0) {
        return(leaflet::leaflet() %>%
                 leaflet::addTiles() %>%
                 leaflet::setView(lng = -0.7, lat = 52.0, zoom = 8))
      }
      
      m <- leaflet::leaflet() %>%
        leaflet::setView(lng = mean(lons, na.rm = TRUE), lat = mean(lats, na.rm = TRUE), zoom = 10)
      ...
      for (i in seq_along(lats)) {
        ...
        lane_vis_val <- if (!is.null(traj$camera)) traj$camera$lane_visibility %||% 0 else 0
        sign_conf_val <- if (!is.null(traj$camera)) traj$camera$sign_confidence %||% 0 else 0
        lidar_dens_val <- if (!is.null(traj$lidar)) traj$lidar$point_density %||% 0 else 0
        lidar_conf_val <- if (!is.null(traj$lidar)) traj$lidar$detection_confidence %||% 0 else 0
        ...
        popup_html <- paste0(
          ...
          "<b>🚗 Vehicle Sensors:</b><br>",
          "<table style='width:100%; font-size:12px;'>"
        )
        
        # Add camera data if available
        if (!is.null(traj$camera)) {
          popup_html <- paste0(popup_html,
            "<tr><td>📷 Lane Visibility:</td><td><b>", round(lane_vis_val * 100, 1), "%</b></td></tr>",
            "<tr><td>📷 Sign Detection:</td><td><b>", round(sign_conf_val * 100, 1), "%</b></td></tr>"
          )
        }
        
        # Add LiDAR data if available
        if (!is.null(traj$lidar)) {
          popup_html <- paste0(popup_html,
            "<tr><td>📡 LiDAR Density:</td><td><b>", round(lidar_dens_val, 0), " pts/m²</b></td></tr>",
            "<tr><td>📡 LiDAR Confidence:</td><td><b>", round(lidar_conf_val * 100, 1), "%</b></td></tr>"
          )
        }
        ...
        popup_html <- paste0(popup_html,
          ...
          ifelse(traj$av_readiness_status == "READY", "#28a745",
                 ifelse(traj$av_readiness_status == "AMBER", "#ffc107", "#dc3545")),
          "; font-weight:bold; font-size:14px;'>", traj$av_readiness_status, "</span><br>",
          "<b>Score:</b> ", round(as.numeric(traj$av_readiness_score) * 10, 1), "/10<br>",
          
          "<hr style='margin:5px 0;'>",
          "<b>🛣️ Road Type:</b> ", traj$road_type %||% "unknown", "<br>",
          "<b>⚡ Speed:</b> ", traj$speed_kmh %||% 0, " km/h<br>",
          "<b>📍 GPS:</b> ", round(as.numeric(traj$lat), 5), ", ", round(as.numeric(traj$lon), 5),
          "</div>"
        )
