...
ui <- dashboardPage(
  ...
  dashboardBody(
    tabItems(
      # GIS Systems Tab with Plotly Maps (NO TERRA)
      tabItem(tabName = "gis",
              fluidRow(
                box(
              ...
              fluidRow(
                box(
                  ...
                  plotlyOutput("gis_map", height = "500px") %>% withSpinner(color = "#008A82")
                ),
...
server <- function(input, output) {
  ...
  # GIS Tab Server Logic - Using Plotly maps (NO TERRA)
  output$gis_map <- renderPlotly({
    # Create map data based on controls
    map_data <- switch(input$gis_layer,
                       "5g" = data.frame(
                         lng = c(-0.1, -2.2, -1.5, -3.2, -4.2, -1.8, -2.9),
                         lat = c(51.5, 53.5, 52.5, 55.9, 50.4, 51.8, 52.2),
                         location = c("London 5G Hub", "Manchester Node", "Birmingham Centre", "Edinburgh Link", "Plymouth Station", "Oxford Hub", "Cardiff Centre"),
                       ...
                       "charging" = data.frame(
                         ...
                         location = c("London Charging Hub", "Manchester Rapid", "Birmingham Fast", "Edinburgh Network", "Plymouth Slow", "Canterbury Hub", "Newcastle Rapid", "Swansea Network"),
                         coverage = c(450, 280, 320, 180, 95, 120, 150, 85),
                         color = c("#1ABC9C", "#3498DB", "#9B59B6", "#E67E22", "#E74C3C", "#2ECC71", "#F39C12", "#E74C3C")
                       ),
                       "routes" = data.frame(
                         lng = c(-0.1, -2.2, -1.5, -3.2, -4.2),
                         lat = c(51.5, 53.5, 52.5, 55.9, 50.4),
                         location = c("M25 Corridor", "M62 Route", "M42 Network", "A90 Highway", "A38 Corridor"),
                         coverage = c(88, 75, 72, 68, 60),
                         color = c("#2ECC71", "#F39C12", "#E67E22", "#E74C3C", "#E74C3C")
                       )
    )
    
    # Set map bounds based on region
    if(input$gis_region == "london") {
      map_data <- map_data[map_data$lng > -0.5 & map_data$lng < 0.3 & map_data$lat > 51.3 & map_data$lat < 51.7, ]
    } else if(input$gis_region == "manchester") {
      map_data <- map_data[map_data$lng > -2.5 & map_data$lng < -1.9 & map_data$lat > 53.3 & map_data$lat < 53.7, ]
    } else if(input$gis_region == "scotland") {
      map_data <- map_data[map_data$lat > 55.0, ]
    }
    ...
    fig <- plot_ly(
      ...
      text = ~paste(location, "<br>", input$gis_layer, ":", coverage),
      hoverinfo = "text"
    ) %>%
      layout(
        mapbox = list(
          ...
          center = list(
            lon = switch(input$gis_region, 
                         "all" = -2.5, "london" = -0.1, 
                         "manchester" = -2.2, "scotland" = -4.0),
            lat = switch(input$gis_region, 
                         "all" = 53.4, "london" = 51.5, 
                         "manchester" = 53.5, "scotland" = 56.5)
          ),
          zoom = switch(input$gis_region, 
                        "all" = 5, "london" = 9, 
                        "manchester" = 8, "scotland" = 6)
        ),
  ...
  output$gis_coverage_text <- renderText({
    coverage_val <- switch(input$gis_layer,
                           "5g" = switch(input$gis_region, "all" = "84%", "london" = "95%", "manchester" = "85%
                           ", "scotland" = "72%"),
  ...
  output$ev_charging_analysis <- renderPlotly({
    ...
    station_data <- switch(input$ev_vehicle_type,
                           "all" = data.frame(
                             types = station_types,
                           ...
                           "cars" = data.frame(
                             types = station_types,
                             count = c(7200, 13800, 20500, 2800),
                             utilization = c(72, 68, 48, 82),
                             stringsAsFactors = FALSE
                           ),
                           "vans" = data.frame(
                             types = station_types,
                             count = c(1100, 1200, 1300, 350),
                             utilization = c(85, 58, 38, 92),
                             stringsAsFactors = FALSE
                           ),
                           "buses" = data.frame(
                             types = station_types,
                             count = c(200, 200, 200, 50),
                             utilization = c(95, 78, 32, 98),
                             stringsAsFactors = FALSE
                           )
    )
  ...
  output$safety_incidents_chart <- renderPlotly({
    incident_data <- switch(input$safety_vehicle_class,
                            "all" = data.frame(
                            ...
                            "cars" = data.frame(
                              types = c("Near Miss", "System Alert", "Weather Event", "Infrastructure", "User Error"),
                              count = c(12, 28, 6, 3, 15),
                              severity = c(2, 2, 3, 5, 4),
                              stringsAsFactors = FALSE
                            ),
                            "commercial" = data.frame(
                              types = c("Near Miss", "System Alert", "Weather Event", "Infrastructure", "User Error"),
                              count = c(18, 35, 12, 8, 8),
                              severity = c(4, 3, 5, 7, 2),
                              stringsAsFactors = FALSE
                            ),
                            "public" = data.frame(
                              types = c("Near Miss", "System Alert", "Weather Event", "Infrastructure", "User Error"),
                              count = c(8, 22, 4, 2, 3),
                              severity = c(2, 1, 3, 4, 2),
                              stringsAsFactors = FALSE
                            )
    )
  ...
  output$safety_compliance_chart <- renderPlotly({
    ...
    compliance_data <- switch(input$safety_vehicle_class,
                              "all" = c(98, 95, 99, 97, 94),
                              "cars" = c(99, 96, 99, 98, 95),
                              "commercial" = c(97, 93, 98, 96, 92),
                              "public" = c(100, 98, 100, 99, 97)
    )
  ...
  output$digital_coverage <- renderPlotly({
    ...
    coverage_data <- switch(input$digital_region_type,
                            "all" = switch(input$digital_technology,
                                           "5g" = data.frame(tech = technologies, coverage = c(95, 84, 65, 88), stringsAsFactors = FALSE),
                                           "edge" = data.frame(tech = technologies, coverage = c(92, 78, 72, 85), stringsAsFactors = FALSE),
                                           "twins" = data.frame(tech = technologies, coverage = c(88, 75, 68, 82), stringsAsFactors = FALSE)
                            ),
                            "urban" = switch(input$digital_technology,
                                             "5g" = data.frame(tech = technologies, coverage = c(98, 92, 85, 95), stringsAsFactors = FALSE),
                                             "edge" = data.frame(tech = technologies, coverage = c(95, 88, 82, 92), stringsAsFactors = FALSE),
                                             "twins" = data.frame(tech = technologies, coverage = c(92, 85, 78, 88), stringsAsFactors = FALSE)
                            ),
                            "rural" = switch(input$digital_technology,
                                             "5g" = data.frame(tech = technologies, coverage = c(87, 45, 32, 65), stringsAsFactors = FALSE),
                                             "edge" = data.frame(tech = technologies, coverage = c(82, 38, 28, 58), stringsAsFactors = FALSE),
                                             "twins" = data.frame(tech = technologies, coverage = c(78, 35, 25, 52), stringsAsFactors = FALSE)
                            ),
                            "highways" = switch(input$digital_technology,
                                                "5g" = data.frame(tech = technologies, coverage = c(92, 78, 55, 82), stringsAsFactors = FALSE),
                                                "edge" = data.frame(tech = technologies, coverage = c(88, 72, 48, 78), stringsAsFactors = FALSE),
                                                "twins" = data.frame(tech = technologies, coverage = c(85, 68, 45, 75), stringsAsFactors = FALSE)
                            )
    )
    
    p <- ggplot(coverage_data, aes(x = tech, y = coverage, fill = tech)) +
      geom_col() +
  ...
  output$digital_twin_maturity <- renderPlotly({
    ...
    maturity_data <- switch(input$digital_technology,
                            "5g" = data.frame(
                              systems = systems,
                            ...
                            "edge" = data.frame(
                              systems = systems,
                              digital_maturity = c(78, 65, 82, 71, 59),
                              integration = c(82, 68, 85, 75, 62),
                              stringsAsFactors = FALSE
                            ),
                            "twins" = data.frame(
                              systems = systems,
                              digital_maturity = c(92, 85, 95, 88, 78),
                              integration = c(95, 88, 98, 92, 82),
                              stringsAsFactors = FALSE
                            )
    )
