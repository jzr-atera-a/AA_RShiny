      ...
      tabItem(
        ...
        fluidRow(
          box(
            ...
            solidHeader = TRUE,
            width = 12,
            p(strong("Instructions:"), "Upload up to 5 receipt images in JPG or JPEG format. The app will extract provider name, amount, date, and description."),
            hr(),
            fileInput(
              "receipt_files",
      ...
      tabItem(
        ...
        fluidRow(
          box(
            ...
            hr(),
            p(strong("Edit the table below:"), "Click on any cell in the category columns (columns 8-13) to edit. First 7 columns are read-only."),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$process_btn, {
    ...
    withProgress(message = 'Processing receipts...', value = 0, {
      ...
      for (i in 1:nrow(input$receipt_files)) {
        ...
        # Copy file to receipts folder with ID-prefixed filename
  ...
  output$categorize_table <- DT::renderDataTable({
    ...
    # Columns 0-6 are read-only (receipt data)
    # Columns 7-12 are editable (category checkboxes: 0 or 1)
    DT::datatable(
      rv$categorize_data,
      editable = list(
        target = 'cell', 
      ...
      options = list(
        ...
        scrollX = TRUE,
        columnDefs = list(
          list(targets = 7:12, className = 'dt-center')  # Center the category columns
        ),
        order = list(list(6, 'desc'))  # Sort by timestamp descending
      ),
      rownames = FALSE
  ...
  observeEvent(input$categorize_table_cell_edit, {
    ...
    # Category columns are 8-13 (Labour, Overheads, Materials, Capital_Usage, TS, Contractor)
    if (col >= 8 && col <= 13) {
      ...
      if (value == 1) {
        rv$categorize_data[row, col] <- 1L
      }
      # Recalculate totals
      calculate_category_totals()
    } else {
      # For other editable columns, just update the value
      rv$categorize_data[row, col] <- value
    }
  })
