      ...
      tabItem(tabName = "viewer3d",
              ...
              fluidRow(
                box(
                  ...
                  fluidRow(
                    column(12,
                           actionButton("convert_and_save_gcode",
...
server <- function(input, output, session) {
  ...
  # Parse/Convert STL to G-code and SAVE with file dialog
  observeEvent(input$convert_and_save_gcode, {
    req(stl_rv$model_loaded, stl_rv$model_path)
    
    if(!processing_rv$is_validated) {
      showNotification("Please validate the model first!", type = "warning", duration = 3)
      return()
    }
    
    add_log("Starting STL to G-code conversion...")
    showNotification("Converting STL to G-code... Please wait.", type = "message", duration = 5)
...
", 
      ...
      temp_gcode <- tempfile(fileext = ".gcode")
      writeLines(gcode_content, temp_gcode)
      
      # Nombre sugerido para guardar
      suggested_name <- paste0(tools::file_path_sans_ext(basename(stl_rv$model_path)), 
                              "_converted.gcode")
      
      # SHINY FILE DOWNLOAD - Usuario elige dónde guardar
      output$download_gcode <- downloadHandler(
        filename = function() { suggested_name },
        content = function(file) {
          file.copy(temp_gcode, file)
          add_log(paste("G-code saved to user-selected location"))
        }
      )
      ...
      file.copy(temp_gcode, local_path, overwrite = TRUE)
      ...
      # Trigger download
      session$sendCustomMessage("triggerDownload", "download_gcode")
      
      showNotification(paste("✓ G-code generated!", 
                            "\nSaved to:", output_dir,
                            "\nFile:", local_filename,
                            "\n\nGo to Print Control tab to select and print."), 
                      type = "message", duration = 10)
      add_log(paste("G-code conversion completed, saved to:", local_path))
