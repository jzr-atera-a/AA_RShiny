...
# R Shiny Application for Windows USB Connection

library(shiny)
library(shinydashboard)
library(serial)  # For serial/USB communication
      ...
      tabItem(tabName = "connection",
              fluidRow(
                box(
                  ...
                  fluidRow(
                    column(4,
                           selectInput("com_port", "COM Port:",
                                       choices = c("Scan for ports..." = ""),
                                       width = "100%")
                    ),
                    column(4,
                           selectInput("baud_rate", "Baud Rate:",
                                       choices = c("115200", "250000", "9600"),
                                       selected = "115200",
                                       width = "100%")
                    ),
                    column(4,
                           br(),
                           actionButton("scan_ports", "Scan Ports", 
                                        icon = icon("search"),
                                        class = "btn-info",
                                        width = "100%")
                    )
              ...
              fluidRow(
                ...
                box(
                  ...
                  actionButton("get_temp", "Get Temperature",
                               ...
                               class = "btn-info",
                               style = "margin: 5px;"),
                  actionButton("get_position", "Get Position",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$scan_ports, {
    add_log("Scanning for available COM ports...")
    
    tryCatch({
      # Windows-specific: scan COM1 to COM20
      available_ports <- character(0)
      
      for(i in 1:20) {
        port_name <- paste0("COM", i)
        test_result <- tryCatch({
          # Try to list the port
          system(paste0("mode ", port_name), intern = TRUE, show.output.on.console = FALSE)
          TRUE
        }, error = function(e) {
          FALSE
        })
        ...
        if(test_result) {
          available_ports <- c(available_ports, port_name)
        }
      }
      
      if(length(available_ports) > 0) {
        updateSelectInput(session, "com_port",
                          choices = available_ports,
                          selected = available_ports[1])
        add_log(paste("Found", length(available_ports), "COM port(s)"))
        showNotification(paste("Found", length(available_ports), "COM port(s)"),
                         type = "message")
      } else {
        showNotification("No COM ports found. Please check printer connection.",
                         type = "warning")
        add_log("No COM ports found")
      }
    }, error = function(e) {
      showNotification(paste("Error scanning ports:", e$message),
                       type = "error")
      add_log(paste("Error scanning ports:", e$message))
    })
  })
  ...
  observeEvent(input$connect_btn, {
    ...
    add_log(paste("Attempting connection to", input$com_port, "at", input$baud_rate, "baud"))
    
    tryCatch({
      # Note: actual serial connection would use the 'serial' package
      # This is a placeholder for demonstration
      # In production, you would use: serialConnection(port, baud, ...)
      ...
      # Send initial commands to get printer info
      add_log("Requesting printer information...")
      ...
      showNotification(paste("Connection failed:", e$message),
                       type = "error")
      add_log(paste("Connection failed:", e$message))
    })
  })
  ...
  observeEvent(input$disconnect_btn, {
    if(rv$connected) {
      ...
      tryCatch({
        ...
        if(!is.null(rv$serial_conn)) {
          # close(rv$serial_conn)
          rv$serial_conn <- NULL
        }
        ...
        showNotification(paste("Error disconnecting:", e$message),
                         type = "error")
        add_log(paste("Error disconnecting:", e$message))
      })
    }
  })
  ...
  output$connection_message <- renderUI({
    if(rv$connected) {
      ...
      div(class = "status-info",
          icon("info-circle"),
          " Please select a COM port and click Connect")
    }
  })
  ...
  output$printer_info <- renderText({
    if(rv$connected) {
      paste(
        ...
        "Firmware: 1.0.0.0 (simulated)",
        "Build Volume: 256 x 256 x 256 mm",
        "Connection: USB Serial",
        paste("COM Port:", input$com_port),
        paste("Baud Rate:", input$baud_rate),
        sep = "\n"
      )
    } else {
      "Not connected to printer.\nPlease connect to view printer information."
    }
  })
