  ...
  observeEvent(input$connect_btn, {
    ...
    tryCatch({
      ...
      if(json_found && !is.null(response$status) && response$status == "connected") {
        ...
        if(grepl("11001|getaddrinfo|Name or service", paste(result, collapse = " "))) {
          error_msg <- paste(
            "Network Error: Cannot reach printer at", input$printer_ip,
            "\n\nCommon fixes:",
            "\n• Verify printer IP (Settings → Network on printer)",
            "\n• Check printer is ON and WiFi connected",
            "\n• Ensure computer and printer on same network",
            "\n• Try: ping", input$printer_ip
          )
        } else if(grepl("ModuleNotFoundError.*paho", paste(result, collapse = " "))) {
          error_msg <- "paho-mqtt not installed. Run: python -m pip install paho-mqtt"
        } else if(grepl("timeout|timed out", paste(result, collapse = " "), ignore.case = TRUE)) {
          error_msg <- paste(
            "Connection timeout. Printer not responding.",
            "\n\nCheck:",
            "\n• Printer is ON and not sleeping",
            "\n• WiFi is connected on printer",
            "\n• Firewall not blocking port 8883"
          )
        } else if(grepl("authentication|credentials", paste(result, collapse = " "), ignore.case = TRUE)) {
          error_msg <- "Authentication failed. Check Access Code (must be exactly 8 digits)"
        }
