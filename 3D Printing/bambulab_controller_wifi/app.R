...
server <- function(input, output, session) {
  ...
  observeEvent(input$connect_btn, {
    ...
    tryCatch({
      ...
      python_cmd <- paste(
        "python bambulab_mqtt.py connect",
        input$printer_ip,
        input$access_code,
        input$serial_number
      )
      ...
      # Parse JSON response
      response <- fromJSON(paste(result, collapse = "\n"))
      
      if(!is.null(response$status) && response$status == "connected") {
        rv$connected <- TRUE
        ...
        showNotification("Connection failed. Check IP, Access Code, and Serial Number.",
                         type = "error")
