# global.R - DriveSafe Suite
# Two sidebar groups: API Configuration + DriveSafe - Driver Health
# =========================================================

cat("\n+--------------------------------------------------+\n")
cat("|  DRIVESAFE SUITE - INITIALISING                  |\n")
cat("|  API Configuration + Driver Health Module        |\n")
cat("+--------------------------------------------------+\n\n")
...
create_ui <- function(module_loader) {
  ...
  # Build grouped collapsible sidebar
  ...
  dashboardPage(
    ...
    dashboardHeader(
      title = tags$span(
        tags$img(src = "css/logo_placeholder.svg",
                 height = "28px", style = "margin-right:8px; vertical-align:middle;"),
        registry$app$name %||% "DriveSafe Suite"
      ),
      titleWidth = 310
    ),
    ...
    dashboardSidebar(
      ...
      tags$div(
        ...
        tags$strong("DriveSafe Suite"), tags$br(),
        "HED + ATERA Analytics", tags$br(),
