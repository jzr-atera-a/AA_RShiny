# R/utils_api.R  -  DriveSafe Suite APIManager
# Shared Claude API + BigQuery (tables: driver_health_log)
...
APIManager <- R6::R6Class(
  ...
  public = list(
    ...
    # ── DriveSafe table ──────────────────────────────────────
    bq_table_driver        = "driver_health_log",
    ...
    bq_table_schedule      = "driver_health_log",
    bq_table_events        = "driver_health_log",
    bq_table_funding       = "driver_health_log",
    ...
    initialize = function() {
      ...
      cat("API Manager initialised (DriveSafe Suite)\n")
    },
    ...
    authenticate_bigquery = function(json_path = NULL, json_text = NULL) {
      tryCatch({
        ...
        # Create driver_health_log if it doesn't exist
        tryCatch({
          ...
          cat("BigQuery: driver_health_log table ready\n")
        }, error = function(e) {
          cat("BigQuery: driver_health_log CREATE check failed:", e$message, "\n")
        })
