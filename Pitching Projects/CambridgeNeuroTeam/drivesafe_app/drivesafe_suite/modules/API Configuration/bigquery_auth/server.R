...
# DriveSafe Suite - BigQuery auth server
        ...
        if (nrow(dat) == 0) {
          output$test_status <- renderUI({
            div(class = "status-warning", icon("exclamation-triangle"),
                " Query succeeded but driver_health_log is empty (no rows yet).")
          })
          ...
          output$test_status <- renderUI({
            div(class = "status-success", icon("check-circle"),
                sprintf(" Retrieved %d row(s) from driver_health_log.", nrow(dat)))
          })
          output$test_table <- DT::renderDataTable({
            DT::datatable(dat,
