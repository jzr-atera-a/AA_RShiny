  ...
  moduleServer(id, function(input, output, session) {
      ...
      tryCatch({
        ...
        output$deParsedPreview <- renderText({
          paste0(
            ...
            "Raison d'Être: ", substr(parsed_de_canvas()$raison_detre, 1, 100), "...\n",
            "Initial Market: ", substr(parsed_de_canvas()$initial_market, 1, 100), "..."
          )
        })
    ...
    observeEvent(input$submitDECanvas, {
      ...
      tryCatch({
        ...
        de_table_id <- paste0(api_manager$bq_project_id, ".", api_manager$bq_dataset_id, ".disciplined_entrepreneurship_canvas")
        ...
        VALUES ('%s', CURRENT_TIMESTAMP(), CURRENT_TIMESTAMP(), '%s', '%s', '%s',
                '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
                                de_table_id, canvas_id_clean,
                                gsub("'", "\\\\'", parsed_de_canvas()$business_area),
                                gsub("'", "\\\\'", parsed_de_canvas()$project),
                                gsub("'", "\\\\'", parsed_de_canvas()$business_focus),
                                gsub("'", "\\\\'", parsed_de_canvas()$raison_detre),
                                gsub("'", "\\\\'", parsed_de_canvas()$initial_market),
                                gsub("'", "\\\\'", parsed_de_canvas()$value_creation),
                                gsub("'", "\\\\'", parsed_de_canvas()$competitive_advantage),
                                gsub("'", "\\\\'", parsed_de_canvas()$customer_acquisition),
                                gsub("'", "\\\\'", parsed_de_canvas()$product_unit_economics),
                                gsub("'", "\\\\'", parsed_de_canvas()$sales),
                                gsub("'", "\\\\'", parsed_de_canvas()$overall_economics),
                                gsub("'", "\\\\'", parsed_de_canvas()$design_build),
                                gsub("'", "\\\\'", parsed_de_canvas()$scaling)
        )
