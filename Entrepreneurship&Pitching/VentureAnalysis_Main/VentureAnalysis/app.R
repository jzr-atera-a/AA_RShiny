# app.R: Venture Analysis by Atera Analytics v4.8
...
ui <- dashboardPage(
  skin = "black",
  dashboardHeader(title = "Venture Analysis · Atera Analytics"),
  dashboardSidebar(
    tags$div(class = "sidebar-book-badge",
             tags$div(class = "book-chip",    "VENTURE ANALYSIS · ATERA"),
             tags$div(class = "book-authors", "Atera Analytics"),
             tags$div(class = "book-pub",     "DeepTech Fundraising Intelligence")),
    sidebarMenu(id = "tabs",
                menuItem("\U0001f4cb Introduction",  tabName = "introduction",         icon = icon("star")),
  ...
  dashboardBody(
    ...
    div(id = "login_overlay", class = "login-overlay", style = "display:flex;",
        div(class = "login-card",
            ...
            actionButton("login_submit", "\U0001f510 Login", class = "login-btn"),
            uiOutput("login_error"),
            div(class = "login-footer",
                "Access restricted to authorised users only.", tags$br(), tags$br(),
                "Would you like to request access? Drop us a message via:", tags$br(),
                tags$a(
                  href   = "https://atera-analytics.co.uk/contact/",
                  target = "_blank",
                  rel    = "noopener noreferrer",
                  style  = "color:#00e5ff;font-weight:700;text-decoration:underline;",
                  "atera-analytics.co.uk/contact"
                ),
                tags$br(),
                tags$span(style = "font-size:10px;color:rgba(173,200,255,0.50);",
                  "Select General Enquiries in the contact form.")
            )
        )
    ),
    ...
    tabItems(
      tabItem(tabName = "introduction",         introduction_ui("introduction")),
...
server <- function(input, output, session) {
  ...
  introduction_server("introduction")
  guide_server("guide")
