    ...
    tags$head(
      ...
      tags$script(HTML("
        ...
        function stopAnalysis() {
          ...
          // Don't set analysisActive to false - keep the audio context alive
        }
        
        function resumeAnalysis() {
          if (analysisActive && !animationId) {
            analyzeAudio();
          }
        }
    ...
    if (audio) {
      if (data.action === 'play') {
        audio.play().then(function() {
          // Resume analysis when play is successful
          if (typeof resumeAnalysis === 'function') {
            resumeAnalysis();
          }
        }).catch(function(e) { console.log('Play failed:', e); });
      } else if (data.action === 'pause') {
        audio.pause();
        // Analysis will be paused by the audio event listener
      } else if (data.action === 'stop') {
        audio.pause();
        audio.currentTime = 0;
        // Analysis will be stopped by the audio event listener
      }
    }
  });
"))
