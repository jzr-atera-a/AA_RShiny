    ...
    fluidRow(
      box(
        ...
        p("Paste text produced by the Generate Diagram tab (or written by hand in the same format). ",
          "Parse it first to preview the components, then upload. Diagram type, category, topic, and title ",
          "are read directly from the pasted text's metadata lines - no need to re-select them here."),

        div(class = "alert alert-info",
            tags$strong("Expected Format:"),
            tags$ul(
              tags$li("First 4 lines (metadata, no field names): [diagram_type]", tags$br(),
                      "[category]", tags$br(), "[topic]", tags$br(), "[title]"),
