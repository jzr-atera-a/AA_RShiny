...
DIAGRAM_TYPES <- c(
  "grid", "quadrant", "table", "stacked_list", "linear_flow",
  "radial", "network",
  "line_chart", "bar_chart", "funnel", "dual_axis_timeseries"
)

DIAGRAM_TYPE_LABELS <- c(
  grid = "Grid (e.g. SWOT, PESTEL)",
  quadrant = "2x2 Quadrant (e.g. VUCA, Value Destruction)",
  table = "Matrix Table (e.g. TOWS, VRIO)",
  stacked_list = "Stacked List (e.g. Purpose of Strategy)",
  linear_flow = "Linear Flow (e.g. Frame-Diagnose-Choose)",
  radial = "Radial / Cyclic Wheel (e.g. Strategic Planning Wheel)",
  network = "Network / Hub-and-Spoke (e.g. Five Forces)",
  line_chart = "Line Chart (e.g. Disruptive Innovation curve)",
  bar_chart = "Bar Chart (e.g. Trend to Commoditization)",
  funnel = "Funnel (e.g. Economic Profit Mobility)",
  dual_axis_timeseries = "Dual-Axis Timeseries (e.g. Active Waiting)"
)
...
# ---- Per-diagram-type instruction blocks - preloaded, never typed by the
#     user, always prepended to their free-text prompt. Each block spells
#     out exactly which fields to fill for that structural family, the
#     delimiter rule, and one worked example row set so Claude's output is
#     directly copy-parseable by parse_diagram_text(). --------------------
diagram_type_instructions <- function(diagram_type, category, topic, title_hint) {
  sep <- DIAG_ITEM_SEP
  ...
  common_header <- paste0(
    ...
    '1. Start with EXACTLY 4 metadata lines, each on its own line. Each line must contain ONLY the actual ',
    'value wrapped in single square brackets - do NOT include a field name, label, or colon inside or around ',
    'the brackets. Lines 1-3 must be EXACTLY these values (they are fixed by the user\'s selection, not yours ',
    'to change); line 4 is the diagram title as instructed below. The first 4 lines of your entire response ',
    'must be EXACTLY:\n',
    '[', diagram_type, ']\n[', category, ']\n[', topic, ']\n', title_line, '\n\n',
  ...
  type_block <- switch(diagram_type,
    "grid" = paste0(
      'DIAGRAM_TYPE = grid (e.g. SWOT, PESTEL). Structure: N equal columns (grid_col = 1..N), each column ',
      'is one [component_type]: shape_rectangle header row plus its bullets packed into [items_packed].\n\n',
      'For EACH column, output ONE block:\n',
      '[component_type]: shape_rectangle\n[layout_role]: grid_cell\n[grid_row]: 1\n[grid_col]: <1..N>\n',
      '[sequence_order]: <same as grid_col>\n[label_text]: <short header, e.g. "STRENGTHS">\n',
    ...
    "quadrant" = paste0(
      'DIAGRAM_TYPE = quadrant (2x2). Output exactly 4 blocks, one per quadrant, plus one title block and ',
      'two axis-label blocks.\n\nFor EACH quadrant:\n',
      '[component_type]: region_quadrant\n[layout_role]: quadrant_region\n',
      '[quadrant_position]: <top_left|top_right|bottom_left|bottom_right>\n',
      '[label_text]: <quadrant name>\n[sub_text]: <1-sentence definition>\n',
      '[items_packed]: <optional bullet 1>', sep, '<optional bullet 2> (N/A if none)\n',
      '[color_hint]: <distinct hint per quadrant>\n\n',
      'Axis blocks (exactly 2):\n[component_type]: axis_label\n[layout_role]: axis\n',
      '[value_axis]: <x|y>\n[label_text]: <axis question/label, e.g. "How much do you know about the situation?">\n\n',
      'Title block:\n[component_type]: title\n[layout_role]: title\n[label_text]: <diagram title>\n'
    ),
    "table" = paste0(
      'DIAGRAM_TYPE = table (2D matrix, e.g. TOWS, VRIO). Output one block per cell using grid_row/grid_col ',
      '(1-indexed) for BOTH header cells and content cells.\n\nFor EACH cell:\n',
    ...
    "stacked_list" = paste0(
      'DIAGRAM_TYPE = stacked_list. A vertical stack of N items, sequence_order = 1..N top to bottom.\n\n',
      'For EACH item:\n[component_type]: bullet_list_item\n[layout_role]: list_item\n',
      '[sequence_order]: <1..N>\n[label_text]: <short bold phrase>\n',
      '[sub_text]: <optional definition line, else N/A>\n[color_hint]: <accent color, can repeat across items>\n\n',
      'Title block:\n[component_type]: title\n[layout_role]: title\n[label_text]: <diagram title>\n'
    ),
    "linear_flow" = paste0(
      'DIAGRAM_TYPE = linear_flow. A horizontal left-to-right sequence of N boxes, sequence_order = 1..N.\n\n',
      'For EACH box:\n[component_type]: shape_rectangle\n[layout_role]: node\n',
      '[sequence_order]: <1..N>\n[label_text]: <box header>\n',
      '[sub_text]: <guiding question or subtitle, else N/A>\n',
      '[items_packed]: <sub-item 1>', sep, '<sub-item 2> (N/A if none)\n\n',
      'Title block:\n[component_type]: title\n[layout_role]: title\n[label_text]: <diagram title>\n'
    ),
    "radial" = paste0(
      'DIAGRAM_TYPE = radial (cyclic wheel, e.g. Strategic Planning wheel). N segments arranged in a circle, ',
      'sequence_order = 1..N clockwise, plus optional concentric center rings.\n\n',
      'For EACH outer segment:\n[component_type]: region_quadrant\n[layout_role]: node\n',
      '[sequence_order]: <1..N clockwise>\n[label_text]: <segment guiding question>\n',
      '[sub_text]: <1-line descriptor>\n[color_hint]: <accent color>\n\n',
      'For EACH center ring (innermost = highest sequence_order, e.g. N+1, N+2):\n',
      '[component_type]: shape_circle\n[layout_role]: node\n[sequence_order]: <N+1, N+2, ...>\n',
      '[label_text]: <ring label>\n\n',
      'Title block:\n[component_type]: title\n[layout_role]: title\n[label_text]: <diagram title>\n'
    ),
    "network" = paste0(
      'DIAGRAM_TYPE = network (hub-and-spoke, e.g. Five Forces). One central node plus N satellite nodes.\n\n',
      'Central node (exactly one):\n[component_type]: shape_circle\n[layout_role]: node\n',
      '[sequence_order]: 0\n[label_text]: <central node name>\n\n',
      'For EACH satellite node:\n[component_type]: shape_rectangle\n[layout_role]: node\n',
      '[sequence_order]: <1..N>\n[label_text]: <satellite node name>\n',
      '[sub_text]: <1-line description of the relationship, else N/A>\n\n',
      'Title block:\n[component_type]: title\n[layout_role]: title\n[label_text]: <diagram title>\n'
    ),
    "line_chart" = paste0(
      'DIAGRAM_TYPE = line_chart. One or more named series, each a sequence of (x,y) data points.\n\n',
      'Axis blocks (exactly 2):\n[component_type]: axis\n[layout_role]: axis\n',
      '[value_axis]: <x|y>\n[metric_name]: <axis name>\n[axis_type]: <linear|log|categorical|time>\n\n',
      'For EACH data point:\n[component_type]: data_point\n[layout_role]: node\n',
      '[series_name]: <which line this belongs to>\n[sequence_order]: <point order along x>\n',
      '[value_axis]: x\n[value_numeric]: <x value>\n\n(repeat the same sequence_order/series_name with [value_axis]: y and the y value as a SEPARATE block)\n\n',
      'Title block:\n[component_type]: title\n[layout_role]: title\n[label_text]: <diagram title>\n'
    ),
    "bar_chart" = paste0(
      'DIAGRAM_TYPE = bar_chart. Categories on X, one or more series (clustered bars) on Y.\n\n',
      'For EACH bar:\n[component_type]: data_point\n[layout_role]: node\n',
      '[series_name]: <cluster/series this bar belongs to>\n[label_text]: <category name, X-axis tick>\n',
      '[value_numeric]: <bar value>\n[unit_label]: <units, e.g. "%">\n[color_hint]: <one per series>\n\n',
      'Title block:\n[component_type]: title\n[layout_role]: title\n[label_text]: <diagram title>\n'
    ),
    "funnel" = paste0(
      'DIAGRAM_TYPE = funnel. Ordered stages, sequence_order = 1..N, each with a value.\n\n',
      'For EACH stage:\n[component_type]: data_point\n[layout_role]: node\n',
    ...
    "dual_axis_timeseries" = paste0(
      'DIAGRAM_TYPE = dual_axis_timeseries. Two diverging series (e.g. opportunity vs threat) over time.\n\n',
      'Axis blocks (exactly 2):\n[component_type]: axis\n[layout_role]: axis\n',
      '[value_axis]: <x|y>\n[metric_name]: <axis name>\n\n',
      'For EACH data point:\n[component_type]: data_point\n[layout_role]: node\n',
      '[series_name]: <series this belongs to, e.g. "Opportunity" or "Threat">\n',
      '[sequence_order]: <point order along x>\n[value_numeric]: <y value, signed>\n',
      '[label_text]: <callout label if this is a spike/inflection point, else N/A>\n\n',
      'Title block:\n[component_type]: title\n[layout_role]: title\n[label_text]: <diagram title>\n'
    ),
    stop("Unknown diagram_type: ", diagram_type)
  )

  paste0(common_header, type_block)
}

generate_diagram_prompt <- function(diagram_type, category, topic, title_hint, user_request) {
  type_instructions <- diagram_type_instructions(diagram_type, category, topic, title_hint)

  paste0(
    'You are a strategy consultant building a "', DIAGRAM_TYPE_LABELS[[diagram_type]], '" diagram. ',
    ...
    'Now generate the diagram, beginning with the 4 metadata lines.'
  )
}

# After generation, force-overwrite the metadata block's first 3 positional
# bare-bracket lines (diagram_type/category/topic - fixed by the user's
# dropdown selection, never Claude's to change) back to the authoritative
# values, regardless of what Claude actually wrote. The 4th line (title) is
# only forced when the user supplied an explicit title_hint - otherwise
# Claude's own invented title (per the instruction in diagram_type_instructions)
# is left untouched. Mirrors the positional metadata convention used by
# Flex Table (parse_table_text) and Mind Map/Knowledge Graph above, rather
# than the old (and never actually emitted) "[field]: value" tag search.
overwrite_diagram_header <- function(text, diagram_type, category, topic, title_override = NULL) {
  lines <- strsplit(text, "\n")[[1]]
  metadata_line_idx <- which(grepl("^\\[.+\\]$", trimws(lines)))
  metadata_line_idx <- head(metadata_line_idx, 4)

  values <- list(diagram_type, category, topic, title_override)
...
#
# Part 1 - METADATA: the first 4 bare "[value]" lines (no field name, no
# colon - matching Flex Table's positional metadata convention and Mind
# Map/Knowledge Graph's "4 metadata lines" convention) give diagram_type/
# category/topic/title. This means Bulk Import and any hand-pasted text
# carry their own classification - no separate dropdowns needed there,
# same as every other suite in this app.
#
...
parse_diagram_text <- function(text, diagram_id, is_template = FALSE, source_diagram_id = NA, created_by = "claude_agent") {
  ...
  # ---- Part 1: metadata block (diagram_type, category, topic, title) ----
  diagram_type <- NULL; category <- NULL; topic <- NULL; title <- NULL
  metadata_count <- 0
  for (i in seq_len(min(20, length(lines)))) {
    line <- trimws(lines[i])
    if (grepl("^\\[.+\\]$", line)) {
      ...
      else if (metadata_count == 2) category <- value
      else if (metadata_count == 3) topic <- value
      else if (metadata_count == 4) title <- value
      else break
    }
  }
  ...
  if (is.null(category) || is.null(topic)) {
    stop("Could not find Category and Topic metadata in the generated diagram text")
  }
  ...
  for (line in lines) {
    ...
    # current block (the latter matters because the 4 metadata lines at
  ...
  df <- data.frame(
    diagram_id = character(), diagram_name = character(), diagram_type = character(),
    category = character(), topic = character(), title = character(), is_template = logical(),
  ...
  for (b in blocks) {
    df <- rbind(df, data.frame(
      diagram_id = diagram_id, diagram_name = diagram_name, diagram_type = diagram_type,
      category = category, topic = topic, title = title, is_template = is_template,
...
# ============================================================
# CATEGORY / TOPIC DROPDOWNS - Strategic Analysis (own sentinels, own
# taxonomy query - NOT the shared category_topic_dropdown_ui() above,
# which is hardcoded to Flex Table's bq_get_flex_taxonomy/state_trigger_flex)
# ============================================================
DIAGRAM_CATEGORY_ADD_NEW_VALUE <- "__ADD_NEW_CATEGORY_DIAGRAM__"
DIAGRAM_TOPIC_ADD_NEW_VALUE <- "__ADD_NEW_TOPIC_DIAGRAM__"
DIAGRAM_DEFAULT_CATEGORIES <- c(
  "Situational Analysis", "Decision-Making Process", "Lifecycle & Innovation Dynamics",
  "Growth & Renewal", "Competitive Positioning", "Ecosystem & Relationship-Based",
  "Timing & Process", "Foundational"
)

diagram_type_dropdown_ui <- function(ns) {
  selectInput(ns("diagram_type_select"), "Diagram Type: *",
              choices = setNames(DIAGRAM_TYPES, DIAGRAM_TYPE_LABELS[DIAGRAM_TYPES]))
}

diagram_category_topic_dropdown_ui <- function(ns) {
  tagList(
    selectInput(ns("category_select"), "Category: *",
                choices = c(setNames(DIAGRAM_DEFAULT_CATEGORIES, DIAGRAM_DEFAULT_CATEGORIES),
                            "+ Add New Category" = DIAGRAM_CATEGORY_ADD_NEW_VALUE)),
    conditionalPanel(
      condition = sprintf("input['%s'] == '%s'", ns("category_select"), DIAGRAM_CATEGORY_ADD_NEW_VALUE),
      textInput(ns("new_category_text"), "New Category Name:")
    ),
    selectInput(ns("topic_select"), "Topic: *",
                choices = c("+ Add New Topic" = DIAGRAM_TOPIC_ADD_NEW_VALUE)),
    conditionalPanel(
      condition = sprintf("input['%s'] == '%s'", ns("topic_select"), DIAGRAM_TOPIC_ADD_NEW_VALUE),
      textInput(ns("new_topic_text"), "New Topic Name:")
    )
  )
}

setup_diagram_category_topic_cascade <- function(input, output, session, api_manager) {
  taxonomy <- reactive({
  ...
  observeEvent(taxonomy(), {
    tax <- taxonomy()
    stored <- if (nrow(tax) > 0) sort(unique(tax$category[nchar(trimws(tax$category)) > 0])) else character(0)
    all_categories <- sort(unique(c(DIAGRAM_DEFAULT_CATEGORIES, stored)))
    choices <- c(setNames(all_categories, all_categories), "+ Add New Category" = DIAGRAM_CATEGORY_ADD_NEW_VALUE)
    current <- isolate(input$category_select)
  ...
  observeEvent(input$category_select, {
    tax <- taxonomy()
    if (is.null(input$category_select) || input$category_select == DIAGRAM_CATEGORY_ADD_NEW_VALUE) {
      updateSelectInput(session, "topic_select", choices = c("+ Add New Topic" = DIAGRAM_TOPIC_ADD_NEW_VALUE))
      return()
    }
    topics <- sort(unique(tax$topic[tax$category == input$category_select & nchar(trimws(tax$topic %||% "")) > 0]))
    if (length(topics) == 0) {
      ...
      updateSelectInput(session, "topic_select",
                        choices = c("+ Add New Topic" = DIAGRAM_TOPIC_ADD_NEW_VALUE, setNames(topics, topics)))
    }
  }, ignoreInit = TRUE)

  reactive({
    category <- if (identical(input$category_select, DIAGRAM_CATEGORY_ADD_NEW_VALUE)) {
      trimws(input$new_category_text %||% "")
    } else {
      input$category_select %||% ""
    }
    topic <- if (identical(input$topic_select, DIAGRAM_TOPIC_ADD_NEW_VALUE)) {
      trimws(input$new_topic_text %||% "")
    } else {
      input$topic_select %||% ""
    }
...
# Diagram picker (for Browse / Visualizations) - lists diagram_id choices
# labeled "diagram_name - title", sourced from the taxonomy.
setup_diagram_picker <- function(input, output, session, api_manager, input_id = "diagram_select") {
  taxonomy <- reactive({
    api_manager$state_trigger_diagram()
    if (!api_manager$bq_authenticated) return(api_manager$empty_diagram_taxonomy())
    tryCatch(api_manager$bq_get_diagram_taxonomy(), error = function(e) api_manager$empty_diagram_taxonomy())
  ...
  observeEvent(taxonomy(), {
    tax <- taxonomy()
    if (nrow(tax) == 0) {
      updateSelectInput(session, input_id, choices = c("No diagrams saved yet" = ""))
      return()
    }
    labels <- sprintf("%s - %s (%s)", tax$diagram_name, tax$title, ifelse(tax$is_template, "template", "generated"))
    choices <- setNames(tax$diagram_id, labels)
...
render_diagram <- function(components_df, diagram_type) {
  cat(sprintf("🎨 [Strategic Analysis][DEBUG] Rendering diagram_type=%s with %d component row(s)\n",
  ...
  html_family <- c("grid", "quadrant", "table", "stacked_list", "linear_flow")
  ...
  if (diagram_type %in% html_family) {
    ...
    render_diagram_html_grid(components_df, diagram_type)
  } else if (diagram_type %in% svg_family) {
    cat("🎨 [Strategic Analysis][DEBUG] Dispatching to render_diagram_svg()\n")
    render_diagram_svg(components_df, diagram_type)
  } else if (diagram_type %in% plotly_family) {
    cat("🎨 [Strategic Analysis][DEBUG] Dispatching to render_diagram_plotly()\n")
    render_diagram_plotly(components_df, diagram_type)
  } else {
    tags$div(class = "status-error", sprintf("Unknown diagram_type: %s", diagram_type))
  }
}
...
render_diagram_html_grid <- function(components_df, diagram_type) {
  ...
  if (diagram_type %in% c("grid", "linear_flow", "table")) {
    n_cols <- max(body_rows$grid_col, body_rows$sequence_order, 1, na.rm = TRUE)
    html_parts <- c(html_parts, sprintf('<div class="diagram-grid" style="grid-template-columns: repeat(%d, 1fr);">', n_cols))

    ordered <- body_rows[order(body_rows$grid_row, body_rows$grid_col, body_rows$sequence_order), , drop = FALSE]
...
render_diagram_svg <- function(components_df, diagram_type) {
  ...
  W <- 700; H <- 500; cx <- W / 2; cy <- H / 2

  if (diagram_type == "radial") {
    outer <- nodes[is.na(nodes$sequence_order) | nodes$sequence_order <= 12, , drop = FALSE]
    outer <- outer[order(outer$sequence_order), , drop = FALSE]
    n <- nrow(outer)
    r_outer <- min(W, H) * 0.35
    ...
    if (n > 0) {
      for (i in seq_len(n)) {
        ...
        svg_nodes <- c(svg_nodes, sprintf(
          '<circle cx="%.1f" cy="%.1f" r="34" fill="var(--diagram-accent-teal, #008A82)" />
           <text x="%.1f" y="%.1f" text-anchor="middle" font-size="11" fill="white">%s</text>',
          x, y, x, y + 4, htmltools::htmlEscape(substr(outer$label_text[i] %||% "", 1, 14))
        ))
      }
    }
    svg_body <- paste(svg_nodes, collapse = "")
    svg <- sprintf(
      '<svg viewBox="0 0 %d %d" xmlns="http://www.w3.org/2000/svg">
         <circle cx="%.1f" cy="%.1f" r="60" fill="var(--diagram-accent-dark, #002C3C)" />
         <text x="%.1f" y="%.1f" text-anchor="middle" font-size="12" fill="white">%s</text>
         %s
       </svg>',
      W, H, cx, cy, cx, cy, htmltools::htmlEscape(substr(title_text, 1, 20)), svg_body
    )
    ...
    if (n > 0) {
      for (i in seq_len(n)) {
        ...
        svg_nodes <- c(svg_nodes, sprintf(
          '<rect x="%.1f" y="%.1f" width="120" height="46" rx="6" fill="var(--diagram-accent-teal, #008A82)" />
           <text x="%.1f" y="%.1f" text-anchor="middle" font-size="11" fill="white">%s</text>',
          x - 60, y - 23, x, y + 4, htmltools::htmlEscape(substr(satellites$label_text[i] %||% "", 1, 18))
        ))
      }
    }
    ...
    svg <- sprintf(
      '<svg viewBox="0 0 %d %d" xmlns="http://www.w3.org/2000/svg">
         %s
         <circle cx="%.1f" cy="%.1f" r="55" fill="var(--diagram-accent-dark, #002C3C)" />
         <text x="%.1f" y="%.1f" text-anchor="middle" font-size="12" fill="white">%s</text>
         %s
       </svg>',
      W, H, paste(svg_edges, collapse = ""), cx, cy, cx, cy + 4,
      htmltools::htmlEscape(substr(central_label, 1, 16)), paste(svg_nodes, collapse = "")
    )
  }
