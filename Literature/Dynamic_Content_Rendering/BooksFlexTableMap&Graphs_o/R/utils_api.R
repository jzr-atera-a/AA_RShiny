...
APIManager <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function() {
      ...
      cat("🔌 API Manager initialized (Book Summary + Flex Table + Mind Map + Knowledge Graph + Strategic Analysis)\n")
    },
    ...
    empty_diagram_taxonomy = function() {
      data.frame(diagram_id = character(), diagram_name = character(), diagram_type = character(),
                 category = character(), topic = character(), title = character(),
    ...
    authenticate_bigquery = function(json_path = NULL, json_text = NULL) {
      tryCatch({
        ...
        #     denormalized (diagram_name/diagram_type/category/topic
        #     repeated on every row belonging to the same diagram_id) so a
        #     whole diagram is one WHERE diagram_id = ... query, no joins.
        #     List-like content (bullets, series points) is packed into
        #     items_packed using DIAG_ITEM_SEP rather than exploded into one
        #     row per item - see R/utils_common.R. Append-only, same
        #     pattern as every other suite in this app. ------------------
        diagram_create <- sprintf("
          CREATE TABLE IF NOT EXISTS `%s` (
            id INTEGER, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP(),
            diagram_id STRING, diagram_name STRING, diagram_type STRING,
            category STRING, topic STRING, title STRING, is_template BOOL,
        ...
        for (q in list(books_create, flex_create, mindmap_create, kg_create, diagram_create)) {
      ...
      result <- tryCatch({
        self$bq_query(sprintf("
          SELECT diagram_id, ANY_VALUE(diagram_name) AS diagram_name,
                 ANY_VALUE(diagram_type) AS diagram_type, ANY_VALUE(category) AS category,
    ...
    bq_insert_diagram = function(data_frame) {
      ...
      required_cols <- c("id", "created_at", "diagram_id", "diagram_name", "diagram_type",
                        "category", "topic", "title", "is_template", "source_diagram_id",
      ...
      cat(sprintf("    diagram_type    : %s\n", data_frame$diagram_type[1]))
      cat(sprintf("    category/topic  : %s / %s\n", data_frame$category[1], data_frame$topic[1]))
