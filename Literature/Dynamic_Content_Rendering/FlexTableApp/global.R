...
# Flexible Comparison Table Suite v1.0 - Modular Architecture
# =============================================================
# Same modular architecture, CSS design system, and API-manager
# reactive-trigger pattern as the "Book Summary Complete Suite" app,
# adapted to store and render Claude-generated comparison tables
# with an EVER-CHANGING number of columns per query.

cat("\n╔══════════════════════════════════════════════╗\n")
cat("║  FLEXIBLE COMPARISON TABLE SUITE - INIT      ║\n")
cat("╚══════════════════════════════════════════════╝\n\n")
...
# ============================================================
# The module loader below installs/attaches packages dynamically based on
# what each module's manifest.yml declares (a YAML value, not literal R
# code). rsconnect's static dependency scanner used when publishing to
# shinyapps.io can ONLY detect packages that appear as a literal
# library()/require() call or a literal pkg::function() reference
# somewhere in the bundled .R files. It cannot see inside YAML files.
#
# Every package any module currently declares as a dependency is listed
# here explicitly so shinyapps.io always installs it, regardless of which
# modules are enabled/disabled in modules/_module_registry.yml.
suppressPackageStartupMessages({
  library(shinyjs)
  library(httr)
  library(jsonlite)
...
create_ui <- function(module_loader) {
  ...
  dashboardPage(
    dashboardHeader(
      title = module_loader$registry$app$name %||% "Flexible Comparison Table Suite"
    ),
    ...
    dashboardBody(
      ...
      # the generate_table module (loading spinner).
      shinyjs::useShinyjs(),
      ...
      tags$head(
        ...
        # MathJax for LaTeX formula rendering inside any generated cell
