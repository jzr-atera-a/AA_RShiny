  ...
  tagList(
    fluidRow(
      box(
        ...
        h4("BigQuery Configuration - shared by all three suites"),
        p("Connect ONCE here. This authenticates Flex Table, Mind Map, AND Knowledge Graph together - ",
          "they share the same project/dataset, but each has its own table, all auto-created below."),
        ...
        fluidRow(
          ...
          column(6,
                 ...
                 p(style = "color: #7f8c8d; font-size: 12px;",
                   "All three tables live in this one project/dataset. Table names are fixed (shown above) ",
        ...
        fluidRow(
          column(6,
                 actionButton(ns("authenticate"), "Connect to BigQuery (all 3 tables)",
                              class = "btn-primary btn-lg", icon = icon("plug"), style = "width: 100%;")
          ),
          column(6,
                 selectInput(ns("test_table_choice"), "Table to test-query:",
                            choices = c("Flex Table" = "flex", "Mind Map" = "mindmap", "Knowledge Graph" = "kg"))
          )
        ),
