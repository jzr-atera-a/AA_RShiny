...
flex_table_about_ui <- function(id) {
  ns <- NS(id)
...
"Model A|||KV|||Fast to train but sensitive to regime shifts|||COL|||Model B|||KV|||Slower, more stable across regimes"
        ...
        p("The Table Viewer tab requires four compulsory, cascading filters - Category, Topic, ",
          "Rows Label (", tags$code("row_dimension_label"), "), and Columns Label (",
          tags$code("column_dimension_label"), "). A single Category+Topic can contain several ",
          "distinct subtables (e.g. Columns Label 'Arms Top 3' vs 'Legs Top 7' under the same ",
          "Topic), so all four together uniquely identify one subtable. Once selected, the FULL ",
          "grid loads - every row, every column - reading back each row's ", tags$code("columns_data"),
          ", taking the union of column headers across all matching rows, and rendering a wide, ",
          "scrollable HTML table labeled with the stored Rows/Columns Label text, with no schema ",
          "migration needed no matter how many columns Claude generated."),
        ...
        tags$ul(
          ...
          tags$li(tags$strong("Table Viewer"), " - the rendered, scrollable, LaTeX-aware comparison grid for one Category+Topic+Rows Label+Columns Label subtable")
        )
      )
    )
  )
