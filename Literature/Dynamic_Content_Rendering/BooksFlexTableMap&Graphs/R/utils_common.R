...
diagram_type_instructions <- function(diagram_type, category, domain, topic, title_hint) {
  sep <- DIAG_ITEM_SEP
  ...
  common_header <- paste0(
    ...
    '1. Start with EXACTLY 5 metadata lines, each on its own line. Each line must contain ONLY the actual ',
    'value wrapped in single square brackets - do NOT include a field name, label, or colon inside or around ',
    'the brackets. Lines 1-4 must be EXACTLY these values (they are fixed by the user\'s selection, not yours ',
    'to change); line 5 is the diagram title as instructed below. The first 5 lines of your entire response ',
    'must be EXACTLY:\n',
    '[', diagram_type, ']\n[', category, ']\n[', domain, ']\n[', topic, ']\n', title_line, '\n\n',
  ...
  type_block <- switch(diagram_type,
    "grid" = paste0(
      'DIAGRAM_TYPE = grid - a named framework broken into labeled cells arranged in a RECTANGULAR grid of ',
      'ROWS and COLUMNS, drawn the way that specific framework is conventionally drawn. Do NOT default to a ',
      'single row of N cells - pick the rows x columns arrangement that matches the framework and the number ',
      'of cells you decide it needs:\n',
      '   - 4 cells (e.g. a SWOT: Strengths/Weaknesses/Opportunities/Threats) -> 2 ROWS x 2 COLUMNS, i.e. ',
      'positions (row=1,col=1) (row=1,col=2) (row=2,col=1) (row=2,col=2). Do NOT lay a SWOT out as 1 row of 4.\n',
      '   - 6 cells (e.g. a PESTEL: Political/Economic/Social/Technological/Environmental/Legal) -> 2 ROWS x ',
      '3 COLUMNS.\n',
      '   - 9 cells (e.g. a 3x3 canvas-style framework) -> 3 ROWS x 3 COLUMNS.\n',
      '   - For any other cell count, choose the rows x columns combination closest to square (e.g. 5 cells ',
      '-> 2 rows x 3 columns with one empty slot, or 2+3; 8 cells -> 2 rows x 4 columns or 4 rows x 2 columns) ',
      '- a single row of N is ONLY correct when N is 3 or fewer, or the content is genuinely a linear sequence ',
      '(in which case use DIAGRAM_TYPE = linear_flow instead, not grid).\n\n',

      'Decide your rows x columns arrangement FIRST, then, for EACH cell, output one block with its ACTUAL ',
      '1-indexed (grid_row, grid_col) position in that arrangement - these are real 2D coordinates the renderer ',
      'places directly, not a sequential counter:\n',
      '[component_type]: shape_rectangle\n[layout_role]: grid_cell\n[grid_row]: <1..num_rows, this cell\'s actual row>\n',
      '[grid_col]: <1..num_cols, this cell\'s actual column>\n',
    ...
    "table" = paste0(
      'DIAGRAM_TYPE = table (2D matrix, e.g. TOWS, VRIO). First decide how many header rows/columns you need ',
      '(typically row 1 = column headers, column 1 = row headers) and how many data rows/columns follow, based ',
      'on the framework. Output one block per cell using its ACTUAL 1-indexed grid_row/grid_col position - ',
      'these are real 2D coordinates the renderer places directly, not a sequential counter. Use them for BOTH ',
...
generate_diagram_prompt <- function(diagram_type, category, domain, topic, title_hint, user_request) {
  type_instructions <- diagram_type_instructions(diagram_type, category, domain, topic, title_hint)
  ...
  paste0(
    ...
    'Now generate the diagram, beginning with the 5 metadata lines.'
  )
}

# After generation, force-overwrite the metadata block's first 4 positional
# bare-bracket lines (diagram_type/category/domain/topic - fixed by the
# user's dropdown selection, never Claude's to change) back to the
# authoritative values, regardless of what Claude actually wrote. The 5th
# line (title) is only forced when the user supplied an explicit
...
overwrite_diagram_header <- function(text, diagram_type, category, domain, topic, title_override = NULL) {
  lines <- strsplit(text, "\n")[[1]]
  metadata_line_idx <- which(grepl("^\\[.+\\]$", trimws(lines)))
  metadata_line_idx <- head(metadata_line_idx, 5)

  values <- list(diagram_type, category, domain, topic, title_override)
...
#
# Part 1 - METADATA: the first 5 bare "[value]" lines (no field name, no
...
# category/domain/topic/title. This means Bulk Import and any hand-pasted text
...
parse_diagram_text <- function(text, diagram_id, is_template = FALSE, source_diagram_id = NA, created_by = "claude_agent") {
  ...
  # ---- Part 1: metadata block (diagram_type, category, domain, topic, title) ----
  diagram_type <- NULL; category <- NULL; domain <- NULL; topic <- NULL; title <- NULL
  metadata_count <- 0
  for (i in seq_len(min(20, length(lines)))) {
    line <- trimws(lines[i])
    if (grepl("^\\[.+\\]$", line)) {
      ...
      else if (metadata_count == 3) domain <- value
      else if (metadata_count == 4) topic <- value
      else if (metadata_count == 5) title <- value
      else break
    }
  }
...
#
# diagram_type is a SEPARATE selector (which renderer family/instruction
# block to use), not part of the classification hierarchy, so it keeps
# its own small dropdown here.
...
SIXSIGMA_GROUPS <- c("define", "measure", "analyse", "improve", "control")
...
SIXSIGMA_TYPES_BY_GROUP <- list(
  define  = c("sipoc", "ctq_tree"),
  measure = c("process_map"),
  analyse = c("fishbone", "five_whys", "pareto", "fmea_table"),
  improve = c("pugh_matrix", "doe_table", "five_s"),
  control = c("control_chart", "balanced_scorecard")
)
...
render_sixsigma <- function(components_df, diagram_type) {
  ...
  svg_family <- c("fishbone", "ctq_tree")
  plotly_family <- c("pareto", "control_chart")
...
render_sixsigma_html <- function(components_df, diagram_type) {
  ...
  if (diagram_type == "sipoc") {
    ...
    for (i in seq_len(nrow(ordered))) {
      ...
      items_html <- if (length(items) > 0) paste0('<ul class="diagram-item-list">', paste0('<li>', htmltools::htmlEscape(items), '</li>', collapse = ""), '</ul>') else ""
      color_style <- if (has_real_value(row$color_hint)) sprintf(' data-color-hint="%s"', row$color_hint) else ""
      html_parts <- c(html_parts, sprintf('<div class="diagram-cell"%s><div class="section-tag">%s</div>%s</div>',
        color_style, htmltools::htmlEscape(row$label_text %||% ""), items_html))
    }
    html_parts <- c(html_parts, '</div>')

  } else if (diagram_type == "process_map") {
    ordered <- body_rows[order(body_rows$sequence_order), , drop = FALSE]
  ...
  } else if (diagram_type == "balanced_scorecard") {
    quad_rows <- body_rows[body_rows$component_type == "region_quadrant", , drop = FALSE]
    html_parts <- c(html_parts, '<div class="diagram-quadrant-grid">')
    for (pos in c("top_left", "top_right", "bottom_left", "bottom_right")) {
      row <- quad_rows[quad_rows$quadrant_position == pos, , drop = FALSE]
  ...
  } else if (diagram_type == "doe_table") {
    n_rows <- max(body_rows$grid_row, 1, na.rm = TRUE)
    n_cols <- max(body_rows$grid_col, 1, na.rm = TRUE)
