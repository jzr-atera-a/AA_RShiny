  ...
  public = list(
    ...
    authenticate_bigquery = function(json_path = NULL, json_text = NULL) {
      tryCatch({
        ...
        # from before the `domain` column was introduced, ADD COLUMN IF NOT
        # EXISTS brings it up to date without touching any existing rows
        # (they simply get domain = NULL, same non-destructive philosophy
        # as every CREATE TABLE IF NOT EXISTS in this app).
        ...
        for (q in list(books_create, flex_create, mindmap_create, kg_create, diagram_create, diagram_alter_add_domain, sixsigma_create)) {
