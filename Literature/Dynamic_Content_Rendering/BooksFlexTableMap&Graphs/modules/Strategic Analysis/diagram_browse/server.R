# modules/Strategic Analysis/diagram_browse/server.R

diagram_browse_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    load_data <- function() {
      ...
      tryCatch({
        api_manager$diagram_taxonomy_cache <- NULL  # force a fresh pull, not the cached copy
        catalog <- api_manager$bq_get_diagram_taxonomy()
        ...
        query <- if (nchar(trimws(input$filter_diagram_id %||% "")) > 0) {
          sprintf("SELECT * FROM `%s` WHERE diagram_id = '%s' ORDER BY diagram_id, grid_row, grid_col, sequence_order LIMIT %d",
                  api_manager$bq_full_table_diagram, safe_sql_escape(trimws(input$filter_diagram_id)), input$max_rows)
        } else {
          sprintf("SELECT * FROM `%s` ORDER BY created_at DESC LIMIT %d",
                  api_manager$bq_full_table_diagram, input$max_rows)
        }
    ...
    output$download_catalog <- downloadHandler(
      filename = function() paste0("diagram_catalog_", format(Sys.Date(), "%Y%m%d"), ".csv"),
    ...
    output$download_components <- downloadHandler(
      filename = function() paste0("diagram_components_", format(Sys.Date(), "%Y%m%d"), ".csv"),
