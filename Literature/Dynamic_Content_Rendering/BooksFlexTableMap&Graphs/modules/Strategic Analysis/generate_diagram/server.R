  ...
  moduleServer(id, function(input, output, session) {
    ...
    run_parse_preview <- function(text, quiet = FALSE) {
      ...
      tryCatch({
        ...
        # diagram_type/category/domain/topic/title now come FROM the text
        # itself (the metadata block Claude was instructed to write) - only
        ...
        if (!quiet) {
          output$status <- renderUI({
            tags$div(class = "status-success", tags$i(class = "fa fa-check-circle"),
                     sprintf(" Parsed %d component row(s) (%s / %s / %s / %s)",
                             nrow(df), df$diagram_type[1], df$category[1], df$domain[1], df$topic[1]))
          })
        }
      }, error = function(e) {
        parsed_preview(NULL)
    ...
    observeEvent(input$generate, {
      ...
      tryCatch({
        ...
        # Force diagram_type/category/domain/topic back to the authoritative
        ...
        diagram_text <- overwrite_diagram_header(
          response_text, input$diagram_type_select, cdt$category, cdt$domain, cdt$topic,
