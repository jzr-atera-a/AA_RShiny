      ...
      tryCatch({
        ...
        output$parse_info <- renderUI({
          tags$div(class = "status-success", tags$i(class = "fa fa-check-circle"),
                   sprintf(" Successfully parsed %d component row(s) for '%s' (Type: %s | Category: %s | Domain: %s | Topic: %s)",
                           nrow(df), df$title[1], df$diagram_type[1], df$category[1], df$domain[1], df$topic[1]))
        })
