...
visualizations_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    #
    # Uses observe() rather than observeEvent(input$viz_genre, ...):
    ...
    # NOT a real dependency. That means if a new book is uploaded under a
    # Genre string that happens to already be selected, Topic/Book would
    # never refresh to show it. observe() tracks every reactive read in
    # its body, so it correctly re-cascades whenever EITHER the taxonomy
    # changes OR the relevant parent selection changes.
    ...
    observe({
      tax <- viz_taxonomy()
      genre_val <- input$viz_genre
      
      if (is.null(genre_val) || genre_val == "") {
    ...
    observe({
      tax <- viz_taxonomy()
      genre_val <- input$viz_genre
      topic_val <- input$viz_topic
      
      if (is.null(genre_val) || is.null(topic_val) ||
          genre_val == "" || topic_val == "") {
        updateSelectInput(session, "select_book", choices = c("(select a topic first)" = ""))
        return()
      }
      
      subset_rows <- tax[tax$genre == genre_val & tax$topic == topic_val, ]
      ...
      if (nrow(subset_rows) == 0) {
        ...
        current <- isolate(input$select_book)
        choices <- setNames(subset_rows$book_name, labels)
        selected <- if (!is.null(current) && current %in% subset_rows$book_name) current else subset_rows$book_name[1]
        updateSelectInput(session, "select_book", choices = choices, selected = selected)
      }
    })
    ...
    observeEvent(input$select_book, {
      ...
      tryCatch({
        ...
        query <- sprintf("SELECT DISTINCT chapter FROM `%s` WHERE book_name = '%s' ORDER BY chapter",
                         api_manager$bq_full_table_books, safe_book)
      ...
      tryCatch({
        ...
        if (input$filter_chapter == "all") {
          ...
          query <- sprintf("SELECT * FROM `%s` WHERE book_name = '%s' AND chapter = '%s' ORDER BY section",
                           api_manager$bq_full_table_books, safe_book, safe_chapter)
        }
