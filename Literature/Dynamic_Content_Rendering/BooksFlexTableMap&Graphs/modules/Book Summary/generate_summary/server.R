  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$generate, {
      ...
      tryCatch({
        ...
        was_truncated <- identical(attr(summary_text, "claude_stop_reason"), "max_tokens")
        ...
        truncation_warning <- if (was_truncated) {
          tagList(
            tags$br(), tags$br(),
        ...
        output$status <- renderUI({
          tags$div(class = if (was_truncated) "status-warning" else "status-success",
                   ...
                   if (was_truncated) " Summary generated, but truncated" else " ✓ Summary generated successfully!",
                   tags$br(),
    ...
    observeEvent(input$copy_to_bulk, {
      if (nchar(summary_data()) > 0) {
        api_manager$set_pending_bulk_text_books(summary_data())
        # session is namespaced to this module; rootScope() is needed to
        # update the sidebarMenu input, which lives outside any module
        updateTabItems(session$rootScope(), "sidebar_menu", selected = "books_bulk_import")
    ...
    observeEvent(input$parse_and_upload, {
      ...
      tryCatch({
        ...
        rows_uploaded <- api_manager$bq_insert_books(parsed_df)
