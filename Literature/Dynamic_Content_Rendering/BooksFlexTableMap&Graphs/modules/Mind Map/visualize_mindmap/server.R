...
visualize_mindmap_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    # inside it is NOT a real dependency. That means if a map is uploaded
    # under a Category/Domain string that happens to already be selected
    # (e.g. reusing a Domain string under a different Category than
    # before), the dropdown's value never changes, the observer never
    # re-fires, and Topic/Map Version silently keep showing stale data.
    ...
    observe({
      ...
      tryCatch({
        maps <- api_manager$bq_get_map_ids_for_topic(cat_val, dom_val, top_val)
        if (nrow(maps) == 0) {
          updateSelectInput(session, "viz_map_id", choices = c("(no maps found)" = ""))
        } else {
          labels <- sprintf("%s (%s)", maps$map_title, maps$map_id)
          current <- isolate(input$viz_map_id)
          choices <- setNames(maps$map_id, labels)
          selected <- if (!is.null(current) && current %in% maps$map_id) current else maps$map_id[1]
          updateSelectInput(session, "viz_map_id", choices = choices, selected = selected)
        }
      }, error = function(e) {
