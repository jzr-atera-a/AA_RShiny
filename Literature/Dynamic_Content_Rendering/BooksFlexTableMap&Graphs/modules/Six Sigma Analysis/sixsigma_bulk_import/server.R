# modules/Six Sigma Analysis/sixsigma_bulk_import/server.R

sixsigma_bulk_import_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {

    parsed_data <- reactiveVal(NULL)

    observeEvent(api_manager$pending_bulk_text_sixsigma(), {
      incoming_text <- api_manager$pending_bulk_text_sixsigma()
    ...
    observeEvent(input$parse, {
      ...
      tryCatch({
        df <- parse_sixsigma_text(input$diagram_text, diagram_id = "", is_template = FALSE, created_by = "manual_bulk_import")
        df$diagram_id <- generate_new_sixsigma_id(df$topic[1])

        parsed_data(df)

        output$parse_info <- renderUI({
          tags$div(class = "status-success", tags$i(class = "fa fa-check-circle"),
                   sprintf(" Successfully parsed %d component row(s) for '%s' (Type: %s | Group: %s | Category: %s | Domain: %s | Topic: %s)",
                           nrow(df), df$title[1], df$diagram_type[1], df$diagram_group[1], df$category[1], df$domain[1], df$topic[1]))
        })
    ...
    observeEvent(input$upload, {
      ...
      tryCatch({
        rows_uploaded <- api_manager$bq_insert_sixsigma(df)
        output$status <- renderUI({
          tags$div(class = "status-success", tags$i(class = "fa fa-check-circle"),
