  ...
  public = list(
    ...
    empty_taxonomy = function() {
      data.frame(category = character(), domain = character(), topic = character(),
                 map_id = character(), map_title = character(),
    ...
    authenticate_bigquery = function(json_path = NULL, json_text = NULL) {
      tryCatch({
        ...
        # Create table if not exists - APPEND-ONLY versioned node schema.
        # An edit is a NEW row (same map_id+node_id, change_type='update'),
        # never an UPDATE/DELETE statement - "current state" is derived by
        # taking the latest row per (map_id, node_id) and dropping anything
        # whose latest change_type is 'delete'. See get_current_tree_state().
    ...
    # Distinct Category/Domain/Topic/map_id combinations (+ map_title),
    # used to build the cascading Category -> Domain -> Topic dropdowns,
    # and to let the user pick a specific map version when more than one
    # exists under the same Topic. Only considers each map_id's LATEST
    # 'create' row (map_title could theoretically differ across an edit,
    # though the app never changes it) to keep this cheap and simple.
    bq_get_taxonomy = function() {
      ...
      query <- sprintf(
        "SELECT DISTINCT category, domain, topic, map_id, map_title
         FROM `%s` ORDER BY category, domain, topic, map_id",
        self$bq_full_table_id
      )
    ...
    # All map_ids that exist for a given Category+Domain+Topic, most
    # recent first (by max created_at across that map's rows) - used by
    # Edit/Visualize tabs when a Topic has more than one generation.
    ...
    # ============================================================
    # CURRENT TREE STATE (append-only reader)
    # ============================================================
    # Reads ALL versioned rows for one map_id, keeps only the LATEST row
    # per node_id (by id), and drops any node whose latest change_type
    # is 'delete'. This is the single shared function used by Visualize,
    # Edit (to load the tree before asking Claude for a delta), and the
    # cascade-delete computation - so "current state" is defined exactly
    # once and consistently everywhere.
    get_current_tree_state = function(map_id) {
      ...
      query <- sprintf(
        "SELECT * FROM `%s` WHERE map_id = '%s' ORDER BY id",
        self$bq_full_table_id, safe_map_id
      )
      ...
      if (nrow(all_versions) == 0) {
        return(list(
          category = "", domain = "", topic = "", map_title = "",
          nodes = data.frame(node_id = character(), node_label = character(),
                             node_content = character(), parent_node_id = character(),
                             cross_links = character(), sort_order = integer(),
                             stringsAsFactors = FALSE)
        ))
      }

      # Latest row per node_id (highest id wins)
      latest_idx <- tapply(seq_len(nrow(all_versions)), all_versions$node_id, function(idx) {
        idx[which.max(all_versions$id[idx])]
      })
      latest <- all_versions[unlist(latest_idx), ]

      # Drop tombstoned nodes
      current <- latest[latest$change_type != "delete", ]
      current <- current[order(current$sort_order, current$node_id), ]

      list(
        category = if (nrow(all_versions) > 0) all_versions$category[1] else "",
        domain = if (nrow(all_versions) > 0) all_versions$domain[1] else "",
        topic = if (nrow(all_versions) > 0) all_versions$topic[1] else "",
        map_title = if (nrow(all_versions) > 0) all_versions$map_title[1] else "",
        nodes = current[, c("node_id", "node_label", "node_content", "parent_node_id",
                            "cross_links", "sort_order")]
      )
    },

    # Append rows to BigQuery. `data_frame` must already have columns:
    # change_type, category, domain, topic, map_id, map_title, node_id,
    # node_label, node_content, parent_node_id, cross_links, sort_order.
