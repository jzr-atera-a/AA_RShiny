...
# Mind Map Suite v1.0 - Modular Architecture
# =============================================================
# Same modular architecture, CSS design system, and API-manager
# reactive-trigger pattern as the Flexible Comparison Table Suite,
# adapted to store and render CLAUDE-GENERATED MIND MAPS: a primary
# hierarchical tree (one parent per node, guaranteed acyclic by
# construction) plus auxiliary cross-links that may point anywhere
# in the graph (including upward, to an ancestor).

cat("\n╔══════════════════════════════════════════════╗\n")
cat("║  MIND MAP SUITE - INIT                        ║\n")
cat("╚══════════════════════════════════════════════╝\n\n")
...
# ============================================================
# See FlexTableApp/global.R for the full explanation: rsconnect's static
# dependency scanner can only see literal library()/pkg::fn() calls, not
# packages named inside YAML manifests, so every package any module
# might need is declared here explicitly regardless of which modules
# are enabled/disabled in modules/_module_registry.yml.
suppressPackageStartupMessages({
  library(shinyjs)
  library(httr)
  library(curl)
...
create_ui <- function(module_loader) {
  ...
  dashboardPage(
    dashboardHeader(
      title = module_loader$registry$app$name %||% "Mind Map Suite"
    ),
    ...
    dashboardBody(
      ...
      tags$head(
        ...
        # D3.js for the mind map visualizer (modules/visualize_mindmap) -
        # loaded once globally rather than as an R htmlwidget package,
        # since the visualizer builds its own SVG/JS directly.
        tags$script(src = "https://cdnjs.cloudflare.com/ajax/libs/d3/7.8.5/d3.min.js"),
        # MathJax for LaTeX formula rendering inside node content
