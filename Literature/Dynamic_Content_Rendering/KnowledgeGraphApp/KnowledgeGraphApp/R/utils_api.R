  ...
  public = list(
    ...
    empty_taxonomy = function() {
      data.frame(category = character(), domain = character(), topic = character(),
                 graph_id = character(), graph_title = character(),
    ...
    call_claude = function(prompt, max_tokens = NULL, progress_callback = NULL) {
      ...
      parse_sse_event <- function(event_block) {
        ...
        for (dl in data_lines) {
          ...
          if (parsed$type == "content_block_delta" &&
            ...
            # stop_reason == "max_tokens" is the API's own ground-truth
            # signal that the response was CUT OFF by the token limit,
            # rather than finishing naturally ("end_turn"). This is far
            # more reliable than trying to infer truncation from what's
            # missing in the parsed text afterward.
      ...
      tryCatch({
        ...
        if (identical(final_stop_reason, "max_tokens")) {
          self$log_debug(sprintf(
            "[%s] WARNING: response was CUT OFF by hitting the max_tokens limit - it is very likely incomplete.",
            call_id
          ), tag = "call_claude")
        }
    ...
    authenticate_bigquery = function(json_path = NULL, json_text = NULL) {
      tryCatch({
        ...
        # Create table if not exists - APPEND-ONLY versioned KNOWLEDGE
        # GRAPH schema. Unlike a tree, a knowledge graph has no root and
        # no hierarchy - it's ENTITIES (typed nodes) connected by
        # RELATIONSHIPS (typed, directed edges = subject-predicate-object
        # triples). Both live in ONE physical table, distinguished by
        # row_kind, so entity metadata (type/description) is stored once
        # per entity rather than repeated on every relationship touching
        # it. Same append-only philosophy as the sibling apps: an edit is
        # a NEW row (same graph_id + entity_id/relationship_id,
        # change_type='update'), never an UPDATE/DELETE statement.
    ...
    # Distinct Category/Domain/Topic/graph_id combinations (+ graph_title),
    # used to build the cascading Category -> Domain -> Topic dropdowns,
    # and to let the user pick a specific graph version when more than
    # one exists under the same Topic.
    bq_get_taxonomy = function() {
      ...
      query <- sprintf(
        "SELECT DISTINCT category, domain, topic, graph_id, graph_title
         FROM `%s` ORDER BY category, domain, topic, graph_id",
        self$bq_full_table_id
      )
    ...
    # All graph_ids that exist for a given Category+Domain+Topic, most
    # recent first - used by Edit/Visualize tabs when a Topic has more
    # than one generation.
    bq_get_graph_ids_for_topic = function(category, domain, topic) {
      if (!self$bq_authenticated) return(character(0))

      safe_category <- gsub("'", "''", category)
      safe_domain <- gsub("'", "''", domain)
      safe_topic <- gsub("'", "''", topic)

      query <- sprintf(
        "SELECT graph_id, MAX(created_at) as last_touched, ANY_VALUE(graph_title) as graph_title
         FROM `%s`
         WHERE category = '%s' AND domain = '%s' AND topic = '%s'
         GROUP BY graph_id ORDER BY last_touched DESC",
        self$bq_full_table_id, safe_category, safe_domain, safe_topic
      )
    ...
    # ============================================================
    # CURRENT GRAPH STATE (append-only reader)
    # ============================================================
    # Reads ALL versioned rows for one graph_id, resolves the latest
    # row per entity_id (among row_kind='entity' rows) and separately
    # the latest row per relationship_id (among row_kind='relationship'
    # rows), dropping anything whose latest change_type is 'delete'.
    # This is the single shared function used by both Visualize tabs,
    # Edit (to load the graph before asking Claude for a delta), and
    # the cascade-delete computation - so "current state" is defined
    # exactly once and consistently everywhere.
    get_current_graph_state = function(graph_id) {
      ...
      query <- sprintf(
        "SELECT * FROM `%s` WHERE graph_id = '%s' ORDER BY id",
        self$bq_full_table_id, safe_graph_id
      )
      ...
      if (nrow(entity_versions) > 0) {
        latest_idx <- tapply(seq_len(nrow(entity_versions)), entity_versions$entity_id, function(idx) {
          idx[which.max(entity_versions$id[idx])]
        })
        ...
        if (nrow(latest) > 0) {
          ...
          current_entities <- latest[, c("entity_id", "entity_label", "entity_type",
                                         "entity_description", "sort_order")]
        }
      }

      current_relationships <- empty_relationships
      if (nrow(relationship_versions) > 0) {
        latest_idx <- tapply(seq_len(nrow(relationship_versions)), relationship_versions$relationship_id, function(idx) {
          idx[which.max(relationship_versions$id[idx])]
        })
      ...
      list(
        category = all_versions$category[1],
        domain = all_versions$domain[1],
        topic = all_versions$topic[1],
        graph_title = all_versions$graph_title[1],
        entities = current_entities,
        relationships = current_relationships
      )
    },

    # Append rows to BigQuery. `data_frame` must already have columns:
    # change_type, row_kind, category, domain, topic, graph_id,
    # graph_title, entity_id, entity_label, entity_type,
    # entity_description, relationship_id, source_entity_id, predicate,
    # target_entity_id, relationship_description, sort_order.
    # id/created_at/source are assigned here.
    bq_insert = function(data_frame, source = "claude") {
      if (!self$bq_authenticated) stop("Not authenticated to BigQuery")
      ...
      for (col in required_cols) {
        if (!col %in% names(data_frame)) data_frame[[col]] <- if (col == "sort_order") 0L else ""
      }
