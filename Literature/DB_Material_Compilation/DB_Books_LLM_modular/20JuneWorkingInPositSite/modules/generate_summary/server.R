...
generate_summary_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$generate, {
      ...
      cat("⚙️  [generate_summary] Building prompt...\n")
      prompt <- tryCatch({
      ...
      tryCatch({
        ...
        # Force-overwrite Claude's [Genre]/[Topic] lines with the exact
        # values selected/typed in the UI, guaranteeing no mismatch
        summary_text <- overwrite_genre_topic_lines(summary_text, gt$genre, gt$topic)
        cat("✓ [generate_summary] Genre/Topic lines force-set to:", gt$genre, "/", gt$topic, "\n")
