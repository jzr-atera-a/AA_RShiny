...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem("About & Overview",     tabName = "about",     icon = icon("circle-info")),
...
server <- function(input, output, session) {
  ...
  # Function to fetch data from IG's REST API via the igfetchr package.
  ...
  # Branches to the IG fetcher when the IG (CFDs) asset class is selected.
    ...
    if (!is.null(data) && nrow(data) > 0) {
      ...
      showNotification(
        paste0("Failed to load data for ", symbol,
               ...
               else ". Yahoo Finance may be temporarily unavailable — please try Refresh."),
        type = "error", duration = 8
      )
    }
  }, ignoreNULL = FALSE)
