...
# templates copied into a "templates" folder (see default path below).
#
# High-fidelity approach: rather than deleting and rebuilding tables (which loses the
# template's own cell shading, merged cells, and column widths), every field is written
# directly into the EXISTING run inside its EXISTING cell via the document's raw XML
# (accessed through officer's underlying xml2 document). Only the text node changes;
# every w:tcPr (shading, width, merge/gridSpan) and w:rPr (font, bold, size) on that
# cell is left completely untouched. Verified directly against both real templates:
# post-edit XML has the identical count of <w:shd> (49), <w:gridSpan> (3) and <w:tcW>
# (90) elements as the untouched originals, with only the target cells' text changed.

u3_docx_set_cell_text <- function(xml_doc, table_index, row, col, text) {
  tbls <- xml2::xml_find_all(xml_doc, "//w:tbl")
  trs  <- xml2::xml_find_all(tbls[[table_index]], "./w:tr")
  tcs  <- xml2::xml_find_all(trs[[row]], "./w:tc")
  tc   <- tcs[[col]]
  ps   <- xml2::xml_find_all(tc, "./w:p")
  p    <- ps[[length(ps)]]

  for (r in xml2::xml_find_all(p, "./w:r")) xml2::xml_remove(r)

  new_r <- xml2::xml_add_child(p, "w:r", .where = "last")
  ppr_rpr <- xml2::xml_find_first(p, "./w:pPr/w:rPr")
  if (!inherits(ppr_rpr, "xml_missing")) xml2::xml_add_child(new_r, ppr_rpr, .copy = TRUE)

  new_t <- xml2::xml_add_child(new_r, "w:t", .where = "last")
  xml2::xml_set_attr(new_t, "xml:space", "preserve")
  xml2::xml_set_text(new_t, as.character(text %||% ""))
  invisible(NULL)
}

# Adds `text` as a new paragraph appended after whatever is already in a cell (used for
# "Overnight news" / "Today's Data Releases", where the template's own label text in
# that cell should stay, with the entered content added as a further line beneath it).
u3_docx_append_par_in_cell <- function(xml_doc, table_index, row, col, text) {
  ...
  ps   <- xml2::xml_find_all(tc, "./w:p")
  last_p <- ps[[length(ps)]]
  new_p  <- xml2::xml_add_sibling(last_p, last_p, .copy = TRUE, .where = "after")
  for (r in xml2::xml_find_all(new_p, "./w:r")) xml2::xml_remove(r)
  new_r <- xml2::xml_add_child(new_p, "w:r", .where = "last")
  new_t <- xml2::xml_add_child(new_r, "w:t", .where = "last")
  xml2::xml_set_attr(new_t, "xml:space", "preserve")
  xml2::xml_set_text(new_t, as.character(text))
  invisible(NULL)
...
# Clones the last row of a table until it has at least `min_rows` rows, so extra
# partial-exit rows beyond the template's built-in 3 keep the same formatting/borders
# as every other row rather than being added as a plain, unstyled row.
u3_docx_ensure_table_rows <- function(xml_doc, table_index, min_rows) {
  tbls <- xml2::xml_find_all(xml_doc, "//w:tbl")
  tbl <- tbls[[table_index]]
  trs <- xml2::xml_find_all(tbl, "./w:tr")
  n <- length(trs)
  if (n < min_rows) {
    last_row <- trs[[n]]
    for (i in seq_len(min_rows - n)) {
      last_row <- xml2::xml_add_sibling(last_row, last_row, .copy = TRUE, .where = "after")
    }
  }
  invisible(NULL)
}
...
u3_build_morning_sheet_docx <- function(template_path, out_path, f) {
  ...
  xml_doc <- doc$doc_obj$get()

  u3_docx_set_cell_text(xml_doc, 1, 1, 2, f$name)
  u3_docx_set_cell_text(xml_doc, 1, 3, 2, f$date)

  u3_docx_set_cell_text(xml_doc, 2, 2, 2, f$dow_price)
  u3_docx_set_cell_text(xml_doc, 2, 2, 3, f$dow_change)
  u3_docx_set_cell_text(xml_doc, 2, 3, 2, f$nikkei_price)
  u3_docx_set_cell_text(xml_doc, 2, 3, 3, f$nikkei_change)

  fx <- f$fx_df
  for (i in seq_len(4)) {
    u3_docx_set_cell_text(xml_doc, 3, i + 1, 2, fx$Low[i])
    u3_docx_set_cell_text(xml_doc, 3, i + 1, 3, fx$High[i])
    u3_docx_set_cell_text(xml_doc, 3, i + 1, 4, fx$Current[i])
    u3_docx_set_cell_text(xml_doc, 3, i + 1, 5, fx$Time[i])
  }

  cm <- f$comm_df
  for (i in seq_len(2)) {
    u3_docx_set_cell_text(xml_doc, 4, i + 1, 2, cm$Low[i])
    u3_docx_set_cell_text(xml_doc, 4, i + 1, 3, cm$High[i])
    u3_docx_set_cell_text(xml_doc, 4, i + 1, 4, cm$Current[i])
    u3_docx_set_cell_text(xml_doc, 4, i + 1, 5, cm$Time[i])
  }

  u3_docx_append_par_in_cell(xml_doc, 5, 1, 1, f$overnight_news)
  u3_docx_append_par_in_cell(xml_doc, 6, 1, 1, f$today_releases)
...
u3_build_trade_sheet_docx <- function(template_path, out_path, f) {
  ...
  u3_docx_set_cell_text(xml_doc, 2, 1, 2, f$chart_period)
  u3_docx_set_cell_text(xml_doc, 2, 2, 2, f$expected_duration)
  u3_docx_set_cell_text(xml_doc, 2, 3, 2, f$initial_risk)
  u3_docx_set_cell_text(xml_doc, 2, 4, 2, f$initial_reward)
  u3_docx_set_cell_text(xml_doc, 2, 5, 2, f$initial_rrr)

  u3_docx_set_cell_text(xml_doc, 3, 2, 1, f$entry_date)
  u3_docx_set_cell_text(xml_doc, 3, 2, 2, f$entry_time)
  u3_docx_set_cell_text(xml_doc, 3, 2, 3, f$entry_bs)
  u3_docx_set_cell_text(xml_doc, 3, 2, 4, f$entry_asset)
  u3_docx_set_cell_text(xml_doc, 3, 2, 5, f$entry_lots)
  u3_docx_set_cell_text(xml_doc, 3, 2, 6, f$entry_price)
  u3_docx_set_cell_text(xml_doc, 3, 2, 7, f$entry_stop)
  u3_docx_set_cell_text(xml_doc, 3, 2, 8, f$entry_target)

  u3_docx_set_cell_text(xml_doc, 4, 1, 2, f$setup_fundamental)
  u3_docx_set_cell_text(xml_doc, 4, 2, 2, f$setup_technical)
  u3_docx_set_cell_text(xml_doc, 4, 3, 2, f$trigger)
  u3_docx_set_cell_text(xml_doc, 4, 4, 2, f$execution)
  u3_docx_set_cell_text(xml_doc, 4, 5, 2, f$reason_stop)
  u3_docx_set_cell_text(xml_doc, 4, 6, 2, f$reason_target)

  if (nzchar(f$actual_duration %||% "")) {
    u3_docx_set_cell_text(xml_doc, 5, 1, 2, paste(f$actual_duration, "hours"))
  }
  exit_df <- f$exit_df
  n_exits <- max(1, nrow(exit_df))
  u3_docx_ensure_table_rows(xml_doc, 5, 3 + n_exits)
  for (i in seq_len(n_exits)) {
    row_i <- 3 + i
    for (col_i in seq_len(9)) {
      u3_docx_set_cell_text(xml_doc, 5, row_i, col_i, exit_df[i, col_i])
    }
  }

  u3_docx_set_cell_text(xml_doc, 6, 1, 2, f$reason_exit)
  u3_docx_set_cell_text(xml_doc, 6, 2, 2, f$lessons_learned)
      ...
      tabBox(id = ns("subtabs"), width = 12, selected = "3. Live Signals",
        ...
        tabPanel("6. Morning Sheet", icon = icon("sun"),
          ...
          box(width = 12, status = "primary", solidHeader = TRUE, title = "Save location",
            ...
            textInput(ns("templateFolder"), "Folder containing the two template .docx files", width = "100%",
              value = "C:/Users/jfz00/Dropbox/London Academy of Trading/Jose-F_Zubizarreta/Templates"),
            uiOutput(ns("templateStatus")),
            u3_context("Point this at wherever ", tags$code("Morning_Sheet_Template.docx"), " and ",
                       tags$code("Trade_Sheet_-_Template.docx"), " live \u2014 defaults to your Dropbox ",
                       "Templates folder so they stay backed up and synced like everything else. ",
        ...
        tabPanel("7. Individual Trade Sheet", icon = icon("file-signature"),
          ...
          box(width = 12, status = "primary", solidHeader = TRUE, title = "Save location",
            ...
            u3_context("The templates folder itself is set once, on the Morning Sheet tab's ",
                       "Save location box \u2014 it's shared by both tabs.")
          ),
          fluidRow(
...
unit3_live_signals_server <- function(id, data_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    # Live validation: confirms both template files exist AND can actually be opened
    # by officer::read_docx() (not just "a file exists at this path") - re-checks
    # automatically whenever the templates folder path changes. Reading a small .docx
    # is cheap (well under a second), so this simply reacts directly rather than
    # debouncing, avoiding timer-based reactives that are awkward to verify reliably.
    template_check <- reactive({
      folder <- input$templateFolder %||% ""
      files <- c("Morning_Sheet_Template.docx", "Trade_Sheet_-_Template.docx")
      lapply(files, function(fn) {
        path <- file.path(folder, fn)
        if (!file.exists(path)) {
          return(list(file = fn, ok = FALSE, msg = "not found at this path."))
        }
        res <- tryCatch({
          d <- officer::read_docx(path)
          n_tbl <- length(unique(officer::docx_summary(d)$doc_index[officer::docx_summary(d)$content_type == "table cell"]))
          list(ok = TRUE, msg = paste0("found and opened successfully (", n_tbl, " table region(s) detected)."))
        }, error = function(e) {
          list(ok = FALSE, msg = paste("found, but could not be opened as a valid .docx:", conditionMessage(e)))
        })
        list(file = fn, ok = res$ok, msg = res$msg)
      })
    })

    output$templateStatus <- renderUI({
      checks <- template_check()
      tagList(lapply(checks, function(c) {
        col <- if (isTRUE(c$ok)) "#1e7a46" else "#c0392b"
        ic  <- if (isTRUE(c$ok)) "circle-check" else "circle-xmark"
        tags$div(style = paste0("display:flex; align-items:flex-start; gap:8px; padding:3px 0; ",
                                 "font-size:12px; color:", col, ";"),
          icon(ic, style = "margin-top:2px;"),
          tags$span(tags$b(c$file), ": ", c$msg)
        )
      }))
    })
