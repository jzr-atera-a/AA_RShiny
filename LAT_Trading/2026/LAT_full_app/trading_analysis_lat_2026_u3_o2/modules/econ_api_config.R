...
econ_bigquery_auth_ui <- function(id) {
  ...
  tagList(
    ...
    ua_callout(paste0(
      "Connects to the shared GCP project (\u201catera-2\u201d, same project used by the FlexTable and ",
      "Business Operations apps) using a service-account JSON key. This app writes to its own dataset, ",
      "kept separate from other apps' data. The table is created automatically on first connect if it ",
      "doesn't already exist \u2014 nothing already stored is ever altered or dropped."
    )),
    fluidRow(
      box(title = "Connection Settings", status = "primary", solidHeader = TRUE, width = 5,
          ...
          textInput(ns("dataset_id"), "BigQuery Dataset:", value = "trading_economic_calendar"),
          tags$hr(),
