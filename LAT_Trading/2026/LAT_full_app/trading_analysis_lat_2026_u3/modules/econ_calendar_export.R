...
econ_calendar_export_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$exportBtn, {
      ...
      df <- current_batch()
      cat("[Export] Current batch:", nrow(df), "row(s)\n")
      if (nrow(df) == 0) {
        cat("[Export] ABORTED: nothing loaded\n")
      ...
      if (!isTRUE(api_manager$bq_authenticated)) {
        cat("[Export] ABORTED: BigQuery not configured\n")
      ...
      tryCatch({
        ...
        cat("[Export] SUCCESS \u2014", n, "row(s) written to", api_manager$bq_full_table_economic_calendar, "\n")
        ...
        cat("========== [Export] EXPORT COMPLETE ==========\n")
      }, error = function(e) {
        cat("[Export] EXPORT FAILED:", e$message, "\n")
        cat("========== [Export] EXPORT FAILED ==========\n")
