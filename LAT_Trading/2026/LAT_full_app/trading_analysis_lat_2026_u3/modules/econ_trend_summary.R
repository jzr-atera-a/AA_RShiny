...
etc_build_prompt <- function(query_date, num_events = 8) {
  ...
  # Split the requested total roughly evenly across the two weeks, current
  # week gets the extra one on an odd total.
  events_prev <- num_events %/% 2
  events_cur  <- num_events - events_prev
  ...
  paste0(
    ...
    "Return a TOTAL of approximately ", num_events, " events across both weeks combined \u2014 roughly ", events_prev,
    " for the previous week and ", events_cur, " for the current week. Prioritise the events that genuinely ",
    "moved or are most likely to move markets (MAIN role) first; only include a supporting CONTEXT event ",
    "(one that helps explain or forecast a MAIN event, e.g. a prior week's ADP print ahead of Non-Farm ",
    "Payrolls) if it meaningfully adds to the picture, and never pad the count with minor or duplicate ",
    "events just to reach the total \u2014 fewer genuinely significant events is better than hitting the number ",
    "with filler.\n\n",
    ...
    "Stop once you have covered approximately ", num_events, " events in total \u2014 do not exceed it."
  )
}
  ...
  tagList(
    ...
    fluidRow(
      box(title = "Generate", status = "primary", solidHeader = TRUE, width = 4,
          ...
          sliderInput(ns("numEvents"), "Number of events to include:",
                      min = 4, max = 40, value = 8, step = 1),
          tags$p("Rows returned = events \u00d7 4 asset classes (e.g. 8 events \u2192 up to 32 rows). Higher values take longer to generate.",
                 style = "font-size:11px; color:#888; margin-top:-8px; margin-bottom:10px;"),
...
econ_trend_summary_server <- function(id, api_manager) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$generate, {
      cat("\n========== [Trend Summary] GENERATE BUTTON CLICKED ==========\n")
      cat("[Trend Summary] Query date input:", as.character(input$queryDate), "\n")
      cat("[Trend Summary] Claude authenticated:", isTRUE(api_manager$claude_authenticated), "\n")

      if (!isTRUE(api_manager$claude_authenticated)) {
        cat("[Trend Summary] ABORTED: Claude not configured\n")
      ...
      tryCatch({
        cat("[Trend Summary] Building prompt for", input$numEvents, "events...\n")
        prompt <- etc_build_prompt(input$queryDate, input$numEvents)
        cat("[Trend Summary] Prompt length:", nchar(prompt), "chars\n")

        # Scale the token budget with the requested event count so small
        # requests aren't over-provisioned and large ones (up to 40 events)
        # aren't truncated: ~450 tokens/event (4 asset lines + description)
        # plus a fixed overhead, floor 4000 / ceiling 20000.
        dynamic_tokens <- max(4000, min(20000, input$numEvents * 450 + 1500))
        cat("[Trend Summary] Using max_tokens:", dynamic_tokens, "\n")

        cat("[Trend Summary] Calling api_manager$call_claude()...\n")
        raw <- withProgress(message = "Generating trend summary...", value = 0, {
          incProgress(0.05, detail = "Connecting to Claude API...")
          api_manager$call_claude(
            prompt, max_tokens = dynamic_tokens, enable_web_search = TRUE,
            progress_callback = function(msg) {
              # Fires periodically while the SSE stream is being read (see
              # call_claude()'s chunk loop) - nudges the bar forward and
              # shows a live chunk count so the UI never looks frozen during
              # what can be a 60-130s web-search-backed generation.
              incProgress(0.01, detail = msg)
            }
          )
        })
        cat("[Trend Summary] Raw response length:", nchar(raw), "chars\n")

        cat("[Trend Summary] Parsing response...\n")
        parsed <- etc_parse_response(raw, input$queryDate)
        cat("[Trend Summary] Parsed", nrow(parsed), "row(s),", ncol(parsed), "column(s)\n")
        ...
        cat("[Trend Summary] Stored on api_manager (last_query_id/last_query_date/last_raw_response/last_parsed_df)\n")
        ...
        if (nrow(parsed) == 0) {
          cat("[Trend Summary] WARNING: 0 rows parsed from a non-empty response \u2014 check the delimited format\n")
        ...
        cat("========== [Trend Summary] GENERATE COMPLETE ==========\n")
      }, error = function(e) {
        cat("[Trend Summary] GENERATE FAILED:", e$message, "\n")
        cat("========== [Trend Summary] GENERATE FAILED ==========\n")
