  ...
  fluidPage(
    tags$head(
      tags$style(HTML("
        ...
        .login-btn {
          background: linear-gradient(45deg, #008A82, #00A39A);
          color: white;
          border: none;
          padding: 12px 30px;
          border-radius: 8px;
        ...
        .login-btn:hover {
          ...
          box-shadow: 0 5px 15px rgba(0, 163, 154, 0.4);
        }
...
# Main UI that switches between login and dashboard
ui <- uiOutput("main_ui")
...
server <- function(input, output, session) {
  ...
  # Reactive data based on filters (only when authenticated)
  ...
  # Value boxes (only render when authenticated)
  ...
  # Overview plots (only render when authenticated)
  ...
  # Energy analysis plots (only render when authenticated)
  ...
  # Provider comparison plots (only render when authenticated)
  ...
  output$radar_chart <- renderPlotly({
    ...
    data <- aieei_data[aieei_data$Provider %in% input$selected_providers[1:min(3, length(input$selected_providers))], ]
  ...
  # Ecosystem mapping (only render when authenticated)
  ...
  # Methodology tab outputs (only render when authenticated)
  ...
  output$sensitivity_analysis <- renderPlotly({
    ...
    sensitivity_data <- data.frame(
      ...
      AIEEI_Impact = c(
        c(-0.15, -0.08, 0, 0.08, 0.15),  # CapEx/MW
  ...
  # Reactive recalculation when parameters change (only when authenticated)
  observeEvent(input$recalculate, {
    ...
    if(!is.null(input$capex_mw) && !is.null(input$alpha_beta)) {
      ...
      showNotification("AIEEI recalculated with new parameters!", type = "message")
    }
  })
  
  # Session timeout (optional security feature)
  session_timer <- reactiveVal(Sys.time())
  
  observe({
    invalidateLater(60000)  # Check every minute
    if (authenticated()) {
      if (difftime(Sys.time(), session_timer(), units = "hours") > 2) {  # 2 hour timeout
        authenticated(FALSE)
        showNotification("Session expired. Please log in again.", type = "warning", duration = 5)
        session$reload()
      }
    }
  })
  
  # Update session timer on any input activity
  observe({
    lapply(names(input), function(x) {
      input[[x]]
      session_timer(Sys.time())
    })
  })
}
