...
# Define UI
ui <- dashboardPage(
  dashboardHeader(title = "FX Analysis Dashboard - MySQL (Bid/Ask Filtered)"),
...
server <- function(input, output, session) {
  ...
  output$marketStats <- renderDT({
    ...
    stats <- data.frame(
      Metric = c("Current_Mid", "Current_Bid", "Current_Ask", "Min_Mid", "Max_Mid", "Avg_Mid", "Price_Range", "Observations"),
      Value = as.character(c(
        round(tail(price_series, 1), 6),
        round(tail(data$Bid, 1), 6),
        round(tail(data$Ask, 1), 6),
        round(min(price_series, na.rm = TRUE), 6),
        round(max(price_series, na.rm = TRUE), 6),
        round(mean(price_series, na.rm = TRUE), 6),
  ...
  output$bidAskStats <- renderDT({
    ...
    bid_ask_stats <- data %>%
      summarise(
        Current_Spread_Abs = round(tail(Ask, 1) - tail(Bid, 1), 6),
        Current_Spread_Pct = round((tail(Ask, 1) - tail(Bid, 1)) / tail(Mid, 1) * 100, 4),
        Mean_Spread_Abs = round(mean(spread, na.rm = TRUE), 6),
        Mean_Spread_Pct = round(mean(spread_pct, na.rm = TRUE), 4),
        Max_Spread_Pct = round(max(spread_pct, na.rm = TRUE), 4),
        Min_Spread_Pct = round(min(spread_pct, na.rm = TRUE), 4),
        Spread_Volatility = round(sd(spread_pct, na.rm = TRUE), 4)
      ) %>%
      pivot_longer(everything(), names_to = "Metric", values_to = "Value")
    
    datatable(bid_ask_stats, options = list(dom = 't'), rownames = FALSE)
  })
