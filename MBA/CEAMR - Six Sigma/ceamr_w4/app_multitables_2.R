                  ...
                  box(
                    ...
                    div(style = "background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin: 10px 0;",
                        ...
                        conditionalPanel(
                          ...
                          p("Required columns: date, Mid, Bid, Ask, pair, time, Timestamp"),
                          p("Note: All fields including time and Timestamp must be provided in the CSV file")
                        )
                    ),
...
server <- function(input, output, session) {
  ...
  # UPDATED UPLOAD LOGIC - NO FIELD DERIVATION FOR DAILY TABLE
  observeEvent(input$uploadData, {
    ...
    tryCatch({
      ...
      if (SELECTED_TABLE == "fx_spot_prices_daily") {
        ...
        # DAILY TABLE PROCESSING - NO FIELD DERIVATION
        showNotification("Processing for DAILY table (fx_spot_prices_daily)", type = "message")
        
        # Check required columns for daily table - ALL FIELDS MUST BE PROVIDED
        required_cols <- c("date", "Mid", "Bid", "Ask", "pair", "time", "Timestamp")
        ...
        # Convert date to proper format only
        ...
        showNotification("Using provided time and Timestamp fields from CSV", type = "message")
      ...
      for (i in seq(1, total_rows, chunk_size)) {
        ...
        if (SELECTED_TABLE == "fx_spot_prices_daily") {
          ...
          # DAILY TABLE INSERT - USING PROVIDED TIME AND TIMESTAMP
          values_list <- apply(chunk_data, 1, function(row) {
            ...
            time_val <- paste0("'", row["time"], "'")  # Use provided time
            timestamp_val <- ifelse(is.na(row["Timestamp"]) || row["Timestamp"] == "NA", "NULL", 
                                    paste0("'", format(as.POSIXct(row["Timestamp"]), "%Y-%m-%d %H:%M:%S"), "'"))
  ...
  observeEvent(input$queryTemplate2, {
    updateTextAreaInput(session, "sqlQuery", 
                        value = "SELECT * FROM fx_spot_prices_daily ORDER BY date DESC LIMIT 100;")
  })
