...
server <- function(input, output) {
  ...
  # Tab 5: Experiential Learning - Updated with text above circles
  output$projects_timeline <- renderPlotly({
    # Create milestone data for project workflow
    milestones_data <- data.frame(
      Step = 1:12,
      Milestone = c("Information\nSession", "Project\nSourcing", "Application\nWindow", 
                    "Team\nAssignment", "Client\nIntroduction", "PID\nDevelopment",
                    "Risk\nAssessment", "Project\nExecution", "Client\nFeedback",
                    "Final\nReport", "Assessment", "Reflection/\nCompletion"),
      Project_Type = c("GCP", "GCP", "GCP", "GCP", "GCP", "All Projects",
                       "All Projects", "All Projects", "GCP", "All Projects", "All Projects", "Summer"),
      Description = c("Attend mandatory information sessions",
                      "Apply for projects or source own contacts",
                      "Submit applications during open window",
                      "Formal assignment by Projects Team",
                      "Meet client and supervisor",
                      "Complete Project Initiation Document",
                      "Submit individual risk assessment",
                      "4-6 weeks full-time project work",
                      "Client provides performance feedback",
                      "Submit final report to faculty",
                      "Faculty assessment and grading",
                      "Reflective report for Summer activities"),
      Color = c("#FF8C00", "#FF8C00", "#FF8C00", "#FF8C00", "#FF8C00", "#FFD700",
                "#FFD700", "#B8860B", "#FF8C00", "#FFD700", "#FFD700", "#DAA520"),
      # Create stepped diagonal y-coordinates for circles
      Y_Position = c(1, 1.3, 1.6, 1.9, 2.2, 2.5, 2.8, 3.1, 3.4, 3.7, 4.0, 4.3),
      # Text positioned above circles
      Y_Text = c(1.4, 1.7, 2.0, 2.3, 2.6, 2.9, 3.2, 3.5, 3.8, 4.1, 4.4, 4.7)
    )
    
    # Create the stepped diagonal milestone timeline plot
    p <- plot_ly() %>%
      # Add connecting lines between milestones first (so they appear behind circles)
      add_trace(data = milestones_data, x = ~Step, y = ~Y_Position, 
                type = 'scatter', mode = 'lines',
                line = list(width = 4, color = '#B8860B', dash = 'solid'),
                showlegend = FALSE, hoverinfo = 'skip', name = 'connector') %>%
      # Add milestone circles
      add_trace(data = milestones_data, x = ~Step, y = ~Y_Position, 
                type = 'scatter', mode = 'markers',
                marker = list(size = 20, color = ~Color, 
                              line = list(width = 3, color = '#2F4F4F')),
                hovertemplate = paste("<b>", milestones_data$Milestone, "</b><br>",
                                      "Step %{x}<br>",
                                      "Type: ", milestones_data$Project_Type, "<br>",
                                      "%{customdata}<br>",
                                      "<extra></extra>"),
                customdata = ~Description,
                showlegend = FALSE, name = 'milestones') %>%
      # Add text labels above circles
      add_trace(data = milestones_data, x = ~Step, y = ~Y_Text, 
                type = 'scatter', mode = 'text',
                text = ~Milestone, 
                textfont = list(size = 11, color = '#2F4F4F', family = 'Arial Bold'),
                showlegend = FALSE, hoverinfo = 'skip', name = 'labels') %>%
      layout(title = list(text = "Project Milestone Timeline - Sequential Steps", 
                          font = list(size = 16, color = '#2F4F4F')),
             xaxis = list(title = "Sequential Steps", 
                          titlefont = list(size = 14, color = '#2F4F4F'),
                          tickmode = 'linear', tick0 = 1, dtick = 1,
                          range = c(0.5, 12.5),
                          tickfont = list(size = 12, color = '#2F4F4F')),
             yaxis = list(title = "", showticklabels = FALSE, 
                          range = c(0.2, 5.5), zeroline = FALSE, showgrid = FALSE),
             plot_bgcolor = '#FFF8DC',
             paper_bgcolor = '#FFF8DC',
             showlegend = FALSE,
             margin = list(t = 80, b = 60, l = 60, r = 60),
             annotations = list(
               # Phase labels positioned at bottom to avoid overlap
               list(x = 2.5, y = 0.4, text = "<b>GCP Process</b>", 
                    showarrow = FALSE, font = list(size = 12, color = "#FF8C00")),
               list(x = 6.5, y = 0.5, text = "<b>All Projects</b>", 
                    showarrow = FALSE, font = list(size = 12, color = "#FFD700")),
               list(x = 10, y = 0.6, text = "<b>Completion</b>", 
                    showarrow = FALSE, font = list(size = 12, color = "#B8860B"))
             ))
    p
  })
