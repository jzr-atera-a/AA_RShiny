    ...
    tags$head(
      ...
      tags$script(HTML("
        $(document).ready(function() {
          // Force sidebar styling after page load
          setTimeout(function() {
            $('.main-sidebar').css({
              'background-color': '#F8F9FA',
              'background': '#F8F9FA'
            ...
            // Add yellow header section
            if (!$('.sidebar-yellow-header').length) {
              $('.main-sidebar').prepend('<div class=\"sidebar-yellow-header\" style=\"position: absolute; top: 0; left: 0; right: 0; height: 50px; background-color: #F2A600; z-index: 999;\"></div>');
            }
            
            // Style menu items
            $('.sidebar-menu > li').css('margin-bottom', '8px');
            ...
            // Style the sidebar menu container
            $('.sidebar-menu').css({
              'margin-top': '60px',
              'padding': '0'
            });
...
server <- function(input, output, session) {
  ...
  output$competitive_landscape <- renderPlotly({
    ...
    if(nrow(data) > 0) {
      ...
      p <- plot_ly(data, x = ~intelligence_score, y = ~efficiency,
                   color = ~provider, size = ~context_window,
                   text = ~paste("Model:", model_name, "<br>Provider:", provider),
                   type = 'scatter', mode = 'markers',
                   marker = list(opacity = 0.8, sizemode = 'diameter',
                                 sizeref = max(data$context_window) / 50)) %>%
        layout(title = list(text = "Competitive Positioning: Performance vs Efficiency", font = list(size = 16)),
               xaxis = list(title = "Intelligence Score"),
